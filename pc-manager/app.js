...
{id:"eier",sortOrder:1,name:"Eierlikörpunsch",category:"Getränke",price:4.5,receiptText:"Eierlikörpunsch",image:"assets/eierpunsch_version_3.png",active:true,quantity:1,unit:"Becher",taxRate:19},
{id:"roterfeger",sortOrder:5,name:"Roter Feger",category:"Getränke",price:2.5,receiptText:"Roter Feger",image:"assets/roter_feger_version_3.png",active:true,quantity:1,unit:"Becher",taxRate:19},
{id:"apfel",sortOrder:6,name:"Apfelpunsch",category:"Getränke",price:2.5,receiptText:"Apfelpunsch",image:"assets/apfelpunsch_version_3.png",active:true,quantity:1,unit:"Becher",taxRate:19},
{id:"schussrum",sortOrder:9000,name:"Schuss Rum",category:"Getränke",price:1,receiptText:"Schuss Rum",image:"assets/rum_version_3.png",active:true,quantity:1,unit:"Becher",taxRate:19},
{id:"schussamaretto",sortOrder:9001,name:"Schuss Amaretto",category:"Getränke",price:1,receiptText:"Schuss Amaretto",image:"assets/amaretto_version_3.png",active:true,quantity:1,unit:"Becher",taxRate:19},
{id:"grot",sortOrder:2,image:"assets/gluehwein_version_3.png",name:"Glühwein rot",category:"Getränke",price:3.50,receiptText:"Glühwein rot",optionGroup:"shot",color:"#8b1e24",active:true,quantity:1,unit:"Becher",taxRate:19,info:{allergens:"Enthält Sulfite"}},
{id:"gweiss",sortOrder:3,image:"assets/gluehwein_weiss_version_3.png",name:"Glühwein weiß",category:"Getränke",price:3.50,receiptText:"Glühwein weiß",optionGroup:"shot",color:"#a36a20",active:true,quantity:1,unit:"Becher",taxRate:19},
{id:"feuer",sortOrder:4,image:"assets/feuerzangenbowle_version_3.png",name:"Feuerzangenbowle",category:"Getränke",price:5,receiptText:"Feuerzangenbowle",depositRule:"automatic",depositComponents:[{name:"Glaspfand",price:2},{name:"Feuerzangenpfand",price:2}],color:"#a43f18",active:true,quantity:1,unit:"Glas",taxRate:19},
...
let accounts=JSON.parse(localStorage.getItem("kcm_accounts")||"null")||DEFAULT_ACCOUNTS.map(a=>({...a}));
{
  const st=DEFAULT_ACCOUNTS[0],i=accounts.findIndex(a=>a.id===st.id);
  if(i<0)accounts.push({...st}); else accounts[i]={...accounts[i],...st,name:"Stadtmarketing Werne"};
  localStorage.setItem("kcm_accounts",JSON.stringify(accounts));
}
...
window.KCImagesV3.apply(articles);
...
function renderArticles(){const body=el("articleBody");const gruppenRang=new Map(groups.slice().sort((a,b)=>a.sortOrder-b.sortOrder).map((g,i)=>[g.name,i]));const rows=articles.map((a,i)=>({a,i})).sort((x,y)=>(gruppenRang.get(x.a.category)??999)-(gruppenRang.get(y.a.category)??999)||(x.a.sortOrder>0?x.a.sortOrder:10000+x.i)-(y.a.sortOrder>0?y.a.sortOrder:10000+y.i));body.innerHTML=rows.map(({a,i})=>`<tr data-i="${i}"><td>${a.id}</td><td>${a.name}</td><td>${a.category}</td><td>${money(a.price)}</td><td><input type="checkbox" data-article-pl="${i}" ${a.priceListVisible!==false?"checked":""} aria-label="${escAttr(a.name)} in Präsentations-Preislisten anzeigen" title="In Präsentations-Preislisten anzeigen"></td></tr>`).join("");body.querySelectorAll("tr").forEach(r=>r.onclick=()=>loadArticle(+r.dataset.i));body.querySelectorAll("[data-article-pl]").forEach(box=>box.onclick=event=>{event.stopPropagation();const article=articles[Number(box.dataset.articlePl)];if(!article)return;article.priceListVisible=box.checked;queueSync("article","upsert",article);saveAll();globalThis.KCManagerMessages?.success?.(`${article.name}: Präsentations-Preisliste ${box.checked?"eingeschaltet":"ausgeschaltet"}.`)})}
...
document.getElementById('masterDataPushBtn')?.addEventListener('click', () => requireAuth(async () => {
  ...
  try {
    const antwort = await fetch('http://127.0.0.1:47392/master-data/push', {
      ...
      body: JSON.stringify({ groups, articles, packages: [], accounts, settings }),
    });
    if (!antwort.ok) throw new Error('Manager antwortete mit ' + antwort.status);
    const daten = await antwort.json();
    statusFeld.textContent = `Gesendet - Stand Nr. ${daten.revision}: Warengruppen, Artikel, Konten und Darstellung. `
