(function (global) {
  ...
  function createEvent(input, existing = []) {
    ...
    // 09.09.2026: die Kasse rundet jetzt selbst fair auf die Viertelstunde (siehe pos/time-
    // clock-pos.js rundeGerecht) - das allein kann schon bis knapp unter 15 Minuten vom echten
    // Scanzeitpunkt abweichen, OHNE dass das ein Grund zur Sorge waere (im Gegenteil, genau das
    // ist gewollt). Die Schwelle war vorher 5 Minuten und haette jede normale Rundung
    // faelschlich abgelehnt - jetzt 15, deckt die faire Rundung ab, verlangt aber weiterhin
    // einen Grund bei echten, groesseren Abweichungen (z.B. Abloesung kommt spaeter).
    if (differenceMinutes > 15 && !correctionReason) throw new Error('Bei abweichender Uhrzeit ist ein Korrekturgrund erforderlich.');
    return {
