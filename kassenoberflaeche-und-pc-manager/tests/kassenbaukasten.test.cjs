...
(async () => {
  ...
  /* 07.09.2026 (Betreiber): "Laufschrift haben wir auf Kasse nicht. Die muss raus." Die Kachel
     des TV-Teils darf im Kassenmodus nicht mehr in der Bibliothek stehen - im TV-Modus aber
     unverändert bleiben. */
  const lauf = await pg.evaluate(() => {
    const el = document.getElementById('tickerQuickStart');
    const btn = document.getElementById('quickTickerBtn');
    return { da: !!el, sichtbar: !!el && el.offsetParent !== null && getComputedStyle(el).display !== 'none',
      knopfSichtbar: !!btn && btn.offsetParent !== null };
  });
  p('Die Laufschrift-Kachel ist im Kassenmodus weg', lauf.da && !lauf.sichtbar && !lauf.knopfSichtbar,
    lauf.da ? 'Element vorhanden, aber ausgeblendet' : 'Element fehlt ganz');
  const laufTv = await pg.evaluate(() => {
    const sel = document.getElementById('modeSelect');
    const vorher = sel.value;
    sel.value = 'tv'; sel.dispatchEvent(new Event('change', { bubbles: true }));
    const el = document.getElementById('tickerQuickStart');
    const sichtbar = !!el && getComputedStyle(el).display !== 'none';
    sel.value = vorher; sel.dispatchEvent(new Event('change', { bubbles: true }));
    return sichtbar;
  });
  p('Im TV-Modus ist sie weiterhin da', laufTv, laufTv ? 'sichtbar' : 'fehlt');
