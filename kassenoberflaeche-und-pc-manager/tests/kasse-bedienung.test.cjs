...
// 10.09.2026 (Betreiber: "nach jeder Buchung ein Fenster mit gruenem Haken... soll von alleine
// weggehen"): die Abschlussmeldung ist jetzt bewusst kein Fenster mehr, das man wegtippen
// muss, sondern dieselbe Meldungszeile wie ueberall sonst (siehe app.js completeSale) - hier
// entsprechend an der Meldungszeile statt an einem offenen <dialog> geprueft.
await p.evaluate(()=>document.getElementById('cashChangeBtn').click());
await p.waitForTimeout(1500);
const zwischen=await p.evaluate(()=>{
  const n=document.getElementById('notificationBar');
  return {meldung:n?n.textContent:null,
...
pruefe('Neuer BAR-Knopf schließt den Bon wirklich ab',/verbucht/.test(zwischen.meldung||'')&&/^0,00/.test((zwischen.gesamt||'').trim()),zwischen.meldung||'keine Meldung');
