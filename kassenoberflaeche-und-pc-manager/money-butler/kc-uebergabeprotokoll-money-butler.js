// Archiv der Übergabeprotokolle - hierher portiert aus dem PC-Manager (dort:
// kc-uebergabeprotokoll-manager.js), 10.09.2026 (Betreiber: "beide sollen zu 100% gleich sein").
//
...
(function (global) {
  ...
  if (!modul || !el('mbProtokolle')) return;
  ...
  function meldung(text, art) {
    const feld = el('mbProtokollMeldung');
  ...
  async function uebernehmen() {
    const eingabe = el('mbProtokollEingabe');
  ...
  el('mbProtokollUebernehmen').addEventListener('click', uebernehmen);
  el('mbProtokollAktualisieren').addEventListener('click', () => { meldung(''); laden(); });
  // Ein Handscanner tippt den Code und schliesst mit Enter ab - das soll direkt übernehmen.
  el('mbProtokollEingabe').addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); uebernehmen(); } });
