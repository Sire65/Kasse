(function(global){
  ...
  class KCCommunicationClient{
    ...
    async uploadAttachment(file,{orgId=null,expiresInHours=168}={}){
      ...
      form.append('sourceProgram',this.sourceProgram);
      if(orgId) form.append('orgId',String(orgId));
      form.append('expiresInHours',String(expiresInHours));
      const name=(typeof File!=='undefined'&&file instanceof File&&file.name)?file.name:'attachment.bin';
      form.append('file',file,name);
      const data=await this._requestForm('kc-communication-attachments',form);
      return data?.attachment||null;
    }

    async uploadAttachments(files,{orgId=null,expiresInHours=168}={}){
      const list=Array.from(files||[]);
      if(list.length>10) throw new KCCommunicationError('Maximal 10 Anhänge pro Nachricht','TOO_MANY_ATTACHMENTS',400);
      const out=[];
      for(const file of list){
        const uploaded=await this.uploadAttachment(file,{orgId,expiresInHours});
    ...
    async deleteAttachment(attachmentId,{orgId=null}={}){
      ...
      if(orgId) form.append('orgId',String(orgId));
      return this._requestForm('kc-communication-attachments',form);
    }
    ...
    async emitEvent(eventKey,{recipients=[],variables={},attachmentIds=[],priority='normal',testOnly=this.defaultTestOnly,correlationId=null,orgId=null}={}){
      ...
      const ids=[...new Set((Array.isArray(attachmentIds)?attachmentIds:[]).filter(Boolean).map(String))].slice(0,10);
      if(ids.length) vars.attachmentIds=ids;
      const payload={
        sourceProgram:this.sourceProgram,
    ...
    async emitEventWithFiles(eventKey,{files=[],recipients=[],variables={},priority='normal',testOnly=this.defaultTestOnly,correlationId=null,orgId=null,expiresInHours=168}={}){
      const uploaded=await this.uploadAttachments(files,{orgId,expiresInHours});
      return this.emitEvent(eventKey,{
        recipients,
        variables,
        attachmentIds:uploaded.map(x=>x.id),
        priority,
        testOnly,
        correlationId,
        orgId
