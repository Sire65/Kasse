...
document.querySelectorAll("[data-date-quick]").forEach(b=>b.onclick=()=>{
  const versatz=Number(b.dataset.dateQuick);
  if(versatz<0&&currentType!=="count")return alert("„Gestern“ gibt es nur bei der Abendzählung - bei Anfangsbestand/Nachfüllung muss der Tag heute oder später sein.");
  const ziel=new Date(Date.now()+versatz*86400000);
  el("effectiveDate").value=localBusinessDate(ziel);
  el("effectiveDate").dispatchEvent(new Event("input"));
  clearTransferOutput();
});
...
function setDatePurpose(){
  el("effectiveDateLabel").textContent=currentType==="count"?"Zähltag *":"Gültig für *";
  // 09.09.2026 (Betreiber): Abendzählung passiert oft erst am naechsten Morgen - "Gestern"
  // muss dafuer waehlbar sein. Bei Anfangsbestand/Nachfuellung bleibt es bei "nicht vor heute"
  // (dort ergibt ein vergangener Tag keinen Sinn).
  el("effectiveDate").min=currentType==="count"?localBusinessDate(new Date(Date.now()-9*86400000)):localBusinessDate();
}
...
document.querySelectorAll(".choice").forEach(b=>b.onclick=()=>{document.querySelectorAll(".choice").forEach(x=>x.classList.toggle("active",x===b));currentType=b.dataset.type;setDatePurpose();pflegeKassettenAuswahl();zeichneAufteilung();zaehlBereicheZeigen();clearTransferOutput()});
document.querySelectorAll("[data-value],[data-roll-value]").forEach(n=>n.oninput=()=>{updateTotal();zeichneAufteilung();clearTransferOutput();zaehlVergleichZeichnen()});
...
// 09.09.2026 (Betreiber): "Money Butler soll das Soll kennen, damit man beim Zaehlen sofort die
// Differenz sieht - am Stand oder am naechsten Morgen, egal ob der Manager gerade erreichbar ist."
let sollAbschluss=null;   /* eingelesener KCLOSE1-Datensatz, oder null */
function zaehlBereicheZeigen(){
  const zaehlung=currentType==="count";
  el("sollSection").hidden=!zaehlung;el("belegeSection").hidden=!zaehlung;el("zaehlKontrolle").hidden=!zaehlung;
  if(!zaehlung){sollAbschluss=null;el("sollCode").value="";el("sollAnzeige").textContent="Noch kein Tagesabschluss eingelesen - die Zählung geht trotzdem, nur ohne sofortigen Vergleich.";}
  zaehlVergleichZeichnen();
}
el("sollUebernehmen").onclick=()=>{
  const text=el("sollCode").value.trim();
  ...
  try{
    ...
    sollAbschluss=payload;
    el("sollAnzeige").textContent=`Soll laut Kasse (${payload.registerName||payload.registerId}, ${String(payload.createdAt||"").slice(0,10)}): ${money(payload.expectedCash)}${Number.isFinite(payload.receiptExpected)?` · ${payload.receiptExpected} Beleg(e)`:""}`;
    if(payload.registerId)el("register").value=payload.registerId;
  }catch(err){sollAbschluss=null;el("sollAnzeige").textContent="Fehler: "+err.message}
  zaehlVergleichZeichnen();
};
el("belegeAnzahl")?.addEventListener("input",zaehlVergleichZeichnen);
function zaehlVergleichZeichnen(){
  if(currentType!=="count")return;
  const gezaehlt=getData().total;
  if(sollAbschluss){
    const diffGeld=+(gezaehlt-Number(sollAbschluss.expectedCash||0)).toFixed(2);
    el("kontrolleGeld").textContent=`${diffGeld===0?"stimmt genau":diffGeld>0?"+"+money(diffGeld)+" zu viel":money(Math.abs(diffGeld))+" fehlen"}`;
    el("kontrolleGeld").className=diffGeld===0?"ok":"warn";
  }else{el("kontrolleGeld").textContent="—";el("kontrolleGeld").className="";}
  const belegeSoll=sollAbschluss&&Number.isFinite(sollAbschluss.receiptExpected)?Number(sollAbschluss.receiptExpected):null;
  const belegeIst=Number(el("belegeAnzahl")?.value||0);
  if(belegeSoll!==null){
    const diffBelege=belegeIst-belegeSoll;
    el("kontrolleBelege").textContent=diffBelege===0?"stimmt genau":`${belegeIst} von ${belegeSoll} (${diffBelege>0?"+":""}${diffBelege})`;
    el("kontrolleBelege").className=diffBelege===0?"ok":"warn";
    el("belegeAnzeige").textContent=`Soll laut Kasse: ${belegeSoll} Beleg(e)`;
  }else{el("kontrolleBelege").textContent="—";el("kontrolleBelege").className="";el("belegeAnzeige").textContent="";}
}
zaehlBereicheZeigen();
function getData(){
...
// 10.09.2026 (Betreiber, nach echten Scan-Problemen: "der Code soll wirklich nur das
// schicken, was wirklich gebraucht wird"): baut aus den vollen, fuer Anzeige/Summen
// gebrauchten Daten (getData()) eine SCHLANKE Fassung NUR fuer den QR-Code selbst:
// 1) Stueckelungen mit Anzahl 0 werden komplett weggelassen (vorher standen alle 13
//    Schein-/Muenzwerte immer im Code, auch wenn leer).
// 2) Jede Muenzrolle wird nur noch als reine Zahl (Rollenanzahl) uebertragen statt als
//    5-Werte-Objekt - Muenzen je Rolle und Wert je Rolle sind aus der Rollenanzahl und der
//    ohnehin bekannten Rollengroesse (COIN_ROLLS oben) vollstaendig rekonstruierbar, genau
//    wie es "breakdown" vorher schon war. Kasse und Manager rechnen sich das beim Einlesen
//    zurueck (siehe pos/app.js applyCashPayload/splitAnteilFuer, pc-manager/app.js).
// Bewusst KEINE eigene, neu erfundene Kodierung (kein "2.1"-Schluesselsystem) - das haette
// Kasse UND Manager an mehreren Stellen gleichzeitig aendern muessen, bei kaum noch
// spuerbarem Zusatzgewinn gegenueber diesen zwei einfachen, risikoarmen Schritten.
...
el("generate").onclick=()=>{const c=getData(),schlank=schlankeStueckelung(c),effectiveDate=el("effectiveDate").value;if(!isBusinessDate(effectiveDate))return alert("Bitte das gültige Einsatzdatum im Kalender auswählen.");if(effectiveDate<localBusinessDate())return alert("Das Einsatzdatum darf nicht in der Vergangenheit liegen.");if(c.total<=0)return alert("Bitte mindestens eine Stückelung eingeben.");const payload={
  ...
  effectiveDate,
  // 09.09.2026 (Betreiber am echten Scanner: "JSON Parse error, Code bricht mittendrin ab") -
  // "breakdown" NICHT mehr mitschicken: es liess sich komplett aus looseBreakdown+coinRolls
  // zurueckrechnen (siehe pos/app.js splitAnteilFuer - genau das tut es dort schon fuer die
  // Geldkassette), war also reine Verdopplung. Kuerzt den QR-Code spuerbar (rund ein Drittel
  // weniger Zeichen bei einer realistischen Stueckelung), ohne dass Kasse oder Manager etwas
  // verlieren - sie rechnen sich das Feld beim Einlesen selbst wieder aus.
  looseBreakdown:schlank.looseBreakdown,
  coinRolls:schlank.coinRolls,
  looseTotal:c.looseTotal,
  rollTotal:c.rollTotal,
  total:c.total,
  note:el("note").value.trim(),
  receiptCount:currentType==="count"&&el("belegeAnzahl").value!==""?Number(el("belegeAnzahl").value):undefined,
  closingId:currentType==="count"&&sollAbschluss?sollAbschluss.closingId:undefined
};
...
// 10.09.2026 (Betreiber: "beide sollen zu 100% gleich sein") - derselbe WLAN-Direktversand,
// den der eingebaute Bereich im Manager schon hatte, hierher portiert. Liest die strukturierten
// Daten aus dem bereits erzeugten currentPayload (Base64-JSON) - keine zweite, eigene
// Dateneingabe. Nur nutzbar, wenn diese App auf demselben Rechner wie der Kassen-Companion
// laeuft (127.0.0.1) - fuer den Fall "Kassenwart zuhause" bleibt Datenbank/E-Mail der richtige
// Weg (siehe kc-money-butler-versand.js).
el("sendCashDirect")?.addEventListener("click",async()=>{
  const roh=currentPayload||"",trenner=roh.indexOf(":");
  if(trenner<0)return alert("Zuerst QR-Code erzeugen.");
  let payload;
  try{payload=JSON.parse(decodeURIComponent(escape(atob(roh.slice(trenner+1)))))}
  catch(e){return alert("Zuerst QR-Code erzeugen.")}
  const feld=el("sendCashDirectResult");
  ...
  try{
    ...
    feld.textContent=payload.scope==="split"
      ...
      :`Übergabe über ${money(payload.total)} an ${el("register").selectedOptions[0]?.textContent||payload.registerId} gesendet - wird von der Kasse innerhalb von 15 Sekunden abgeholt.`;
  }catch(e){
    feld.textContent="Konnte nicht gesendet werden - läuft diese App auf demselben Rechner wie der Kassen-Companion, im selben WLAN wie die Kasse? Ersatzweise QR-Code, Kurzcode oder Datei verwenden.";
  }
});
