(function(global){
  ...
  const DEFINITIONS={
    ...
    'kc-money-butler':{
      ...
      events:{
        ...
        // 10.09.2026 (Betreiber: "Money Butler koennte die Geldmenge an die Kassen senden"):
        // neues Ereignis fuer die Geldkassetten-/Anfangsbestand-Uebergabe an die Kassen.
        cash_transfer_ready:{required:['registerId','amount'],recipient:'central'}
      }
    },
    'kc-bilderrechner':{
      ...
      events:{
        ...
        // 10.09.2026 (Betreiber: "die Kassen koennten abends den Tagesabschluss an den
        // Kassenwart senden"): neues Ereignis fuer den Tagesabschluss-Versand.
        closing_report_ready:{required:['registerId','expectedCash'],recipient:'central'}
      }
    },
    'kc-wm':{
    ...
    'pc-backup-vault':{
      displayName:'PC Backup Vault',
      events:{
        backup_success:{required:[],recipient:'central'},
        backup_warning:{required:[],recipient:'central'},
        backup_failed:{required:[],recipient:'central'},
        backup_cancelled:{required:[],recipient:'central'},
        backup_interrupted:{required:[],recipient:'central'},
        backup_resumed:{required:[],recipient:'central'},
        verify_failed:{required:[],recipient:'central'},
        restore_test_failed:{required:[],recipient:'central'},
        tuev_failed:{required:[],recipient:'central'},
        storage_unreachable:{required:[],recipient:'central'},
        capacity_warning:{required:[],recipient:'central'},
        capacity_blocked:{required:[],recipient:'central'},
        scheduler_failed:{required:[],recipient:'central'},
        communication_test:{required:[],recipient:'central'}
      }
    }
  };
  ...
  class KCFachprogrammCommunicationAdapter{
    ...
    async uploadAttachment(file,options={}){
      return this.client.uploadAttachment(file,options);
    }

    async uploadAttachments(files,options={}){
      return this.client.uploadAttachments(files,options);
    }

    async deleteAttachment(attachmentId,options={}){
      return this.client.deleteAttachment(attachmentId,options);
    }
    ...
    async emit(eventKey,data={},options={}){
      ...
      const attachmentIds=[...new Set((options.attachmentIds||data.attachmentIds||[]).filter(Boolean).map(String))].slice(0,10);
      delete variables.attachmentIds;
    ...
    async emitWithFiles(eventKey,data={},files=[],options={}){
      const check=this.validate(eventKey,data);
      if(!check.ok){
        const err=new Error('Pflichtdaten fehlen: '+check.missing.join(', '));
        err.code=check.code; err.missing=check.missing; throw err;
      }
      const uploaded=await this.uploadAttachments(files,{orgId:options.orgId||data.orgId||null,expiresInHours:options.expiresInHours||168});
      return this.emit(eventKey,data,{...options,attachmentIds:uploaded.map(x=>x.id)});
    }
  }
