...
(function (global) {
  ...
  // Gutscheinnummer.
  //
  // FRUEHER: GS-<Jahr>-<lfd>-<Pruefzeichen>, wobei das Pruefzeichen die Quersumme der Zeichen
  // modulo 97 war und die laufende Nummer aus dem localStorage DIESER Kasse gezaehlt wurde.
  // Zwei Fehler auf einmal: die Endung kann sich jeder selbst ausrechnen, der das Muster
  // einmal sieht - und zwei Kassen am selben Stand vergeben zwangslaeufig dieselbe Nummer.
  //
  // JETZT: KC-<Jahr>-<lfd>, vom Manager vergeben und dort signiert. Die Kasse holt die
  // Nummer aus dem Vorrat ihres eigenen Companions, der ihn im Hintergrund auffuellt,
  // solange Verbindung besteht. Am Markttag laeuft die Ausgabe damit auch offline weiter.
  // Das Geheimnis fuer die Signatur kommt NIE auf das Tablet.
  const NOTNUMMER_MERKER = 'kc_voucher_notnummern_v1';

  async function nummerHolen() {
    const bauen = global.KCSyncConnection?.buildUrl;
    if (typeof bauen === 'function') {
      try {
        const antwort = await fetch(bauen('/kc-sync-voucher-number'), {signal: AbortSignal.timeout(4000)});
        if (antwort.ok) {
          const daten = await antwort.json();
          if (daten?.nummer && daten?.qr) return {code: daten.nummer, qr: daten.qr, vorrat: daten.vorrat};
        }
      } catch (e) { /* kein Companion erreichbar - unten weiter mit der Notnummer */ }
    }
    return null;
  }

  // Notnummer, wenn der Vorrat leer ist UND der Companion nicht erreichbar ist.
  //
  // Der Verkauf am Stand darf daran NICHT scheitern - ein wartender Gast ist schlimmer als
  // eine Nummer ohne Signatur. Sie ist deshalb bewusst als solche erkennbar (N statt einer
  // Ziffernfolge an dritter Stelle), traegt keinen QR-Code und faellt im Manager sofort auf.
  // Der Restwert ist trotzdem sicher: er wird gemeldet wie jeder andere Gutschein auch.
  function notNummer() {
    const jahr = new Date().getFullYear();
    let zaehler = 0;
    try { zaehler = Number(localStorage.getItem(NOTNUMMER_MERKER) || 0) + 1; } catch (e) { zaehler = 1; }
    try { localStorage.setItem(NOTNUMMER_MERKER, String(zaehler)); } catch (e) { /* nicht kritisch */ }
    const kasse = String(global.KCSyncConnection?.config?.registerId || 'X').replace(/[^A-Z0-9]/gi, '').slice(-2).toUpperCase() || 'X';
    return {code: `KC-${jahr}-N${kasse}${String(zaehler).padStart(3, '0')}`, qr: null, notnummer: true};
  }
  ...
  async function ausstellen(betrag, art) {
    const jetzt = new Date();
    const ablauf = new Date(jetzt); ablauf.setFullYear(ablauf.getFullYear() + GUELTIG_JAHRE);
    const nummer = (await nummerHolen()) || notNummer();
    const gutschein = {
      code: nummer.code,
      qr: nummer.qr || null,              // signierter Code fuer den QR auf dem Bon
      notnummer: !!nummer.notnummer,
  ...
  // Der Scanner liefert den vollen signierten Code, der Bediener tippt bei zerknittertem Bon
  // nur die Nummer ab. Beides muss zum selben Gutschein fuehren.
  function nummerAus(text) {
    const roh = String(text || '').trim().toUpperCase();
    return roh.startsWith('KCG1:') ? (roh.split(':')[1] || '') : roh;
  }
  const finde = (code) => { const n = nummerAus(code); return lies().find((g) => g.code.toUpperCase() === n); };
  ...
  function einloesen(code, betrag, angaben) {
    const liste = lies();
    const gesucht = nummerAus(code);
    const g = liste.find((x) => x.code.toUpperCase() === gesucht);
  ...
  global.KCGutschein = {
    ausstellen, einloesen, finde, nummerAus, alle: lies, zustand, ZUSTAND_TEXT, melden,
...
(function (global) {
  ...
  function druckeBon(g) {
    ...
    // In den QR gehoert der SIGNIERTE Code, nicht die blosse Nummer - sonst koennte sich
    // jeder einen QR mit einer Wunschnummer erzeugen. Bei einer Ersatznummer gibt es keinen
    // signierten Code; dann bleibt die Nummer gross und lesbar, der Gutschein ist von Hand
    // einloesbar statt unbrauchbar.
    const qr = g.qr ? qrBild(g.qr, 200) : null;
  ...
  // Die Nummer kommt jetzt aus dem Vorrat des Companions - das ist ein Netzaufruf, deshalb
  // asynchron. Der Knopf wird waehrenddessen gesperrt: sonst erzeugt ein zweiter Tipp einen
  // zweiten Gutschein, waehrend der erste noch unterwegs ist.
  async function ausstellen() {
    ...
    knopf.disabled = true;
    let g;
    try { g = await K.ausstellen(betrag, 'gutschein'); }
    ...
    melde('gsErgebnis', 'gut',
      ...
      + (g.notnummer ? '<em>Ersatznummer ohne Prüfcode – die Sicherung war gerade nicht erreichbar. Der Gutschein gilt trotzdem.</em><br>' : '')
