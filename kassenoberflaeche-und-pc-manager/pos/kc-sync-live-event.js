...
(function (global) {
  ...
  function sendeLebenszeichen() {
    try {
      // 10.09.2026: trafficTx zaehlt NUR echte, vom Companion bestaetigte Uebertragungen seit
      // dem letzten Heartbeat (siehe sendeEinmal weiter unten) - danach zurueckgesetzt, damit
      // der Wert "Verkehr seit letztem Lebenszeichen" zeigt, nicht einen endlos wachsenden
      // Gesamtzaehler. 0 heisst "Kasse lebt, aber gerade keine echten Buchungen unterwegs" -
      // KC System Check kann daraus grau (kein Verkehr) von gruen (echter Verkehr) trennen,
      // ohne dass ein reines Lebenszeichen faelschlich als Datenfluss angezeigt wird.
      const gesendet = traffic.tx;
      traffic.tx = 0;
      const registerId = global.KCSyncConnection?.config?.registerId || null;
      sendLiveEvent('heartbeat', {
        ...
        // 10.09.2026 (KC System Check: "brauche eindeutig erkennbar, ob Kasse 1 oder Kasse 2 -
        // idealerweise instanceId/sourceId wie kasse-01/kasse-02"): registerId gab es schon
        // ("KASSE-01"), aber in einer anderen Schreibweise als von KC Check erwartet. Eigenes
        // Feld in genau dem gewuenschten Format ergaenzt, registerId bleibt unveraendert fuer
        // alles, was schon darauf aufbaut (Manager-Zuordnung, Kassetten-Splitting usw.).
        instanceId: registerId ? registerId.toLowerCase().replace(/_/g, '-') : null,
  ...
  // Zaehler fuer tatsaechlich beim Companion angekommene Uebertragungen (siehe sendeEinmal) -
  // wird bei jedem Heartbeat gelesen und zurueckgesetzt, siehe sendeLebenszeichen oben.
  const traffic = { tx: 0 };
  ...
  async function sendeEinmal(type, payload) {
    ...
    try {
      ...
      const erfolg = ergebnis?.recorded === true;
      // 10.09.2026 (KC System Check: "Heartbeat bleibt reines Lebenszeichen, echter Datenfluss
      // muss getrennt davon gemessen werden, gekoppelt an tatsaechlich erfolgreiche
      // Uebertragungen"): hier - und NUR hier, an der einen Stelle, an der eine Buchung
      // NACHWEISLICH beim Companion angekommen ist (ergebnis.recorded===true) - zaehlt der
      // Datenverkehr. sendeEinmal() wird sowohl vom ersten Versuch (recordReliableEvent) als
      // auch von jedem Nachreich-Versuch (reicheNach) durchlaufen, deckt also jeden echten
      // Transfer ab, unabhaengig davon, ob er sofort oder erst spaeter durchkam. Bewusst
      // NICHTS an Verkaufslogik, Bons, Bargeld, Warteschlange oder Nachreichen veraendert -
      // reine Telemetrie obendrauf.
      if (erfolg) traffic.tx += 1;
      return erfolg;
