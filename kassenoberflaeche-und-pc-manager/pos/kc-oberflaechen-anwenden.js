...
(function (global) {
  ...
  function sammelZeichnen() {
    ...
    $('.kc-sammel-artikel', sammelEbene).innerHTML = gruppen.map((g) => {
      ...
      const anz = Object.keys(sammelWahl).filter((k) => { const p = kPROD().find((x) => x.id === k.split('|')[0]); return p && p.category === g; }).reduce((n, k) => n + sammelWahl[k], 0);
      ...
      const karte = (p, o) => { const k = sammelSchluessel(p, o); const n = sammelWahl[k] || 0;
        const farbe = p.color || '#315d8d'; const textfarbe = sammelAnsicht === 'farbe' ? sammelKontrast(farbe) : '';
        const bild = sammelAnsicht === 'bild' && !o && p.image ? `<img src="${p.image}" alt="">` : '';
        return `<div role="button" tabindex="0" data-sammel-key="${k}" class="kc-sammel-karte ansicht-${sammelAnsicht}${n ? ' gewaehlt' : ''}${o ? ' variante' : ''}" style="--tile-color:${farbe};${textfarbe ? `--tile-text:${textfarbe};` : ''}">${bild}<span class="kc-sammel-name">${o ? o.label || o.name : p.name}</span><span class="kc-sammel-preis">${geld(o ? o.price : p.price)}</span>${n ? `<b class="kc-sammel-anzahl">${n}×</b><button type="button" class="kc-sammel-minus" data-sammel-minus="${k}" aria-label="Einen weniger">−</button>` : ''}</div>`;
      };
