...
(function (global) {
  ...
  // 10.09.2026 (Betreiber: "PERSONAL soll bei Auszahlung ALS SPENDE anzeigen") - staffBtn stand
  // hier bisher fest auf "PERSONAL" und wurde bei jedem Durchlauf stur wieder darauf
  // zurueckgesetzt (derselbe Fehlertyp, der weiter unten fuer den BAR-Knopf schon einmal
  // beschrieben und geloest wurde). app.js verwaltet die Beschriftung von staffBtn jetzt SELBST
  // (PERSONAL <-> ALS SPENDE, je nach Betrag) - deshalb bewusst aus dieser starren Liste
  // entfernt, statt sie staendig zu ueberschreiben.
  const original = {};
  ...
  function ledsAktualisieren() {
    ...
    // 10.09.2026 (Betreiber: "die 2 LEDs neben dem Programm-Knopf sind Altlasten, koennen die
    // weg?"): die alten, rein dekorativen .supabase-led-group-Punkte sind jetzt weg (nie von
    // irgendeiner Datei aktualisiert, ueberflüssig neben dieser echten Cloud-LED). Aber ihr
    // title-Text war heimlich die einzige Datenquelle FUER GENAU DIESE LED hier - beim
    // Entfernen zuerst uebersehen, dann gefunden und sauber aufgeloest: der Zustand ("noch
    // nicht eingerichtet") stand nirgends dynamisch fest, war schon immer ein reiner
    // Festwert - jetzt direkt hier als Festwert, keine Abhaengigkeit von einem HTML-Element
    // mehr, das nur zufaellig noch existierte.
    const supTitel = 'Online-Sicherung: noch nicht eingerichtet';
