  ...
  async function pruefeAufBefehl() {
    try {
      ...
      if (daten.befehl === 'reload_stammdaten') {
        ...
        location.reload();
      } else if (daten.befehl === 'kassen_pin_zuruecksetzen') {
        // 08.09.2026 Notfallzugang: PIN des Geheimwegs (versteckte Kassenfunktionen) vom
        // Manager aus zuruecksetzen, wenn sie am Stand verloren ging. Nur die Geheimweg-PIN,
        // nicht die Master-PIN.
        try { localStorage.removeItem('kc.kassenfunktionen.pin.v1'); } catch (e) { /* ignorieren */ }
        if (window.KCAufbau && window.KCAufbau.kassenPinZuruecksetzen) window.KCAufbau.kassenPinZuruecksetzen('PC-Manager');
