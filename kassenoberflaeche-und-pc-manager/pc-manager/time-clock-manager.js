(function (global) {
  'use strict';
  const VERSION = '0.4.0', CONFIG_KEY = 'kcm_time_clock_config_v1', PEOPLE_KEY = 'kcm_time_clock_people_v1', EVENTS_KEY = 'kcm_time_clock_events_v1';
  ...
  function dienstplanZeilen(alleEreignisse = [], alleLeute = []) {
    ...
    const bauen = (personId, status, kommen, gehen) => {
      ...
      const zeile = {
        ...
        status,
        sourceEventId: [
          kommen?.id || 'kein-kommen', kommen?.effectiveAt || '',
          gehen?.id || 'kein-gehen', gehen?.effectiveAt || '',
        ].join('|'),
      };
      zeilen.push(zeile);
  ...
  const DP_SYNC_KEY = 'kcm_time_clock_dp_sync_v1';
  const datenFingerabdruck = (rows) => {
    const text = JSON.stringify(rows.map((r) => [r.sourceEventId,r.memberNo,r.personId,r.date,r.start,r.end,r.breakMinutes,r.status]));
    let hash = 2166136261; for (let i=0;i<text.length;i++) { hash ^= text.charCodeAt(i); hash = Math.imul(hash,16777619); }
    return config.eventId + ':' + (hash >>> 0).toString(16);
  };
  function zeigeDpSyncStatus(text, art='') {
    const feld=document.getElementById('tcDpSyncStatus'); if(!feld)return;
    feld.textContent=text; feld.className='tc-status'+(art==='warn'?' warn':art==='gut'?' ok':'');
  }
  async function sendeIstzeitenAnDp2(vomBenutzer=false) {
    const api=global.KCSupabase;
    if(!api?.istAngemeldet?.()) {
      if(vomBenutzer) tell('Bitte zuerst oben über die Supabase-Anzeige mit dem Manager-Adminkonto anmelden.','warn');
      zeigeDpSyncStatus('Automatische Übertragung wartet auf die Supabase-Anmeldung.','warn'); return null;
    }
    const alle=dienstplanZeilen(events,people).zeilen, fingerprint=datenFingerabdruck(alle);
    if(!vomBenutzer && localStorage.getItem(DP_SYNC_KEY)===fingerprint)return {unchanged:true,received:alle.length};
    if(!alle.length){localStorage.setItem(DP_SYNC_KEY,fingerprint);zeigeDpSyncStatus('Keine Istzeiten zum Senden.','gut');return {received:0,inserted:0,skipped:0};}
    zeigeDpSyncStatus(alle.length+' Istzeit(en) werden geschützt an DP2 übertragen …');
    let inserted=0,skipped=0;
    for(let i=0;i<alle.length;i+=500){
      const out=await api.rufeFunktionAuf('kc_dp_timeclock_publish',{p_org_id:'KC_WERNE',p_event_id:config.eventId,p_rows:alle.slice(i,i+500)});
      const row=Array.isArray(out)?out[0]:out; inserted+=Number(row?.inserted||0); skipped+=Number(row?.skipped||0);
    }
    localStorage.setItem(DP_SYNC_KEY,fingerprint);
    const text=inserted?inserted+' neue Istzeit(en) an DP2 übergeben; '+skipped+' bereits vorhanden.':alle.length+' Istzeit(en) waren in DP2 bereits vorhanden.';
    zeigeDpSyncStatus(text,'gut'); if(vomBenutzer)tell(text);
    return {received:alle.length,inserted,skipped};
  }
    ...
    section.innerHTML = `
      ...
      <div class="tc-grid">
        ...
        <article class="panel"><h3>Dienstplan-Schnittstelle</h3><p>Exportiert ALLE Buchungen – auch unvollständige. Wer das Einbuchen oder den Feierabend vergessen hat,
          ...
          <div class="tc-actions"><button id="tcSendRoster" class="primary">Ist-Zeiten an DP2 senden</button><button id="tcExportRoster">Datei für DP2 speichern</button></div>
          <p id="tcDpSyncStatus" class="tc-status">Noch nicht an DP2 gesendet.</p>
        </article>
  ...
  function bind() {
    ...
    starteAbrufTakt();
    document.getElementById('tcSendRoster').onclick = async () => {
      try { await sendeIstzeitenAnDp2(true); } catch(error) { zeigeDpSyncStatus('Übertragung an DP2 fehlgeschlagen: '+error.message,'warn'); tell(error.message,'warn'); }
    };
  ...
  async function holeGemeldeteZeiten(vomBenutzer) {
    ...
    try {
      ...
      await sendeIstzeitenAnDp2(false).catch((fehler) => zeigeDpSyncStatus('Übertragung an DP2 fehlgeschlagen: '+fehler.message,'warn'));
  ...
  global.KCTimeClockManager = {version:VERSION, refresh:render, dienstplanZeilen, sendeIstzeitenAnDp2};
})(window);
