...
(function (global) {
  ...
  function haengeAnLeds() {
    document.querySelectorAll('.kc-live-led-group').forEach((g) => {
      ...
      // 10.09.2026 (Betreiber: "es kommt ganz kurz ein kleines Fenster, das wird sofort von
      // dem grossen Testfenster ueberdeckt"): ECHTER FUND - diese allgemeine Bindung ("jede
      // LED oeffnet die Diagnose") erwischte versehentlich AUCH die Supabase-LED, die aber
      // schon ihr eigenes, spezifischeres Fenster mit Anmeldung hat (kc-manager-supabase-
      // status.js). Beide Klick-Handler haengen am selben Element und feuerten beide - das
      // spaeter registrierte (dieses hier) landete sichtbar obenauf und verdeckte das
      // Anmeldefenster, bevor man etwas eintippen konnte. Die Supabase-LED-Gruppe deshalb
      // hier bewusst ausgenommen - sie bleibt bei ihrem eigenen Fenster.
      if (g.id === 'kcSupabaseLedGroup') return;
      g.dataset.kcdiag = '1';
