...
(function (global) {
  ...
  function zeichne(liste) {
    ...
    ziel.innerHTML = `<table class="kcbs-tabelle"><thead><tr>
        <th>Zeitpunkt</th><th>Kasse</th><th>Erwartet</th><th>Barverkäufe</th><th>Konto</th><th>Umsatz</th>
      ...
      ${liste.map((a) => `<tr>
        ...
        <td class="kcbs-zahl" title="${esc((a.accountBreakdown || []).map((k) => `${k.name}: ${geld(k.amount)}`).join(' · ') || 'keine Kontobuchungen')}">${geld(a.accountSales || 0)}${(a.accountBreakdown || []).length ? `<br><small>${esc((a.accountBreakdown || []).map((k) => `${k.name} ${geld(k.amount)}`).join(' · '))}</small>` : ''}</td>
        <td class="kcbs-zahl"><b>${geld(a.totalSales || 0)}</b></td>
