...
(function (global) {
  ...
  async function render() {
        ...
        ${kassenIds.map((id) => `
          <div class="kc-notfall-zeile" data-register="${id}" style="padding:10px 0;border-top:1px solid #fee2e2;">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
              ...
              <button type="button" class="kc-admin-kassenpin-btn" data-register="${id}" title="Setzt nur die PIN des Geheimwegs (versteckte Kassenfunktionen) zurück - nicht die Master-PIN" style="background:#1d4ed8;color:#fff;border:none;">🔑 Kassen-PIN zurücksetzen</button>
    ...
    /* 08.09.2026 Notfallzugang: Geheimweg-PIN der Kasse vom Manager aus zuruecksetzen */
    ziel.querySelectorAll('.kc-admin-kassenpin-btn').forEach((btn) => {
      btn.onclick = () => sendeBefehl(btn.dataset.register, 'kassen_pin_zuruecksetzen', btn.closest('.kc-notfall-zeile').querySelector('.kc-admin-notfall-ergebnis'));
    });
