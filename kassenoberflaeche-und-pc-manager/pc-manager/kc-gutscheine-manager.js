    ...
    el('gsmListe').innerHTML = liste.length ? `
      ...
      ${liste.map((g) => {
        ...
        return `<tr class="gsm-${z}">
          <td class="gsm-code">${esc(g.code)}${g.signaturGeprueft === 0
            ? ' <span class="gsm-warnung" title="Der Prüfcode dieses Gutscheins passt nicht zu unserem Geheimnis. Bitte vor dem Einlösen nachfragen.">\u26a0</span>'
            : ''}</td>
