...
// 09.09.2026 (Betreiber): "Schuss Rum"/"Schuss Amaretto" zusaetzlich als eigene, einzeln
// verkaufbare Artikel - bisher gab es sie nur als Zusatzwahl (optionGroup:"shot") zu
// Gluehwein/Rotem Feger, das bleibt unveraendert bestehen. sortOrder passend zur Kasse
// (pos/app.js) gesetzt, damit beide dort ganz am Ende der Gruppe stehen.
{id:"schussrum",name:"Schuss Rum",category:"Getränke",price:1.00,receiptText:"Schuss Rum",color:"#5a3a1e",active:true,quantity:1,unit:"Schuss",taxRate:19,sortOrder:9000,image:"assets/rum_flasche_11-09.webp"},
{id:"schussamaretto",name:"Schuss Amaretto",category:"Getränke",price:1.00,receiptText:"Schuss Amaretto",color:"#7a4a1e",active:true,quantity:1,unit:"Schuss",taxRate:19,sortOrder:9001,image:"assets/amaretto_flasche_11-09.webp"},
{id:"grot",name:"Glühwein rot",category:"Getränke",price:3.50,receiptText:"Glühwein rot",optionGroup:"shot",color:"#8b1e24",active:true,quantity:1,unit:"Becher",taxRate:19,image:"assets/gluehwein_rot_11-09.webp",info:{allergens:"Enthält Sulfite"}},
// 10.09.2026 (Claude, beim Codex-Abgleich gefunden): "Eierlikörpunsch" und "Roter Feger" fehlten
// im Manager-Katalog komplett - nicht nur die Reihenfolge, der ganze Artikel. Bestand schon vor
// der heutigen Codex-Bearbeitung, hat nichts mit dessen Aenderungen zu tun. Werte 1:1 aus dem
// Kassen-Katalog (pos/app.js) uebernommen, inkl. sortOrder.
{id:"eier",name:"Eierlikörpunsch",category:"Getränke",price:4.50,sortOrder:1,receiptText:"Eierlikörpunsch",color:"#c9942f",active:true,quantity:1,unit:"Becher",taxRate:19},
// 11.09.2026 (Claude, beim Bilder-Einbau gefunden): "Apfelpunsch" fehlte im Manager-Katalog
// komplett - dieselbe Luecke wie bei eier/roterfeger/gruenkohlmett/sauerkrautmett am 10.09.
// (siehe Kommentar unten), nur bisher nicht aufgefallen. Werte 1:1 aus dem Kassen-Katalog
// (pos/app.js) uebernommen.
{id:"apfel",name:"Apfelpunsch",category:"Getränke",price:2.50,receiptText:"Apfelpunsch",color:"#c9942f",active:true,quantity:1,unit:"Becher",taxRate:19,image:"assets/apfelpunsch_apfel_oben_11-09.webp",depositRule:"automatic",depositComponents:[{name:"Glaspfand",price:2}]},
{id:"roterfeger",name:"Roter Feger",category:"Getränke",price:2.50,sortOrder:4,receiptText:"Roter Feger",optionGroup:"shot",color:"#b3202a",active:true,quantity:1,unit:"Becher",taxRate:19,image:"assets/roter_feger_kirschen_11-09.webp",info:{shortDescription:"Alkoholfreier roter Kirschpunsch",important:"Alkoholfrei - auch für Kinder geeignet."}},
{id:"gweiss",name:"Glühwein weiß",category:"Getränke",price:3.50,sortOrder:3,receiptText:"Glühwein weiß",optionGroup:"shot",color:"#a36a20",active:true,quantity:1,unit:"Becher",taxRate:19},
{id:"feuer",name:"Feuerzangenbowle",category:"Getränke",price:5,receiptText:"Feuerzangenbowle",depositRule:"automatic",depositComponents:[{name:"Glaspfand",price:2},{name:"Feuerzangenpfand",price:2}],color:"#a43f18",active:true,quantity:1,unit:"Glas",taxRate:19,image:"assets/feuerzangenbowle_gewuerze_11-09.webp"},
...
{id:"sauerkraut",name:"Sauerkrauteintopf",category:"Speisen",price:5.5,sortOrder:4,halfAllowed:true,halfPrice:3.00,/* 08.09.2026 (Betreiber): halbe Portion freigegeben */receiptText:"Sauerkrauteintopf",color:"#77643d",active:true,quantity:1,unit:"Portion",taxRate:7},
// 10.09.2026 (Claude, beim Codex-Abgleich gefunden): dieselbe Luecke wie bei den Getraenken -
// "Grünkohl + Mettwurst" und "Sauerkrauteintopf + Mettwurst" fehlten im Manager-Katalog.
{id:"gruenkohlmett",name:"Grünkohl + Mettwurst",category:"Speisen",price:7.00,sortOrder:1,receiptText:"Grünkohl + Mettwurst",color:"#365d36",active:true,quantity:1,unit:"Portion",taxRate:7,info:{allergens:"Kann Senf, Sellerie und Spuren von Gluten enthalten",important:"Mettwurst separat im Bestand geführt."}},
{id:"sauerkrautmett",name:"Sauerkrauteintopf + Mettwurst",category:"Speisen",price:7.00,halfAllowed:true,halfPrice:4.00,sortOrder:3,receiptText:"Sauerkraut + Mettwurst",color:"#785332",active:true,quantity:1,unit:"Portion",taxRate:7,info:{allergens:"Kann Sellerie, Senf und Spuren von Gluten enthalten",important:"Mettwurst separat im Bestand geführt."}},
{id:"hering",name:"Kartoffel mit Hering",category:"Speisen",price:4.5,sortOrder:5,image:"assets/hering_kartoffeln_auth.webp",receiptText:"Kartoffel m. Hering",color:"#42657d",active:true,quantity:1,unit:"Portion",taxRate:7},


{id:"knirpsecreme",name:"Kartoffel mit Kartoffelcreme",category:"Speisen",price:3.5,sortOrder:6,image:"assets/kartoffelcreme_auth.webp",receiptText:"Kartoffel m. Kartoffelcreme",color:"#a66e24",active:true,quantity:1,unit:"Portion",taxRate:7},
...
// 09.09.2026: derselbe Nachtrag wie an der Kasse - die Artikelnummern-Tabelle wurde nie
// tatsaechlich in die Artikel eingetragen. Jetzt beim Start nachgeholt (siehe pos/app.js).
try{window.KCArtikelnummern?.eintragen(articles)}catch(e){}
// 10.09.2026 (Betreiber, nach dem Codex-Abgleich): dieselbe Nachtragung wie an der Kasse
// (pos/app.js) - falls hier noch eine aeltere, vor der Reihenfolgen-Aenderung gespeicherte
// Artikelliste (kcm_articles) im Speicher dieses PCs liegt, fehlt dort "sortOrder" komplett.
// Traegt fehlende Werte fuer die bekannten Artikel nach, unabhaengig vom Ursprung der Liste.
...
articles.forEach(a=>{if(a&&!(a.sortOrder>0)&&SORTORDER_BEKANNT[a.id])a.sortOrder=SORTORDER_BEKANNT[a.id]});
...
// 10.09.2026: die Zusammenfuehrung eben kann Artikel aus der POS-Registry nachladen, die noch
// kein sortOrder tragen (kam beim Pruefen tatsaechlich vor: "gruenkohl" erschien erst HIER,
// nach der ersten Nachtragung weiter oben - die griff dadurch zu frueh). Deshalb hier ein
// zweites Mal, NACH der Zusammenfuehrung, damit wirklich jeder bekannte Artikel erfasst wird.
articles.forEach(a=>{if(a&&!(a.sortOrder>0)&&SORTORDER_BEKANNT[a.id])a.sortOrder=SORTORDER_BEKANNT[a.id]});
...
// 11.09.2026 (Betreiber: "die neuesten Sachen (Fotos) in PC Manager einbauen"): die obige
// Migration setzt nur ein Bild, wenn NOCH KEINS gesetzt ist - fuer Artikel, die schon ein
// (jetzt veraltetes) Bild hatten, greift das nicht. Diese Migration ERSETZT deshalb gezielt
// genau die bekannten ALTEN Pfade durch die neuen, von der Kasse (pos/app.js) heute Nacht
// uebernommenen Fotos - beruehrt NIE ein Bild, das der Verein selbst irgendwann per Hand
// gesetzt hat (die Bedingung prueft exakt den alten Auslieferungspfad, nicht nur "irgendein
// Bild da"). schussrum/schussamaretto hatten bisher gar kein Bild - fuer die reicht die
// Migration oben (jetzt mit aktuellem Pfad), hier nur zur Sicherheit nochmal explizit gesetzt.
{const ersetzung={
  grot:{alt:"assets/gluehwein_rot_auth.webp",neu:"assets/gluehwein_rot_11-09.webp"},
  apfel:{alt:"assets/apfelpunsch_auth.webp",neu:"assets/apfelpunsch_apfel_oben_11-09.webp"},
  feuer:{alt:"assets/feuerzangenbowle_auth.webp",neu:"assets/feuerzangenbowle_gewuerze_11-09.webp"},
  roterfeger:{alt:"assets/roter_feger.webp",neu:"assets/roter_feger_kirschen_11-09.webp"},
};
let geaendertBilder=false;
articles.forEach(article=>{
  const e=ersetzung[article.id];
  if(e&&article.image===e.alt){article.image=e.neu;geaendertBilder=true}
  if((article.id==="schussrum")&&!article.image){article.image="assets/rum_flasche_11-09.webp";geaendertBilder=true}
  if((article.id==="schussamaretto")&&!article.image){article.image="assets/amaretto_flasche_11-09.webp";geaendertBilder=true}
});
if(geaendertBilder)localStorage.setItem('kcm_articles',JSON.stringify(articles));
}
// 11.09.2026: "Apfelpunsch" fehlte im Manager-Katalog komplett (dieselbe Art Luecke wie bei
// eier/roterfeger/gruenkohlmett/sauerkrautmett zuvor) - fuer laufende Installationen (deren
// kcm_articles schon gefuellt ist, DEFAULT_ARTICLES also nicht mehr greift) hier nachgetragen.
if(!articles.some(a=>a.id==="apfel")){
  articles.push({id:"apfel",name:"Apfelpunsch",category:"Getränke",price:2.50,receiptText:"Apfelpunsch",color:"#c9942f",active:true,quantity:1,unit:"Becher",taxRate:19,image:"assets/apfelpunsch_apfel_oben_11-09.webp",depositRule:"automatic",depositComponents:[{name:"Glaspfand",price:2}]});
...
// 11.09.2026 (Betreiber: "Spende im Dashboard neben Trinkgeld separat anzeigen"): genau
// dasselbe Muster wie managerTips - eigene, von Trinkgeld GETRENNTE Speicherung (die Kasse
// trennt Spende/Trinkgeld ja bereits streng, siehe kc_donation_records dort).
let managerDonations=JSON.parse(localStorage.getItem("kcm_donations")||"[]");
...
let registers=JSON.parse(localStorage.getItem("kcm_registers")||"null")||[{id:"KASSE-01",name:"Kasse 1",operator:"Team",active:true},{id:"KASSE-02",name:"Kasse 2",operator:"",active:true},{id:"KASSE-03",name:"Kasse 3",operator:"",active:true}];
...
localStorage.setItem("kcm_donations",JSON.stringify(managerDonations));
...
document.querySelectorAll("#articleToolbar button").forEach(b=>b.onclick=()=>{const c=b.dataset.cmd;if(c==="new"){aIndex=-1;["aId","aName","aShort","aReceipt","aBarcode","aImage"].forEach(id=>el(id).value="");return}if(c==="save"){const a=readArticle();if(!a.id||!a.name)return alert("Artikelnummer und Bezeichnung erforderlich");if(a.halfAllowed&&a.halfPrice<=0)return alert("„½ Portion erlaubt“ ist angehakt, aber es fehlt der ½-Preis. Bitte einen Preis eintragen (oder den Haken wieder entfernen), sonst bietet die Kasse die halbe Portion nicht an.");if(aIndex>=0)articles[aIndex]=a;else articles.push(a);queueSync("article","upsert",a);saveAll();renderArticles()}if(c==="delete"&&aIndex>=0){articles.splice(aIndex,1);aIndex=0;saveAll();renderArticles()}if(c==="image")el("imageFile").click();if(c==="export")download("Artikel.json",JSON.stringify(articles,null,2));if(c==="print")window.print()});
...
// 09.09.2026 (Betreiber: "Preis wurde vergessen, halbe Portion kam nicht an der Kasse an") -
// sichtbarer Hinweis, sobald der Haken gesetzt ist, aber kein Preis > 0 dabeisteht. Reagiert
// live, nicht erst beim Speichern.
function halbePortionWarnungPflegen(){
  const an=el("aHalfAllowed").checked,preis=Number(el("aHalfPrice").value||0);
  el("aHalfWarnung").hidden=!(an&&preis<=0);
}
el("aHalfAllowed").addEventListener("change",halbePortionWarnungPflegen);
el("aHalfPrice").addEventListener("input",halbePortionWarnungPflegen);
...
// 09.09.2026 (Betreiber): "Live-Ansicht von zuhause" - die Master-PIN schuetzte bisher nur
// einzelne Aktionen (Export, Geld-QR, Admin), nicht das blosse OEFFNEN/ANSEHEN des Dashboards.
// Sobald der Manager auch von ausserhalb erreichbar sein soll, muss die PIN schon beim Start
// kommen, bevor irgendetwas zu sehen ist - derselbe, bereits vorhandene und sicher gespeicherte
// PIN-Mechanismus, nur fruehzeitiger ausgeloest. Laeuft ohne PIN noch nie eingerichtet wurde,
// fuehrt das genau in denselben "Manager erstmals absichern"-Dialog wie bisher schon bei der
// ersten geschuetzten Aktion.
requireAuth(()=>{});
...
el("importSales").onclick=()=>requireAuth(async()=>{
  ...
  try{
    for(let index=0;index<files.length;index++){
      ...
      if(Array.isArray(payload.donations))payload.donations.forEach(d=>{if(!managerDonations.some(x=>x.id===d.id))managerDonations.push({...d,registerId:payload.registerId,registerName:payload.registerName})});
...
// 10.09.2026 (Betreiber: "wenn die Daten schon da sind, können sie ja auch direkt angezeigt
// werden, am besten auch ein Button um die Daten manuell zu holen. Aber nicht schon wieder
// eine neue Seite einbauen"): Verkäufe kommen über den zuverlässigen Sync-Kanal längst sicher
// beim Manager-Companion-Dienst an (siehe manager-companion/index.js, neuer Endpunkt
// /verkaeufe-abholen) - bisher wurden sie dort aber nie abgeholt, nur der manuelle Datei-
// Import fütterte das Dashboard. Bewusst KEINE neue Seite/Ansicht - nur ein Knopf im
// bestehenden Dashboard-Kopf plus ein einmaliger, automatischer Abruf beim Start, damit
// bereits angekommene Verkäufe ohne Zutun sichtbar werden.
const KC_LETZTER_VERKAUFSABRUF_KEY="kcm_letzter_verkaufsabruf_v1";
async function holeVerkaeufeVomManagerCompanion(manuell=false){
  const knopf=el("fetchLiveSales"),ergebnisFeld=el("fetchLiveSalesResult");
  if(knopf)knopf.disabled=true;
  try{
    const seit=localStorage.getItem(KC_LETZTER_VERKAUFSABRUF_KEY)||"";
    const antwort=await fetch(`http://127.0.0.1:47392/verkaeufe-abholen${seit?`?seit=${encodeURIComponent(seit)}`:""}`);
    if(!antwort.ok)throw new Error(`HTTP ${antwort.status}`);
    const daten=await antwort.json();
    const eingegangen=Array.isArray(daten.verkaeufe)?daten.verkaeufe:[];
    if(eingegangen.length){
      const merged=window.KCSalesImportCore?.merge?.(sales,eingegangen,{});
      if(merged){sales=merged.transactions;saveAll();renderDashboard();renderReport?.();renderCashMovementsOverview?.();}
    }
    localStorage.setItem(KC_LETZTER_VERKAUFSABRUF_KEY,daten.abgefragtUm||new Date().toISOString());
    const text=eingegangen.length?`${eingegangen.length} Verkäufe vom Manager-Dienst übernommen.`:"Keine neuen Verkäufe seit dem letzten Abruf.";
    if(ergebnisFeld)ergebnisFeld.textContent=text;
    if(manuell)window.KCManagerMessages?.success?.(text);
  }catch(err){
    // Der Manager-Companion-Dienst laeuft nur auf demselben Rechner (Loopback) - laeuft er
    // nicht oder ist die Kasse noch nie verbunden gewesen, ist das der Normalfall am Vormittag,
    // kein Fehler, der den Bediener beunruhigen sollte.
...
el("fetchLiveSales")?.addEventListener("click",()=>holeVerkaeufeVomManagerCompanion(true));
// Einmaliger, stiller Abruf beim Start - Verkaeufe, die laengst sicher angekommen sind,
// sollen ohne eigenes Zutun im Dashboard auftauchen.
holeVerkaeufeVomManagerCompanion(false);
// 10.09.2026 (Betreiber: "ja automatisch"): nicht nur einmal beim Start, sondern auch
// regelmaessig im Hintergrund - waehrend der Manager den ganzen Markttag ueber offen bleibt,
// sollen neu ankommende Verkaeufe von selbst nachkommen, ohne dass jemand den Knopf druecken
// muss. 45 Sekunden: haeufig genug, um "fast live" zu wirken, ohne den lokalen Dienst bei
// mehreren offenen Manager-Fenstern unnoetig zu belasten. Bewusst manuell=false - ein
// automatischer Hintergrund-Abruf soll nicht bei jedem Leerlauf eine Meldung einblenden.
setInterval(()=>holeVerkaeufeVomManagerCompanion(false),45000);
...
function renderDashboard(){
  ...
  // 11.09.2026 (Betreiber: "Spende im Dashboard neben Trinkgeld separat anzeigen"): eigene
  // Kachel, eigene Summe - bewusst NICHT mit Trinkgeld addiert oder vermischt, genau wie die
  // Kasse Spende und Trinkgeld schon streng getrennt bucht.
  el("kpiDonations")&&(el("kpiDonations").textContent=money(managerDonations.filter(d=>!d.training).reduce((s,d)=>s+Number(d.amount||0),0)));
...
document.querySelectorAll("[data-denom],[data-cash-roll-value]").forEach(n=>n.oninput=()=>{renderCashTotal();clearManagerCashOutput();cashZaehlVergleichZeichnen()});
...
// 10.09.2026 (Betreiber: "beide sollen zu 100% gleich sein") - Abendzaehlung, Zeile fuer Zeile
// nach demselben Vorbild wie die eigenstaendige Money-Butler-App (dort: setDatePurpose,
// pflegeKassettenAuswahl, zaehlBereicheZeigen, sollUebernehmen, zaehlVergleichZeichnen).
let cashSollAbschluss=null;
function cashSetDatePurpose(){
  const zaehlung=el("cashType").value==="count";
  const label=el("cashEffectiveDateLabel");
  if(label)label.childNodes[0].textContent=zaehlung?"Zähltag (Pflichtfeld)":"Gültig für (Pflichtfeld)";
  el("cashEffectiveDate").min=zaehlung?localBusinessDate(new Date(Date.now()-9*86400000)):localBusinessDate();
}
function cashPflegeKassettenAuswahl(){
  const option=[...el("cashRegister").options].find(o=>o.value===KASSETTE_ZIEL);
  if(!option)return;
  const gesperrt=el("cashType").value==="count";
  ...
  if(gesperrt&&istKassettenZiel())el("cashRegister").value="KASSE-01";
}
function cashZaehlBereicheZeigen(){
  const zaehlung=el("cashType").value==="count";
  el("cashSollSection").hidden=!zaehlung;el("cashBelegeSection").hidden=!zaehlung;el("cashZaehlKontrolle").hidden=!zaehlung;
  if(!zaehlung){cashSollAbschluss=null;el("cashSollCode").value="";el("cashSollAnzeige").textContent="Noch kein Tagesabschluss eingelesen - die Zählung geht trotzdem, nur ohne sofortigen Vergleich.";}
  cashZaehlVergleichZeichnen();
}
el("cashSollUebernehmen")?.addEventListener("click",()=>{
  const text=el("cashSollCode").value.trim();
  if(!text)return alert("Bitte zuerst den Tagesabschluss-Code der Kasse einfügen oder scannen.");
  try{
    if(!text.startsWith("KCLOSE1:"))throw new Error("Das ist kein Tagesabschluss-Code (muss mit KCLOSE1: beginnen).");
    const payload=JSON.parse(decodeURIComponent(escape(atob(text.slice(8)))));
    const kopie=JSON.parse(JSON.stringify(payload)),soll=kopie.checksum;delete kopie.checksum;
    if(!soll||checksum(JSON.stringify(kopie))!==soll)throw new Error("Prüfsumme falsch - Code beschädigt oder verändert.");
    cashSollAbschluss=payload;
    el("cashSollAnzeige").textContent=`Soll laut Kasse (${payload.registerName||payload.registerId}, ${String(payload.createdAt||"").slice(0,10)}): ${money(payload.expectedCash)}${Number.isFinite(payload.receiptExpected)?` · ${payload.receiptExpected} Beleg(e)`:""}`;
    if(payload.registerId)el("cashRegister").value=payload.registerId;
  }catch(err){cashSollAbschluss=null;el("cashSollAnzeige").textContent="Fehler: "+err.message}
  cashZaehlVergleichZeichnen();
});
el("cashBelegeAnzahl")?.addEventListener("input",cashZaehlVergleichZeichnen);
function cashZaehlVergleichZeichnen(){
  if(el("cashType").value!=="count")return;
  const gezaehlt=cashData().total;
  if(cashSollAbschluss){
    const diffGeld=+(gezaehlt-Number(cashSollAbschluss.expectedCash||0)).toFixed(2);
    el("cashKontrolleGeld").textContent=diffGeld===0?"stimmt genau":diffGeld>0?"+"+money(diffGeld)+" zu viel":money(Math.abs(diffGeld))+" fehlen";
    el("cashKontrolleGeld").className=diffGeld===0?"ok":"warn";
  }else{el("cashKontrolleGeld").textContent=money(gezaehlt);el("cashKontrolleGeld").className=""}
  const anzahl=el("cashBelegeAnzahl").value;
  if(anzahl!==""&&cashSollAbschluss&&Number.isFinite(cashSollAbschluss.receiptExpected)){
    const diffBelege=Number(anzahl)-cashSollAbschluss.receiptExpected;
    el("cashKontrolleBelege").textContent=diffBelege===0?"stimmt genau":diffBelege>0?"+"+diffBelege+" zu viel":Math.abs(diffBelege)+" fehlen";
    el("cashKontrolleBelege").className=diffBelege===0?"ok":"warn";
    el("cashBelegeAnzeige").textContent=`Soll laut Kasse: ${cashSollAbschluss.receiptExpected} Beleg(e)`;
  }else{
    el("cashKontrolleBelege").textContent=anzahl===""?"–":anzahl;el("cashKontrolleBelege").className="";
    el("cashBelegeAnzeige").textContent="";
  }
}
el("cashType")?.addEventListener("change",()=>{cashSetDatePurpose();cashPflegeKassettenAuswahl();zeichneKassettenAufteilung();cashZaehlBereicheZeigen();clearManagerCashOutput()});
cashSetDatePurpose();
el("cashReset")?.addEventListener("click",()=>{
  document.querySelectorAll("[data-denom],[data-cash-roll-value]").forEach(n=>n.value=0);
  el("cashEffectiveDate").value="";el("cashNote").value="";
  renderCashTotal();zeichneKassettenAufteilung();clearManagerCashOutput();cashZaehlVergleichZeichnen();
});

// 10.09.2026 (Betreiber: "beide sollen zu 100% gleich sein") - Uebergabeprotokoll, nach
// demselben Vorbild wie die eigenstaendige App portiert. Nutzt DASSELBE gemeinsame Modul
// shared/kc-uebergabeprotokoll.js (bereits im Manager geladen, fuer das Lesen fremder
// Protokolle) - keine zweite, eigene Kodierung, sonst gilt ein gueltiger Beleg irgendwann als
// falsch.
const cashProtokollAktiv=()=>!!window.KC_RUNTIME_FLAGS?.uebergabeProtokoll;
function cashStueckelungsZeilen(daten){
  const zeilen=[];
...
function cashZeichneProtokoll(payload,gesamt,teile){
  const blatt=el("cashProtokollBlatt"),knopf=el("cashProtokoll");
  if(!blatt||!knopf)return;
  if(!cashProtokollAktiv()){blatt.hidden=true;knopf.hidden=true;return}
  knopf.hidden=false;
  const kassette=payload.scope==="split";
  const anteile=kassette&&teile?payload.registerIds.map(kasse=>`
      <h4>${kasse}</h4>
      <table class="protokoll-tabelle">${cashStueckelungsZeilen(teile[kasse])}
        <tr class="protokoll-summe"><td>Summe</td><td></td><td>${money(teile[kasse].total)}</td></tr></table>`).join(""):"";
  const beleg=payload.total>0&&window.KCUebergabeprotokoll?window.KCUebergabeprotokoll.erzeugen(payload,gesamt):null;
  blatt.innerHTML=`<div id="cashProtokollInhalt">
    <div class="protokoll-kopf">
      ...
      <div class="protokoll-qr">
        <canvas id="cashProtokollQrCanvas" width="230" height="230" aria-label="QR-Code mit dem vollständigen Protokollinhalt"></canvas>
        <small id="cashProtokollQrText">—</small>
      </div>
    </div>
    <h4>Inhalt gesamt</h4>
    <table class="protokoll-tabelle">${cashStueckelungsZeilen(gesamt)}
  ...
  blatt.hidden=false;
  const flaeche=el("cashProtokollQrCanvas"),beschriftung=el("cashProtokollQrText");
  if(flaeche&&beleg){
    try{
      drawRealQR(flaeche,beleg);
      const gelesen=window.KCUebergabeprotokoll.lesen(beleg);
      beschriftung.textContent="Beleg "+gelesen.id.slice(0,8).toUpperCase();
      window.KCUebergabeprotokoll.melden(gelesen,"pc-manager");
    }catch(err){
...
el("cashProtokoll")?.addEventListener("click",()=>{
  if(!el("cashQrPayload").value)return alert("Zuerst QR-Code erzeugen.");
  document.body.classList.add("print-protokoll-cash");
  window.print();
  setTimeout(()=>document.body.classList.remove("print-protokoll-cash"),300);
});
window.addEventListener("afterprint",()=>document.body.classList.remove("print-protokoll-cash"));
cashZeichneProtokoll({scope:"register",type:"opening",effectiveDate:localBusinessDate(),registerId:"KASSE-01",total:0},{looseBreakdown:{},coinRolls:{}});
el("cashProtokollBlatt").hidden=true;
...
el("generateCashQr").onclick=()=>requireAuth(()=>{
  ...
  el("cashResultType").textContent=payload.type==="opening"?"Anfangsbestand":payload.type==="topup"?"Nachfüllung":"Abendzählung";
  ...
  cashZeichneProtokoll(payload,cashData(),teile);
...
// 10.09.2026 (Betreiber, nach echten Scan-Problemen bei Money Butler): dieselbe Verschlankung
// hier auch fuer die vom Manager selbst erzeugten Kassen-Uebergaben - vorher sogar noch dichter
// als der (damals schon einmal verkuerzte) Money-Butler-Code, weil "breakdown" hier immer noch
// mitgeschickt wurde. Siehe money-butler/app.js schlankeStueckelung() fuer dieselbe Begruendung.
function schlankeStueckelung(c){
  const looseBreakdown={};
  Object.entries(c.looseBreakdown||{}).forEach(([wert,anzahl])=>{if(Number(anzahl)>0)looseBreakdown[wert]=anzahl});
  const coinRolls={};
  Object.entries(c.coinRolls||{}).forEach(([wert,rolle])=>{if(Number(rolle?.rolls)>0)coinRolls[wert]=rolle.rolls});
  return{looseBreakdown,coinRolls};
}
function baueCashTransferPayload(){
  const c=cashData(),schlank=schlankeStueckelung(c),effectiveDate=el("cashEffectiveDate").value,istZaehlung=el("cashType").value==="count";
  if(!isBusinessDate(effectiveDate))return{fehler:istZaehlung?"Bitte den gültigen Zähltag im Kalender auswählen.":"Bitte das gültige Einsatzdatum im Kalender auswählen."};
  // 10.09.2026: bei der Abendzaehlung DARF das Datum in der Vergangenheit liegen (wird oft erst
  // am naechsten Morgen erfasst) - derselbe Grund wie beim min-Attribut oben.
  if(!istZaehlung&&effectiveDate<localBusinessDate())return{fehler:"Das Einsatzdatum darf nicht in der Vergangenheit liegen."};
  if(c.total<=0)return{fehler:"Bitte mindestens eine Stückelung oder Münzrolle eingeben."};
  const payload={format:istZaehlung?"KC_CASH_COUNT":"KC_CASH_TRANSFER",version:istZaehlung?3:4,transferId:crypto.randomUUID(),countId:istZaehlung?crypto.randomUUID():undefined,registerId:el("cashRegister").value,type:el("cashType").value,time:new Date().toISOString(),effectiveDate,looseBreakdown:schlank.looseBreakdown,coinRolls:schlank.coinRolls,looseTotal:c.looseTotal,rollTotal:c.rollTotal,total:+c.total.toFixed(2),note:el("cashNote").value.trim(),receiptCount:istZaehlung&&el("cashBelegeAnzahl").value!==""?Number(el("cashBelegeAnzahl").value):undefined,closingId:istZaehlung&&cashSollAbschluss?cashSollAbschluss.closingId:undefined};
  if(istKassettenZiel()){
...
// 10.09.2026 (Betreiber: "der eingebaute Money Butler im PC Manager muss genauso sein wie der
// standalone, z.B. die 3 Datumsknoepfe") - dieselben drei Schnellknoepfe wie in der
// eigenstaendigen Money-Butler-App (Gestern/Heute/Morgen). "Gestern" ist hier IMMER gesperrt,
// weil dieser Bereich (anders als die App) keine Abendzaehlung kennt - cashType hat nur
// "opening"/"topup", niemals "count" - dieselbe Erklaerung wie in der App, statt den Knopf
// einfach wegzulassen.
document.querySelectorAll("[data-cash-date-quick]").forEach(b=>b.onclick=()=>{
  const versatz=Number(b.dataset.cashDateQuick);
  // 10.09.2026 AKTUALISIERT: seit die Abendzaehlung jetzt auch hier existiert, gilt dieselbe
  // Regel wie in der App - "Gestern" nur bei der Zaehlung, sonst weiterhin gesperrt.
  if(versatz<0&&el("cashType").value!=="count")return alert("„Gestern“ gibt es nur bei der Abendzählung - bei Anfangsbestand/Nachfüllung muss der Tag heute oder später sein.");
  const ziel=new Date(Date.now()+versatz*86400000);
  el("cashEffectiveDate").value=localBusinessDate(ziel);
  el("cashEffectiveDate").dispatchEvent(new Event("input"));
});

// 10.09.2026 (Betreiber: "der eingebaute Money Butler im PC Manager muss auch angepasst
// werden"): dieselben zwei Wege wie in der eigenstaendigen Money-Butler-App - hier braucht es
// KEINE eigene Anmeldung, der Manager hat bereits eine (KCSupabase, siehe
// kc-manager-supabase-status.js). sourceProgram bleibt "kc-money-butler" - das ist dieselbe
// fachliche Handlung, unabhaengig davon, ob sie aus der App oder von hier aus ausgeloest wird.
function kcFinanceClient(){
  if(!window.KCCommunicationClient)return null;
  return new window.KCCommunicationClient({sourceProgram:"kc-money-butler",getAccessToken:async()=>window.KCSupabase?.holeZugriffsToken?.()||null});
}
el("cashSendeDatenbank").onclick=()=>requireAuth(async()=>{
  const{payload,teile,fehler}=baueCashTransferPayload();
  if(fehler)return alert(fehler);
  const feld=el("cashVersandResult");
  if(teile){feld.textContent="Für die Geldkassette (beide Kassen zusammen) bitte weiterhin QR-Code oder WLAN-Direktversand verwenden - die Datenbank kennt nur eine Zielkasse je Übergabe.";return}
  if(!window.KCSupabase?.istAngemeldet?.()){feld.textContent="Bitte zuerst bei Supabase anmelden (oben rechts).";return}
  const kc=kcFinanceClient(),fb=kc&&window.createKCFinanceBridge?window.createKCFinanceBridge(kc):null;
  if(!fb){feld.textContent="KC-Communication-Bausteine nicht geladen.";return}
  feld.textContent="Wird gesendet …";
  try{
    const antwort=await fb.createCashTransfer({registerId:payload.registerId,amount:payload.total,businessDate:payload.effectiveDate,variables:{type:payload.type,note:payload.note||""},correlationId:`money-${payload.transferId}`,orgId:"KC_WERNE"});
    if(!antwort?.ok)throw new Error(antwort?.error||"Unbekannter Fehler");
    feld.textContent=`${money(payload.total)} für ${payload.registerId} an die Finance Bridge gesendet.`;
  }catch(e){feld.textContent=`Konnte nicht gesendet werden: ${e.message}`}
});
el("cashSendeEmail").onclick=()=>requireAuth(async()=>{
  const{payload,fehler}=baueCashTransferPayload();
  if(fehler)return alert(fehler);
  const feld=el("cashVersandResult");
  if(!window.KCSupabase?.istAngemeldet?.()){feld.textContent="Bitte zuerst bei Supabase anmelden (oben rechts).";return}
  const kc=kcFinanceClient(),adapter=kc&&window.createKCCommunicationAdapter?window.createKCCommunicationAdapter(kc):null;
  if(!adapter){feld.textContent="KC-Communication-Bausteine nicht geladen.";return}
  feld.textContent="Wird verschickt …";
  try{
    await adapter.emit("cash_transfer_ready",{registerId:payload.registerId,amount:payload.total,emailText:`Geldübergabe (nur Information, kein automatischer Eintrag): ${payload.type==="opening"?"Anfangsbestand":"Nachfüllung"} ${money(payload.total)} für ${payload.registerId}, gültig ab ${payload.effectiveDate}.${payload.note?` Notiz: ${payload.note}`:""}`},{testOnly:false,correlationId:`mail-${payload.transferId}`});
    feld.textContent="Informations-E-Mail verschickt.";
  }catch(e){
    feld.textContent=(e.code==="EVENT_NOT_DEFINED"||e.code==="SOURCE_PROGRAM_NOT_ENABLED")?"Noch nicht verschickt: diese Benachrichtigung ist zentral noch nicht freigeschaltet.":`Konnte nicht verschickt werden: ${e.message}`;
  }
});
...
// 09.09.2026 (Betreiber): Entnahmen kamen beim Manager zwar an, wurden aber nirgends gezeigt -
// zwei getrennte, einfache Tabellen: normale Entnahmen und Reklamationen (mit Ergebnis/Kosten).
function renderWithdrawalsUndComplaints(){
  const ent=el("withdrawalsBody"),rek=el("complaintsBody");
  if(!ent||!rek)return;
  const normale=cashWithdrawals.filter(w=>!w.complaint).sort((a,b)=>String(b.time).localeCompare(String(a.time)));
  ent.innerHTML=normale.map(w=>`<tr><td>${w.registerName||w.registerId}</td><td>${String(w.time).slice(0,10)}</td><td>${escAttr(w.reason||"")}</td><td>${money(w.amount)}</td><td>${w.receiptAvailable?"✓ vorhanden":"– keiner"}</td><td>${escAttr(w.note||"")}</td></tr>`).join("")||`<tr><td colspan="6">Keine Entnahmen.</td></tr>`;
  const reklamationen=cashWithdrawals.filter(w=>w.complaint).sort((a,b)=>String(b.time).localeCompare(String(a.time)));
  const ERG_TEXT={ersatz:"Ersatz (kostenlos)",auszahlung:"Auszahlung",nichts:"Nichts"};
  rek.innerHTML=reklamationen.map(w=>{const art=w.complaint.articles?.[0];return `<tr><td>${w.registerName||w.registerId}</td><td>${String(w.time).slice(0,10)}</td><td>${escAttr(art?.name||"")}</td><td>${escAttr(w.complaint.reason||"")}</td><td>${ERG_TEXT[w.complaint.outcome]||escAttr(w.complaint.outcome||"")}</td><td>${money(w.amount)}</td></tr>`}).join("")||`<tr><td colspan="6">Keine Reklamationen.</td></tr>`;
}
function closingRows(){
  return closings.map(c=>{
    // 09.09.2026: zuerst die Zählung suchen, die AUSDRÜCKLICH zu diesem Abschluss gehört
    // (Money Butler legt die closingId mit hinein, sobald der Soll-Code eingelesen wurde) -
    // erst wenn keine verknüpfte Zählung existiert, auf die alte Näherung zurückfallen
    // (die letzte Zählung derselben Kasse, für Zähl-Codes ohne Verknüpfung).
    const verknuepft=cashCounts.filter(x=>x.closingId&&x.closingId===c.closingId);
    const candidates=verknuepft.length?verknuepft:cashCounts.filter(x=>x.registerId===c.registerId&&!x.closingId);
    ...
    const receiptSoll=Number.isFinite(c.receiptExpected)?c.receiptExpected:null;
    const receiptIst=count&&Number.isFinite(count.receiptCount)?count.receiptCount:null;
    const receiptDiff=receiptSoll!==null&&receiptIst!==null?receiptIst-receiptSoll:null;
    return {closing:c,count,actual,diff,receiptSoll,receiptIst,receiptDiff};
  });
}
function renderClosings(){
  const rows=closingRows();
  el("closingBody").innerHTML=rows.map(r=>`<tr><td>${r.closing.registerName||r.closing.registerId}</td><td>${String(r.closing.createdAt).slice(0,10)}</td><td>${money(r.closing.expectedCash)}</td><td>${r.actual===null?"—":money(r.actual)}</td><td class="${r.diff===0?"diff-ok":"diff-warn"}">${r.diff===null?"—":money(r.diff)}</td><td class="${r.receiptDiff===null?"":r.receiptDiff===0?"diff-ok":"diff-warn"}">${r.receiptSoll===null?"—":r.receiptIst===null?`${r.receiptSoll} / —`:`${r.receiptSoll} / ${r.receiptIst}${r.receiptDiff!==0?` (${r.receiptDiff>0?"+":""}${r.receiptDiff})`:""}`}</td><td>${Number(r.closing.staffTotal||0)?`${money(r.closing.staffTotal)} (${r.closing.staffCount||0})`:"—"}</td><td>${r.count?(r.diff===0?"Stimmt":"Differenz"):"Zählung fehlt"}</td></tr>`).join("");
  ...
  renderWithdrawalsUndComplaints();
...
document.querySelectorAll("[data-view=\"kassenkonfiguration\"]").forEach(b=>b.addEventListener("click",()=>setTimeout(renderFeatureConfig,0)));
