(function(global){
  "use strict";
  const VERSION="0.3.0",text=(value,max=120)=>String(value??"").slice(0,max),num=value=>Number(value)||0;
  // 0.3.0 (23.09.2026): Warengruppe und Leihglaeser (bei Personal/Helfer weggelassenes Pfand) und die
  // Helfer-Gruppe bleiben erhalten - vorher warf der Import alles ausser id/name/qty/price weg, eine
  // Helfer-Auswertung im PC-Manager haette keine einzige Gruppe gesehen. Altes Format bleibt lesbar.
  function item(row,index=0){const out={id:text(row?.id||`ITEM-${index+1}`,80),name:text(row?.name,100),qty:num(row?.qty),price:num(row?.price)};if(row?.category)out.category=text(row.category,40);const leih=Array.isArray(row?.leihPfand)?row.leihPfand.length:num(row?.leih);if(leih)out.leih=leih;return out}
  function transaction(row,source={}){
    return{transactionId:text(row?.transactionId||row?.id||`${source.registerId||row?.registerId||"UNKNOWN"}-${row?.bon||row?.bonNumber||row?.time}`,100),bon:text(row?.bon||row?.bonNumber,40),time:row?.time||row?.endTime||row?.startTime,registerId:text(source.registerId||row?.registerId||"UNKNOWN",80),registerName:text(source.registerName||row?.registerName,80),operator:text(row?.operator,80),type:text(row?.type||"sale",30),training:!!row?.training,method:text(row?.method||row?.payment,40),total:num(row?.total??row?.due),items:Array.isArray(row?.items)?row.items.map(item):[],...(row?.helperGroup?{helperGroup:text(row.helperGroup,60)}:{})}
  }
  ...
  function pack(rows){
    ...
    const packed=(rows||[]).map(raw=>{const row=transaction(raw),registerKey=`${row.registerId}\u0000${row.registerName}`;if(!registerMap.has(registerKey)){registerMap.set(registerKey,registers.length);registers.push([row.registerId,row.registerName])}
      return[row.transactionId,row.bon,row.time,registerMap.get(registerKey),dict(operatorMap,operators,row.operator),dict(typeMap,types,row.type),row.training?1:0,dict(methodMap,methods,row.method),row.total,row.items.map(entry=>entry.category||entry.leih?[articleIndex(entry),entry.qty,entry.price,entry.category||"",entry.leih||0]:[articleIndex(entry),entry.qty,entry.price]),...(row.helperGroup?[row.helperGroup]:[])];
    });
  ...
  function unpack(value){
    ...
    return value.d.map(row=>{const register=value.r?.[row[3]]||["",""],items=(row[9]||[]).map(entry=>{const article=value.a?.[entry[0]]||["",""];const out={id:article[0],name:article[1],qty:num(entry[1]),price:num(entry[2])};if(entry[3])out.category=text(entry[3],40);if(entry[4])out.leih=num(entry[4]);return out});
      return{transactionId:text(row[0],100),bon:text(row[1],40),time:row[2],registerId:register[0],registerName:register[1],operator:value.o?.[row[4]]||"",type:value.y?.[row[5]]||"sale",training:!!row[6],method:value.m?.[row[7]]||"",total:num(row[8]),items,...(row[10]?{helperGroup:text(row[10],60)}:{})};
    });
  }
