...
function pushPlan(manager, schichten, sourceUpdatedAt='2026-09-23T04:15:00.000Z') {
  const res = fakeRes();
  manager._dienstplanPush(
    { socket: { remoteAddress: '127.0.0.1' } },
    res,
    { eventId: 'KC_DP', sourceUpdatedAt, schichten }
  );
...
(async () => {
  ...
  const third = pushPlan(manager, [{ ...planA[0], end: '19:00' }], '2026-09-23T04:20:00.000Z');
  assert.equal(third.revision, 2);
  assert.equal(third.unchanged, false);
  manager.db.close();

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kc-dienstplan-v3-'));
  const dbPath = path.join(tmpDir, 'device.sqlite');
  const device1 = new DeviceCompanion({ dbPath });
  device1.db.prepare("UPDATE device_identity SET credential_id = ?, pinned_fingerprint = ? WHERE id = 1")
  ...
  device1._request = async () => ({
    ...
    revision: 7,
    eventId: 'KC_DP',
    sourceUpdatedAt: '2026-09-23T04:15:00.000Z',
    updatedAt: '2026-09-23T04:16:00.000Z',
  });
  await device1._syncDienstplan();

  let row = device1.db.prepare('SELECT * FROM dienstplan_cache WHERE id = 1').get();
  assert.equal(row.revision, 7);
  assert.equal(row.event_id, 'KC_DP');
  assert.equal(row.source_updated_at, '2026-09-23T04:15:00.000Z');
  assert.deepEqual(JSON.parse(row.schichten_json), planA);
  assert.ok(row.cached_at);
  device1.db.close();

  const device2 = new DeviceCompanion({ dbPath });
  row = device2.db.prepare('SELECT * FROM dienstplan_cache WHERE id = 1').get();
  assert.equal(row.revision, 7);
  assert.deepEqual(JSON.parse(row.schichten_json), planA);

  device2.db.prepare("UPDATE device_identity SET credential_id = ?, pinned_fingerprint = ? WHERE id = 1")
    .run('cred_test.secret', 'sha256:test');
  ...
  console.log('dienstplan-bridge-v3: OK');
