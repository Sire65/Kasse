...
for(const dir of ['pos','schulung/pos']){
 ...
 const api=context.window.KCImagesV3;assert.equal(Object.keys(api.entries).length,15);
 ...
 const unrelated={id:'glasplus',image:'assets/amaretto_auth.webp'};api.migrate(unrelated);assert.equal(unrelated.image,'assets/amaretto_auth.webp');
}
