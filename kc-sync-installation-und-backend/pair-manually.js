...
async function main() {
  ...
  // Liest NUR die bereits vorhandene Identität des laufenden Managers - erzeugt nichts neu,
  // kein Konflikt mit dem parallel laufenden Manager-Prozess.
  const managerDb = openManagerDb(managerDbPath);
  const identity = await loadOrCreateIdentity(managerDb, managerDbPath);

  // Vergibt einen neuen, einmalig gültigen Kopplungs-Token direkt in derselben Datenbank, die
  // der laufende Manager-Prozess ebenfalls verwendet - er übernimmt den Token beim nächsten
  // Kopplungsversuch automatisch, ohne dass der Manager-Prozess selbst neu gestartet werden muss.
