...
class DeviceCompanion {
  ...
  async _syncMasterData() {
    ...
    this.db.prepare(`
      INSERT INTO master_data_cache (id, groups_json, articles_json, packages_json, settings_json, revision, updated_at) VALUES (1, ?, ?, ?, ?, ?, ?)
    ...
    `).run(JSON.stringify(antwort.groups || []), JSON.stringify(antwort.articles || []), JSON.stringify(antwort.packages || []), JSON.stringify(antwort.settings || {}), antwort.revision, antwort.updatedAt || new Date().toISOString());
  }
  ...
  startLocalStatusServer(port = 47391, { allowedOrigins = ['null', 'http://127.0.0.1', 'http://localhost'], bindAddress = '127.0.0.1' } = {}) {
    ...
    const server = http.createServer(async (req, res) => {
      ...
      if (req.method === 'GET' && req.url.split('?')[0] === '/kc-sync-master-data') {
        const zeile = this.db.prepare('SELECT groups_json, articles_json, packages_json, settings_json, revision, updated_at FROM master_data_cache WHERE id = 1').get();
        res.writeHead(200, { 'Content-Type': 'application/json' });
