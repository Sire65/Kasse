...
class ManagerCompanion {
  ...
  _masterDataPush(req, res, body) {
    ...
    this.db.prepare(`
      INSERT INTO master_data (id, groups_json, articles_json, packages_json, settings_json, revision, updated_at) VALUES (1, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(id) DO UPDATE SET groups_json=excluded.groups_json, articles_json=excluded.articles_json, packages_json=excluded.packages_json, settings_json=excluded.settings_json, revision=excluded.revision, updated_at=excluded.updated_at
    `).run(JSON.stringify(body.groups), JSON.stringify(body.articles), JSON.stringify(packages), JSON.stringify(settings), neueRevision, new Date().toISOString());
  ...
  _masterDataGet(req, res) {
    const auth = this._authenticate(req);
    if (!auth.ok) return this._json(res, 401, { error: auth.error });
    const zeile = this.db.prepare('SELECT groups_json, articles_json, packages_json, settings_json, revision, updated_at FROM master_data WHERE id = 1').get();
    if (!zeile) return this._json(res, 200, { groups: [], articles: [], packages: [], settings: {}, revision: 0, updatedAt: null });
    this._json(res, 200, {
