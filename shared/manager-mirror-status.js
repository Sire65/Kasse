(()=>{
  ...
  function injectCss(){
    if(!document.querySelector('link[data-kc-mirror-status-css]')){const link=document.createElement('link');link.rel='stylesheet';link.href='shared/manager-mirror-status.css';link.dataset.kcMirrorStatusCss='1';document.head.appendChild(link);}
    if(!document.querySelector('link[data-kc-mirror-overview-css]')){const link=document.createElement('link');link.rel='stylesheet';link.href='shared/manager-mirror-overview.css';link.dataset.kcMirrorOverviewCss='1';document.head.appendChild(link);}
  }
  ...
  function autoOpenProblem(data){
    ...
    const core=$('kcMirrorCore');
    if(core){if(badCore){core.open=true;core.dataset.autoOpen='true';}else{delete core.dataset.autoOpen;}}
    const card=document.querySelector('#mirrorManagerDialog .mirror-manager-card');
    if(card){card.classList.remove('mirror-attention-yellow','mirror-attention-red');const signal=data?.live?.signal;if(signal==='red')card.classList.add('mirror-attention-red');else if(signal==='yellow')card.classList.add('mirror-attention-yellow');}
  }

  function apply(data){
    if(!data||document.body.classList.contains('mirror-sim-active'))return;
    const live=data.live||{},runs=Array.isArray(data.runs)?data.runs:[];const watchdog=runs.find(r=>r.run_type==='watchdog')||null;const active=Boolean(live.is_transferring);const color=live.signal||'gray';const currentTable=live.active_table||live.last_table||'–';const group=groupFor(currentTable);const kbps=Number(live.current_kilobytes_per_second||0);const avg=Number(live.average_kilobytes_per_second||0);const idx=Number(live.batch_index||0),total=Number(live.batch_total||0);const progress=total?Math.max(0,Math.min(100,idx/total*100)):(active?50:100);
