(()=>{
  ...
  function injectCss(){
    if(document.querySelector('link[data-kc-mirror-status-css]'))return;
    const link=document.createElement('link');link.rel='stylesheet';link.href='shared/manager-mirror-status.css';link.dataset.kcMirrorStatusCss='1';document.head.appendChild(link);
  }
  ...
  function buildExtra(){
    ...
    extra.innerHTML=`
      <div class="mirror-schedule-head"><strong>Automatik & Überwachung</strong><span id="mirrorActiveGroup">Aktive Gruppe: –</span></div>
      <div class="mirror-schedule-grid">
        <article><span class="mirror-group-badge">A</span><div><small>Mirror-Gruppe A</small><strong id="mirrorNextA">–</strong></div></article>
        <article><span class="mirror-group-badge">B</span><div><small>Mirror-Gruppe B</small><strong id="mirrorNextB">–</strong></div></article>
        <article><span class="mirror-group-badge">C</span><div><small>Mirror-Gruppe C</small><strong id="mirrorNextC">–</strong></div></article>
        <article><span id="mirrorWatchdogLed" class="mirror-led gray"></span><div><small>Watchdog</small><strong id="mirrorWatchdogCompact">–</strong></div></article>
      </div>`;
    const metrics=live.querySelector('.mirror-metrics');
    live.insertBefore(extra,metrics||null);
    return true;
  }

  function secondsToNext(offset){
    const now=new Date();
    const minute=now.getMinutes(),second=now.getSeconds();
    let add=((offset-minute)%5+5)%5;
    if(add===0&&second===0)return 0;
    if(add===0)add=5;
    return add*60-second;
  }
  const countdown=(sec)=>sec<=0?'jetzt':sec<60?`${sec}s`:`${Math.floor(sec/60)}:${String(sec%60).padStart(2,'0')} min`;
  function updateCountdowns(){
    if($('mirrorNextA'))$('mirrorNextA').textContent=`in ${countdown(secondsToNext(0))}`;
    if($('mirrorNextB'))$('mirrorNextB').textContent=`in ${countdown(secondsToNext(2))}`;
    if($('mirrorNextC'))$('mirrorNextC').textContent=`in ${countdown(secondsToNext(4))}`;
  }
  ...
  async function fetchDashboard(){
    ...
    const res=await fetch(`${SUPABASE_URL}/functions/v1/kc-db-mirror-control?action=dashboard`,{headers:{Authorization:`Bearer ${s.access_token}`,apikey:SUPABASE_KEY},cache:'no-store'});
    if(!res.ok)return null;
    return await res.json().catch(()=>null);
  }

  function colorForStatus(status){return status==='ok'?'green':status==='warning'?'yellow':status==='error'?'red':status==='running'?'blue':'gray';}
  function apply(data){
    if(!data||document.body.classList.contains('mirror-sim-active'))return;
    const live=data.live||{},runs=Array.isArray(data.runs)?data.runs:[];
    const watchdog=runs.find(r=>r.run_type==='watchdog')||null;
    const active=Boolean(live.is_transferring);
    const color=live.signal||'gray';
    const currentTable=live.active_table||live.last_table||'–';
    const group=groupFor(currentTable);
    const kbps=Number(live.current_kilobytes_per_second||0);
    const avg=Number(live.average_kilobytes_per_second||0);
    const idx=Number(live.batch_index||0),total=Number(live.batch_total||0);
    const progress=total?Math.max(0,Math.min(100,idx/total*100)):(active?50:100);
    ...
    const wdColor=colorForStatus(watchdog?.status);
    if($('mirrorWatchdogLed'))$('mirrorWatchdogLed').className=`mirror-led ${wdColor}`;
    if($('mirrorWatchdogCompact'))$('mirrorWatchdogCompact').textContent=watchdog?`${String(watchdog.status).toUpperCase()}${watchdog.finished_at?' · '+new Date(watchdog.finished_at).toLocaleTimeString('de-DE'):''}`:'Keine Daten';
    if($('mirrorWatchdog'))$('mirrorWatchdog').textContent=watchdog?`${String(watchdog.status).toUpperCase()} · ${String(watchdog.message||'')}`:'–';
  }

  async function tick(){
    updateCountdowns();
    const dialog=$('mirrorManagerDialog');
    if(!dialog?.open||!session()?.access_token||document.body.classList.contains('mirror-sim-active'))return;
    const data=await fetchDashboard();apply(data);
  }
