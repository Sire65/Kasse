...
export async function beginKeyRotation({expectedDeviceId,expectedRegisterId}={}){
  ...
  const pair=await generateSigningKeyPair();
  const pending={deviceId:active.deviceId,registerId:active.registerId,keyVersion:Number(active.keyVersion||1)+1,createdAt:new Date().toISOString(),previousKeyVersion:Number(active.keyVersion||1),privateKey:pair.privateKey,publicKey:pair.publicKey};
  await dbPut(PENDING_KEY_RECORD,pending);
  return enrollmentDescriptor(pending);
}
...
export async function activatePendingRotation({serverConfirmedKeyVersion}={}){
  const active=await dbGet(KEY_RECORD),pending=await dbGet(PENDING_KEY_RECORD);
  if(!active||!pending)throw new Error('PENDING_ROTATION_MISSING');
  ...
  pending.activatedAt=new Date().toISOString();await dbPut(KEY_RECORD,pending);await dbDelete(PENDING_KEY_RECORD);
...
export async function signRequest(identity,{method,url,bodyText='',timestamp=Math.floor(Date.now()/1000),nonce=generateNonce()}={}){
  ...
  const ts=String(timestamp);const canonical=await canonicalRequest(method,url,ts,nonce,bodyText);const signature=await signCanonical(identity.privateKey,canonical);
