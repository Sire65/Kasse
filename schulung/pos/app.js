...
const DEFAULT_PRODUCTS=[
 ...
 // im Manager ergänzen.
 {id:"schussrum",sortOrder:9000,name:"Schuss Rum",price:1.00,category:"Getränke",image:"assets/rum_version_3.png"},
 {id:"schussamaretto",sortOrder:9001,name:"Schuss Amaretto",price:1.00,category:"Getränke",image:"assets/amaretto_version_3.png"},
...
function cartAction(a,i){
  ...
  if(state.cart[i]&&state.cart[i].qty===0){const removed=state.cart.splice(i,1)[0];if(Array.isArray(state.discount?.keys)&&state.discount.keys.includes(removed.key)){state.discount.keys=state.discount.keys.filter(key=>key!==removed.key);if(!state.discount.keys.length)resetDiscount()}if(state.selectedCartKey===removed.key)state.selectedCartKey=state.cart[Math.min(i,state.cart.length-1)]?.key||null}
  renderCart()
}
...
el("staffBtn").onclick=()=>{
  if(mitgliederBuchungWartet){beendeMitgliederWartezustand();setSystemHint("Mitgliederbuchung abgebrochen","info");return}
  if(!state.cart.length)return showMessage("Kein Bon","0,00 €","Bitte zuerst Artikel wählen.");
