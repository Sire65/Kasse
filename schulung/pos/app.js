// Absicherung: crypto.randomUUID() existiert erst seit Safari 15.4 (Maerz 2022) - auf aelteren
// Geraeten (z.B. iPad mit aelterem iOS) wuerde jede der vielen Stellen, die diese Funktion
// nutzen (Verkaufsabschluss, Bon loeschen, Rabatte, Kassenabschluesse, ...), sofort mit einem
// stillen Fehler abbrechen, noch bevor irgendetwas gespeichert wird - der Klick kaeme an, es
// wuerde aber sichtbar nichts passieren. Ersetzt die Funktion einmalig durch eine gleichwertige,
// selbst gebaute Variante, falls sie fehlt - deckt alle bestehenden Aufrufstellen im Programm ab.
...
// 08.09.2026: sichtbarer Stempel des Aufbau-Stands - damit man am Tablet sofort sieht, ob die
// heutigen Dateien geladen sind (Starthinweis, Mehr-Fenster). Die Versionspruefung bleibt.
const KC_AUFBAU_STAND="Aufbau 08.09.2026";
...
// Die Mitgliedsnummer ist die Klammer: derselbe Schluessel steht im Ausweis-QR, in der
// Stechuhr und im Dienstplan. Fehlt hier ein Profil, zeigt die Stechuhr beim Einstempeln
// die nackte Mitgliedsnummer an (siehe anzeigenameFuer in time-clock-pos.js) - deshalb
// muessen ALLE gefuehrten Mitglieder hier stehen, nicht nur ein Teil.
...
// 09.09.2026 (Betreiber-Anforderung, feste Reihenfolge nach Verkaufshaeufigkeit, nicht aus
// echten Zahlen berechnet): Getraenke, Speisen, Pfand, Favoriten, Happy Hour. WG01/02/03 sind
// die festen IDs der Grundgruppen (siehe DEFAULT_GROUPS); "Favoriten" und "Happy Hour" haben
// keine eigene ID und werden ueber ihren Namen einsortiert (siehe categories()/keyFor unten).
DEFAULTS.categoryOrder=["WG01","WG02","WG03","Favoriten","Happy Hour"];
const OPTIONS={
...
const DEFAULT_PRODUCTS=[
 {id:"grot",name:"Glühwein rot",price:3.50,category:"Getränke",sortOrder:2,image:"assets/gluehwein_version_3.png",color:"#8b1e24",info:{status:"approved",shortDescription:"Heißer roter Glühwein",ingredients:"Rotwein, Zucker, Gewürze",additives:"Keine Angaben",allergens:{sulphites:"contained"},important:"Alkoholhaltig. Optional mit Rum oder Amaretto.",nutrition:{energyKj:355,energyKcal:85,fat:0,saturates:0,carbohydrate:9.5,sugars:9.5,protein:0.1,salt:0.01},source:"Lieferantenangabe",approvedAt:"2026-07-17"},optionGroup:"shot",depositComponents:[{id:"glass",name:"Glaspfand",price:2}]},
 {id:"gweiss",name:"Glühwein weiß",price:3.50,category:"Getränke",sortOrder:3,image:"assets/gluehwein_weiss_version_3.png",color:"#a36a20",info:{status:"approved",shortDescription:"Heißer weißer Glühwein",ingredients:"Weißwein, Zucker, Gewürze",additives:"Keine Angaben",allergens:{sulphites:"contained"},important:"Alkoholhaltig. Optional mit Rum oder Amaretto.",nutrition:{energyKj:350,energyKcal:84,fat:0,saturates:0,carbohydrate:9.2,sugars:9.2,protein:0.1,salt:0.01},source:"Lieferantenangabe",approvedAt:"2026-07-17"},optionGroup:"shot",depositComponents:[{id:"glass",name:"Glaspfand",price:2}]},
 {id:"feuer",name:"Feuerzangenbowle",price:5.00,category:"Getränke",sortOrder:4,image:"assets/feuerzangenbowle_version_3.png",color:"#a43f18",info:{ingredients:"Rotwein, Gewürze, Zuckerhut, Rum",allergens:"Enthält Sulfite",important:"Alkoholhaltig. Zusätzlich Glaspfand und Feuerzangenpfand beachten."},depositComponents:[{id:"glass",name:"Glaspfand",price:2},{id:"tong",name:"Feuerzangenpfand",price:2}]},
 {id:"apfel",name:"Apfelpunsch",price:2.50,category:"Getränke",sortOrder:6,image:"assets/apfelpunsch_version_3.png",depositComponents:[{id:"glass",name:"Glaspfand",price:2}]},
 {id:"roterfeger",name:"Roter Feger",price:2.50,category:"Getränke",sortOrder:5,image:"assets/roter_feger_version_3.png",color:"#b3202a",info:{status:"approved",shortDescription:"Alkoholfreier roter Kirschpunsch",ingredients:"Kirschsaft, Gewürze, Zucker",additives:"Keine Angaben",important:"Alkoholfrei - auch für Kinder geeignet.",source:"Vereinsangabe"},optionGroup:"shot",depositComponents:[{id:"glass",name:"Glaspfand",price:2}]},
 {id:"eier",name:"Eierlikörpunsch",price:4.50,category:"Getränke",sortOrder:1,image:"assets/eierpunsch_version_3.png",depositComponents:[{id:"glass",name:"Glaspfand",price:2}]},
 // 09.09.2026 (Betreiber): "Schuss Rum" und "Schuss Amaretto" gab es bisher NUR als
 // Zusatzwahl (optionGroup:"shot") zu Glühwein/Rotem Feger, nicht als eigenen, einzeln
 // verkaufbaren Artikel. Beides jetzt zusätzlich als eigene Kachel bei Getränke - die
 // Zusatzwahl-Variante bleibt unverändert bestehen. sortOrder bewusst sehr hoch (9000/9001),
 // damit beide IMMER ganz am Ende der Gruppe stehen, auch nach allen unsortierten Artikeln.
 // Kein Glaspfand angenommen (Schuss wird nicht im Pfandglas ausgegeben) - bei Bedarf einfach
 // im Manager ergänzen.
 {id:"schussrum",name:"Schuss Rum",price:1.00,category:"Getränke",sortOrder:9000,image:"assets/rum_version_3.png"},
 {id:"schussamaretto",name:"Schuss Amaretto",price:1.00,category:"Getränke",sortOrder:9001,image:"assets/amaretto_version_3.png"},
 {id:"sauerkraut",name:"Sauerkrauteintopf",price:5.50,category:"Speisen",sortOrder:4,image:"assets/sauerkraut_version_3.png",color:"#77643d",info:{ingredients:"Sauerkraut, Kartoffeln, Speck, Gewürze",allergens:"Kann Sellerie und Senf enthalten",important:"Heiß ausgegeben."}},
 {id:"sauerkrautmett",name:"Sauerkrauteintopf + Mettwurst",price:7.00,halfAllowed:true,halfPrice:4.00,sortOrder:3,/* 08.09.2026 (Betreiber): ein Artikel als halbe Portion freigegeben */category:"Speisen",image:"assets/sauerkraut_wurst_version_3.png",color:"#785332",info:{ingredients:"Sauerkrauteintopf mit Mettwurst",allergens:"Kann Sellerie, Senf und Spuren von Gluten enthalten",important:"Mettwurst separat im Bestand geführt."}},
 {id:"gruenkohl",name:"Grünkohl",price:5.50,category:"Speisen",sortOrder:2,image:"assets/gruenkohl_version_3.png",color:"#315e35",info:{ingredients:"Grünkohl, Kartoffeln, Zwiebeln, Gewürze",allergens:"Kann Senf und Sellerie enthalten",important:"Heiß ausgegeben."}},
 {id:"gruenkohlmett",name:"Grünkohl + Mettwurst",price:7.00,category:"Speisen",sortOrder:1,image:"assets/gruenkohl_wurst_version_3.png",color:"#365d36",info:{ingredients:"Grünkohlgericht mit Mettwurst",allergens:"Kann Senf, Sellerie und Spuren von Gluten enthalten",important:"Mettwurst separat im Bestand geführt."}},
 {id:"mettwurst",name:"Mettwurst",price:1.50,category:"Speisen",sortOrder:7,image:"assets/mettwurst_version_3.png"},
 {id:"hering",name:"Kartoffel mit Hering",price:4.50,category:"Speisen",sortOrder:5,image:"assets/hering_kartoffeln_version_3.png"},


 {id:"knirpsecreme",name:"Kartoffel mit Kartoffelcreme",price:3.50,category:"Speisen",sortOrder:6,image:"assets/creme_kartoffeln_version_3.png",color:"#a66e24",info:{ingredients:"Kartoffeln, \u00d6l, Gew\u00fcrze, Kartoffelcreme",allergens:"Creme kann Milch und Milchprodukte enthalten"}},
 ...
 {id:"glasplus",name:"Glaspfand",price:2.00,category:"Pfand",image:"assets/pfandglas_geben_bv2.webp",manualDeposit:true},
 {id:"zangeplus",name:"Feuerzangenpfand",price:2.00,category:"Pfand",image:"assets/feuerzange_placeholder.svg",manualDeposit:true},
 {id:"glasminus",name:"Glasrückgabe",price:-2.00,category:"Pfand",image:"assets/pfandglas_rueckgabe_bv2.webp"},
...
// im Scanner verdrahtet, aber NICHTS rief je "eintragen()" auf, um die Nummern tatsaechlich in
// die Artikel zu schreiben - die ganze Tabelle war bislang wirkungslos. Jetzt beim Start
// einmal nachgetragen (nur wo noch keine Nummer steht, von Hand vergebene bleiben unberuehrt).
try{window.KCArtikelnummern?.eintragen(PRODUCTS)}catch(e){}
// 10.09.2026 (Betreiber: "habe die Kasse frisch ueber den QR gestartet, Reihenfolge stimmt
// trotzdem nicht") - GENAU DASSELBE MUSTER wie der Artikelnummern-Fund von vorhin, nur bei der
// Reihenfolge: ein frischer Kassen-Start schliesst Zwischenspeicherung der KASSE selbst aus -
// die eigentliche Ursache ist der PC-Manager. Er hat seine EIGENEN, im Browser des PCs
// gespeicherten Artikeldaten (kcm_articles) - wurden die vor der heutigen sortOrder-Aenderung
// gespeichert, fehlt dort "sortOrder" komplett. Der Manager schickt diese Daten ueber
// kc-sync-master-data.js an die Kasse und UEBERSCHREIBT damit PRODUCTS - selbst ein voellig
...
const SORTORDER_BEKANNT={eier:1,grot:2,gweiss:3,roterfeger:4,schussrum:9000,schussamaretto:9001,gruenkohlmett:1,gruenkohl:2,sauerkrautmett:3,sauerkraut:4,hering:5,knirpsecreme:6};
...
// schickt Muenzrollen jetzt nur noch als reine Rollenanzahl statt als 5-Werte-Objekt - Muenzen
// je Rolle und Wert je Rolle lassen sich aus der Rollenanzahl und dieser (auf allen drei
// Seiten gleichen) Tabelle vollstaendig zurueckrechnen. Dieselben Werte wie in
// money-butler/app.js COIN_ROLLS und pc-manager/app.js.
const MUENZEN_JE_ROLLE={2:25,1:25,0.5:40,0.2:40,0.1:40,0.05:50,0.02:50,0.01:50};
// Liest eine Muenzrolle unabhaengig vom Format: neu = reine Zahl (Rollenanzahl), alt = Objekt
// mit .rolls (aeltere, noch nicht aktualisierte Absender - z.B. ein Money Butler, der noch
// nicht auf dem neuesten Stand ist). Gibt {rolls, coinsPerRoll} zurueck, nie ein defektes Feld.
function rollenLesen(wert,rolle){
  const istZahl=typeof rolle==="number"||typeof rolle==="string";
  const rolls=Math.max(0,Number(istZahl?rolle:rolle?.rolls)||0);
  const coinsPerRoll=Math.max(0,Number(istZahl?MUENZEN_JE_ROLLE[wert]:(rolle?.coinsPerRoll||MUENZEN_JE_ROLLE[wert]))||0);
  return{rolls,coinsPerRoll};
}
const REQUIRED_DEPOSIT_RETURNS=[
 {id:"glasminus",name:"Glasrückgabe",price:-2.00,category:"Pfand",image:"assets/pfandglas_rueckgabe_bv2.webp",color:"#9f1239"},
...
if(depositReturnMigration)localStorage.setItem("kc_products_v050",JSON.stringify(PRODUCTS));

// Freigegebene Speisenbilder und Preise auf bereits gespeicherte Artikel anwenden.
...
if(changed)localStorage.setItem("kc_products_v050",JSON.stringify(PRODUCTS));}

// Artikelnummern aus der gemeinsamen Tabelle (shared/kc-artikelnummern-core.js) in das
// vorhandene Feld "barcode" eintragen. Die Artikel-ID bleibt unveraendert - sie haengt in
// Bons, Auswertungen und Bildpfaden. Eine von Hand abweichend gesetzte Nummer wird nicht
// ueberschrieben, sondern nur in der Konsole gemeldet.
...
// Bilderversion 2: nur bekannte alte Bildpfade ersetzen; eigene Bilder und Artikeldaten erhalten.
const TRAINING_BV2_IMAGES={
  "assets/eierlikoerpunsch_auth.webp": "assets/eierlikoerpunsch_bv2.webp",
  "assets/gluehwein_rot_11-09.webp": "assets/gluehwein_rot_bv2.webp",
  "assets/gluehwein_weiss_auth.webp": "assets/gluehwein_weiss_bv2.webp",
  "assets/feuerzangenbowle_gewuerze_11-09.webp": "assets/feuerzangenbowle_bv2.webp",
  "assets/apfelpunsch_apfel_oben_11-09.webp": "assets/apfelpunsch_bv2.webp",
  "assets/roter_feger_kirschen_11-09.webp": "assets/roter_feger_bv2.webp",
  "assets/rum_flasche_11-09.webp": "assets/rum_flasche_bv2.webp",
  "assets/amaretto_flasche_11-09.webp": "assets/amaretto_flasche_bv2.webp",
  ...
  "assets/gruenkohl_mettwurst_auth.webp": "assets/gruenkohl_mettwurst_bv2.webp",
  "assets/hering_kartoffeln_auth.webp": "assets/hering_kartoffeln_bv2.webp",
  "assets/kartoffelcreme_auth.webp": "assets/kartoffelcreme_bv2.webp",
  "assets/mettwurst_auth.webp": "assets/mettwurst_bv2.webp",
  "assets/gluehwein_rot_auth.webp": "assets/gluehwein_rot_bv2.webp",
  "assets/gluehwein_rot.webp": "assets/gluehwein_rot_bv2.webp",
  "assets/gluehwein_weiss.webp": "assets/gluehwein_weiss_bv2.webp",
  "assets/feuerzangenbowle_auth.webp": "assets/feuerzangenbowle_bv2.webp",
  "assets/apfelpunsch_auth.webp": "assets/apfelpunsch_bv2.webp",
  "assets/roter_feger.webp": "assets/roter_feger_bv2.webp",
  "assets/amaretto_auth.webp": "assets/amaretto_flasche_bv2.webp"
};
function trainingBv2Image(image){return TRAINING_BV2_IMAGES[image]||image}
{let changed=false;for(const p of PRODUCTS){const image=trainingBv2Image(p.image);if(image!==p.image){p.image=image;changed=true}}if(changed)localStorage.setItem("kc_products_v050",JSON.stringify(PRODUCTS));}
...
const DEFAULT_PACKAGES=[
 {id:"PKG-GK-GR",name:"Grünkohl + Glühwein rot",componentIds:["gruenkohl","grot"],price:11.00,category:"Kombi",active:true,autoManaged:false,source:"manual",note:"Startpackage"},
...
// Auswertung und auf dem Etikett, deshalb wird er an EINER Stelle gefuehrt.
const KOMBI_GRUPPE="Kombi";
{// vorhandene Geraete tragen die alte Bezeichnung noch im Speicher - einmalig nachziehen
 let geaendert=false;
 GROUPS.forEach(g=>{if(g.name==="Packages"||g.shortName==="Packages"){g.name=KOMBI_GRUPPE;g.shortName=KOMBI_GRUPPE;geaendert=true}});
...
function packageProductView(pkg,index=0){
  ...
  // Eine Kombination aus zwei Artikeln mit NUR EINEM Bild ist am Stand nicht erkennbar -
  // man sieht den Gruenkohl und tippt ihn an, obwohl der Punsch mitverkauft wird. Die Kachel
  // zeigt deshalb beide: das Getraenk oben, das Essen unten rechts, getrennt durch eine
  // schraege Linie. Reihenfolge nach KATEGORIE, nicht nach Eingabereihenfolge - sonst haengt
  // das Bild davon ab, was jemand beim Anlegen zuerst getippt hat.
  ...
  return {...pkg,isPackage:true,category:KOMBI_GRUPPE,image:lead?.image||"assets/logo.webp",color:"#6d28d9",kombiBilder,
    barcode:pkg.barcode||window.KCArtikelnummern?.fuerArtikel(pkg.id)
      ||window.KCArtikelnummern?.tagespackageNummer(index+1)||"",
    info:{status:"approved",shortDescription:`Package aus ${components.map(p=>p.name).join(" + ")}`,important:pkg.note||"Extern freigegeben."}};
}
...
// Die Happy Hour wird ab jetzt im PC-Manager gesteuert (Ansicht "Happy Hour" plus die
// Felder am Artikel); die Kasse holt sich den Zeitplan beim Stammdaten-Abgleich und
// uebersetzt ihn in Angebote - siehe kc-happyhour-pos.js.
//
// DIESE BEISPIELAKTION IST DESHALB AUSGESCHALTET (active:false). Sie stand ab Werk auf
// aktiv und lief taeglich von 17 bis 18 Uhr von selbst an - also genau zur anlaufenden
// Stosszeit am Markttag, ohne dass jemand sie gestartet haette. Sie bleibt als Vorlage
// stehen, damit man den Aufbau eines Angebots nachlesen kann.
const DEFAULT_OFFERS=[
 {id:"OFFER-HH-GLUEHWEIN",name:"Happy Hour Glühwein (Vorlage, ausgeschaltet)",type:"happyhour",productIds:["grot","gweiss"],priceMode:"percent",priceValue:10,
  startDate:"",endDate:"",startTime:"17:00",endTime:"18:00",weekdays:[1,2,3,4,5,6,0],active:false,manualStart:true,note:"Vorlage – die Happy Hour wird im PC-Manager gepflegt."}
];
...
const state={master:{...DEFAULTS,...previousMaster,...(window.KCSyncConnection?.config?.registerId?{registerId:window.KCSyncConnection.config.registerId}:{})},cart:[],cartStartedAt:null,activeCategory:"Getränke",given:0,cashSelections:[],discount:{percent:0,reason:"",note:"",keys:[]},keypadBuffer:"",lastAdded:null,selectedCartKey:null,operatorConfirmedForSale:previousMaster.requireOperatorConfirmation!==true,role:"cashier",scanBuffer:"",scanTimer:null,pendingProduct:null,groupIndex:0,articleIndex:0,groupEditMode:false,articleEditMode:false,productPage:0,lastSelectedProduct:null,workflowMode:null,keypadMode:"cash"};
if(window.KCSyncConnection?.config?.registerId&&window.KCSyncConnection.config.registerId!==previousMaster.registerId)saveMaster();
window.__kcPinLockEnabled=state.master.pinLockEnabled!==false;
const KC_EMBEDDED_TRAINING=new URLSearchParams(location.search).get("embeddedTraining")==="1";
...
function safeImage(value){const image=trainingBv2Image(String(value||"").trim());return /^(assets\/[A-Za-z0-9._/-]+|data:image\/(png|jpeg|webp|gif);base64,)/i.test(image)?image:"assets/logo.webp"}
...
// Geraete, die schon laufen, haben die alte, unvollstaendige Bedienerliste im Speicher -
// DEFAULTS greifen dort nicht mehr. Deshalb einmalig die fehlenden Profile nachtragen
// (Abgleich ueber die Mitgliedsnummer, nicht ueber den Namen). Danach bleibt die Liste
// frei bearbeitbar; die Nachruestung wiederholt sich nicht.
// BEFUND 02.09.2026: die Kasse ARBEITET mit ihrer Bedienerliste, legt sie aber nie ab, solange
// niemand etwas in den Einstellungen aendert - im Speicher steht dann nur {neuesLayout}. Fuer die
// Kasse selbst ist das folgenlos (sie faellt auf dieselben Vorgaben zurueck), fuer jedes andere
// Werkzeug auf diesem Geraet aber nicht: das Scanner-Testblatt las dort nach und fand nichts.
// Ein Zustand, den nur ein Teil des Programms kennt, ist genau die Sorte Halbwahrheit, an der
// hier schon mehrfach Zeit verloren ging. Deshalb wird die tatsaechlich benutzte Liste einmal
// festgeschrieben - dieselben Daten, nur nicht mehr unsichtbar.
function sichereBedienerlisteAb(){
  try{
    const roh=JSON.parse(localStorage.getItem("kc_master_v040")||"null");
    if(roh&&Array.isArray(roh.operatorProfiles)&&roh.operatorProfiles.length)return false;
    normalizeOperatorProfiles();
    saveMaster();
    return true;
  }catch(err){return false}
}
function ensureBedienerstamm(){
  const FLAG="kc_bedienerstamm_v18_migriert";
  try{
    if(localStorage.getItem(FLAG)==="done"){sichereBedienerlisteAb();return false}
    const vorhanden=normalizeOperatorProfiles();
    const bekannt=new Set(vorhanden.map(x=>x.memberNo).filter(Boolean));
    const fehlend=KC_BEDIENERSTAMM.filter(x=>x.memberNo&&!bekannt.has(x.memberNo));
    if(fehlend.length){state.master.operatorProfiles=vorhanden.concat(fehlend.map(x=>({...x})));normalizeOperatorProfiles();saveMaster()}
    localStorage.setItem(FLAG,"done");
    sichereBedienerlisteAb();
...
// bestehenden Aufrufstellen von readTransactions()/saveTransactions() unverändert weiter sehen.
let _txCache=[],_trainingTxCache=[];
try{migrateTransactions()}catch(err){console.error("Transaktionsmigration (alte Schlüsselversionen) fehlgeschlagen",err)}
const _txHydrated=hydrateTransactionCache().catch(err=>{console.error("IndexedDB-Umsatzspeicher konnte nicht geladen werden, Kasse arbeitet vorerst nur im Arbeitsspeicher",err);return false});
window.__kcTxHydrated=_txHydrated;
window.__kcRehydrate=hydrateTransactionCache;
async function hydrateTransactionCache(){
  if(!window.KCTransactionStore)return false; // Modul nicht eingebunden (z.B. alter Cache) - Kasse funktioniert trotzdem, nur ohne dauerhafte IndexedDB-Speicherung
  const MIGRATION_DONE_KEY="kc_indexeddb_tx_migrated_v001";
  // Beim allerersten Start auf einem Gerät: die bisherigen localStorage-Daten (bereits durch
  // migrateTransactions() von älteren Schlüsselversionen konsolidiert) als "lokalen Bestand" in
  // die Zusammenführung geben. Bei jedem weiteren Start ist dieser Bestand bewusst leer - die
  ...
  _txCache=salesResult.rows;_trainingTxCache=trainingResult.rows;
  if(firstRun)localStorage.setItem(MIGRATION_DONE_KEY,"done");
  if(salesResult.recovered||trainingResult.recovered){
...
function saveTransactions(rows,training=false){
  ...
  // versucht und der Bedienperson eine sichtbare Warnung angezeigt (eine fehlgeschlagene
...
function beginAdminSession(loginMethod,actor,credentialId=null){
  adminSession={sessionId:crypto.randomUUID(),startedAt:new Date().toISOString(),loginMethod,actor,credentialId,valid:true};
  state.role="superadmin";
  document.body.classList.add("role-admin","role-superadmin","admin-open");
  appendAdminAudit("login","success");
  // Chromium/Tablet: modale Dialoge nicht im selben Ereigniszyklus wechseln.
  // Der direkte close()+showModal()-Wechsel konnte Backdrop und UI einfrieren.
  runAfterDialogClosed(el("serviceDialog"),()=>{
    const home=el("adminHomeDialog");
    if(home&&!home.open)home.showModal();
  });
}
...
function setSystemHint(text,type="ok",richtung=null){
  ...
  const n=el("notificationBar");if(!n)return;n.textContent=String(text||"");
}
...
function applyModes(){
  if(state.master.allowTraining===false&&state.master.trainingMode)state.master.trainingMode=false;
  if(state.master.showRushMode===false&&state.master.rushMode)state.master.rushMode=false;
  ...
  // BEFUND aus der TÜV-Prüfung: die Anzeige "Normalbetrieb" in der Kopfzeile stand fest
  // im HTML und wurde NIE geändert - sie zeigte also auch im Trainingsmodus und in
  // Stoßzeiten weiter "Normalbetrieb" an. Wer im Betrieb kurz auf die Kopfzeile schaut,
  // bekam damit eine falsche Auskunft darüber, ob echt gebucht wird. Jetzt sagt sie die
  // Wahrheit, und im Trainingsmodus fällt sie zusätzlich farblich auf.
  const modus=el("modeStatus");
  if(modus){
    modus.textContent=labels.length?labels.join(" · "):"Normalbetrieb";
    modus.classList.toggle("modus-training",!!state.master.trainingMode);
    modus.classList.toggle("modus-stoss",!!state.master.rushMode&&!state.master.trainingMode);
    modus.title=state.master.trainingMode
      ?"Trainingsmodus: Buchungen zählen NICHT als echter Umsatz"
      :(state.master.rushMode?"Stoßzeiten: verkürzte Bedienung":"Normalbetrieb: Buchungen zählen als echter Umsatz");
  }

  document.body.classList.toggle("rush-mode",!!state.master.rushMode);

  // BEFUND 31.08.2026 (Generalprobe): body.training-mode wurde AUSSCHLIESSLICH in
  // renderWorkspaceModePanel() gesetzt, und diese Funktion wird nur aus
  // renderDiscountSummary() heraus aufgerufen - also erst beim naechsten Neuzeichnen des
  // Bons. Wer den Trainingsmodus bei leerem Bon einschaltete, bekam deshalb NICHT den
  // orangefarbenen Rahmen um den ganzen Bildschirm, nicht die dunklere Kopfzeile und nicht
  // die blaue Arbeitsflaeche - also genau die Signale, an denen man von weitem erkennt,
  // dass hier gerade NICHT echt gebucht wird. Sie kamen erst, sobald jemand einen Artikel
  // antippte. Am Stand ist das die falsche Reihenfolge: erst sieht man es, dann bucht man.
  // Gemessen: nach dem Umschalten stand in der Kopfzeile "Trainingsmodus aktiv", aber
  // document.body trug die Klasse nicht.
  renderWorkspaceModePanel();
}

// Vom Manager eingetroffene Darstellung sofort anwenden (siehe kc-sync-master-data.js).
// Ohne das wuerde eine geaenderte Knopfgroesse erst nach einem Neustart der Kasse sichtbar -
// am Stand faellt das niemandem auf, und es sieht aus, als sei die Uebertragung gescheitert.
window.applyMasterSettings=function(){
  try{
    state.master={...state.master,...JSON.parse(localStorage.getItem("kc_master_v040")||"{}")};
    applyVisibility();renderHeader();renderCategories();renderProducts();
  }catch(e){console.error("Darstellung konnte nicht angewendet werden",e)}
};
function applyVisibility(){applyModes();
  document.body.classList.toggle("hide-staff",state.master.showStaff===false);
  // Neues Kassenlayout: wird ueber eine einzige Klasse am body geschaltet. Steht sie nicht,
  // greift keine einzige der neuen Regeln und die Kasse sieht exakt aus wie bisher.
  document.body.classList.toggle("kc-layout-neu",state.master.neuesLayout===true);
  ...
  document.body.classList.toggle("hide-card",state.master.showCard===false);
  document.body.classList.toggle("hide-account",state.master.showAccount===false);
  document.body.classList.toggle("hide-discount",state.master.showDiscount===false);
  document.body.classList.toggle("hide-happyhour",state.master.showHappyHour===false);
  document.body.classList.toggle("hide-rush",state.master.showRushMode===false);
  document.body.classList.toggle("hide-training",state.master.allowTraining===false);
...
// Warengruppen-Symbole als gezeichnete Bilder statt Emoji.
//
// BEFUND (User am Stand): "man erkennt die Symbole nicht gut". Ursache: es waren Farb-Emoji.
// Die lassen sich NICHT einfaerben - auf dunklem Gruppen-Hintergrund bleibt das Recycling-Zeichen
// gruen und das Fragezeichen rot, und auf jedem Geraet sehen sie anders aus (Windows, Linux, iPad
// haben eigene Emoji-Schriften). Gezeichnete Symbole nehmen die Schriftfarbe an, sind ueberall
// gleich und beliebig vergroesserbar.
const KATEGORIE_BILDER={
  // Flasche und Glas nebeneinander - vom User so gewuenscht.
  "Getränke":'<path d="M4 8h6l-.7 11a1.6 1.6 0 0 1-1.6 1.5H6.3A1.6 1.6 0 0 1 4.7 19L4 8z"/><path d="M4.4 12h5.2"/><path d="M16 2.5h3v2.2c0 .6.3 1.1.7 1.5.8.7 1.3 1.7 1.3 2.8V19a1.6 1.6 0 0 1-1.6 1.5h-2.8A1.6 1.6 0 0 1 15 19V9c0-1.1.5-2.1 1.3-2.8.4-.4.7-.9.7-1.5V2.5z"/><path d="M15 12h6"/>',
  // Teller in der Mitte, Messer links, Gabel rechts - vom User so gewuenscht.
  "Speisen":'<circle cx="12" cy="12" r="5.6"/><circle cx="12" cy="12" r="2.6"/><path d="M3.4 3.2c1 1.5 1.5 3.1 1.5 4.8 0 1-.5 1.8-1.4 2.2M3.5 21V10.2"/><path d="M19 3.2v4.2M21 3.2v4.2M20 7.4c1 0 1.7.8 1.6 1.8L21 21"/>',
  // 08.09.2026 (Betreiber): Rueckgabe als Kreislaufpfeile (Recycling-Zeichen) statt Kasten.
  "Pfand":'<path d="M4 12a8 8 0 0 1 14-5.3"/><path d="M18 3v4h-4"/><path d="M20 12a8 8 0 0 1-14 5.3"/><path d="M6 21v-4h4"/>',
  "Favoriten":'<path d="m12 3.4 2.6 5.3 5.8.8-4.2 4.1 1 5.8-5.2-2.7-5.2 2.7 1-5.8L3.6 9.5l5.8-.8z" fill="currentColor" stroke="none"/>',
  "Sonstiges":'<path d="M8.6 8.6a3.5 3.5 0 1 1 4.6 3.3c-.9.3-1.5 1.1-1.5 2v.8"/><circle cx="11.7" cy="18.6" r="1.1" fill="currentColor" stroke="none"/>',
  // 08.09.2026 (Betreiber): zwei Kettenglieder fuer Kombi/Paket statt Geschenk.
  "Kombi":'<path d="M10 13a4 4 0 0 0 5.7 0l3-3a4 4 0 0 0-5.7-5.7l-1.5 1.5"/><path d="M14 11a4 4 0 0 0-5.7 0l-3 3a4 4 0 0 0 5.7 5.7l1.5-1.5"/>',
  // 08.09.2026 (Betreiber): wie Kombi - zwei Kettenglieder.
  "Packages":'<path d="M10 13a4 4 0 0 0 5.7 0l3-3a4 4 0 0 0-5.7-5.7l-1.5 1.5"/><path d="M14 11a4 4 0 0 0-5.7 0l-3 3a4 4 0 0 0 5.7 5.7l1.5-1.5"/>',
  "Angebote":'<path d="M20.5 12.7 12.8 20.4a1.7 1.7 0 0 1-2.4 0l-6.8-6.8a1.7 1.7 0 0 1-.5-1.2V4.6c0-.9.7-1.6 1.6-1.6h7.8c.5 0 .9.2 1.2.5l6.8 6.8c.7.7.7 1.7 0 2.4z"/><circle cx="7.6" cy="7.6" r="1.3"/>',
  // 08.09.2026 (Betreiber): Sonne fuer Happy Hour statt Cocktailglas (klar unterscheidbar von Getraenke).
  "Happy Hour":'<circle cx="12" cy="12" r="4"/><path d="M12 2.5V5M12 19v2.5M2.5 12H5M19 12h2.5M5.3 5.3l1.8 1.8M16.9 16.9l1.8 1.8M5.3 18.7l1.8-1.8M16.9 7.1l1.8-1.8"/>',
};
function kategorieSymbol(name){
  const bild=KATEGORIE_BILDER[name];
  if(bild)return `<svg class="kategorie-bild" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${bild}</svg>`;
  // Unbekannte Gruppen (im Manager frei anlegbar) behalten ihr Kuerzel.
  return `<span class="kategorie-kuerzel" aria-hidden="true">${escapeHtml(name.slice(0,2).toUpperCase())}</span>`;
}

// --- Farbe der Warengruppe lesbar machen -------------------------------------------------
// BEFUND (User): "mach die Farben nicht ganz so dunkel, man erkennt die Symbole darin nicht gut".
// Die Farben kommen aus dem PC-Manager und koennen beliebig sein - deshalb wird NICHT pauschal
// aufgehellt, sondern gerechnet: die Flaeche wird zur hellen Fassung der Gruppenfarbe, und
// Schrift und Symbol werden je nach Helligkeit automatisch dunkel oder weiss. Damit bleibt jede
// kuenftig eingestellte Farbe lesbar. Die AUSGEWAEHLTE Gruppe behaelt die kraeftige Farbe.
function farbeZuRgb(farbe){
  const wert=String(farbe||"").trim();
  const kurz=/^#([0-9a-f])([0-9a-f])([0-9a-f])$/i.exec(wert);
  if(kurz)return [parseInt(kurz[1]+kurz[1],16),parseInt(kurz[2]+kurz[2],16),parseInt(kurz[3]+kurz[3],16)];
  const lang=/^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i.exec(wert);
  if(lang)return [parseInt(lang[1],16),parseInt(lang[2],16),parseInt(lang[3],16)];
  const rgb=/^rgba?\(([^)]+)\)$/i.exec(wert);
  if(rgb){const t=rgb[1].split(",").map(x=>parseInt(x,10));if(t.length>=3&&t.every(Number.isFinite))return t.slice(0,3)}
  return null;
}
// Wahrgenommene Helligkeit (0-255) nach der ueblichen Gewichtung: Gruen wirkt heller als Blau.
const farbHelligkeit=([r,g,b])=>0.299*r+0.587*g+0.114*b;
const mitWeiss=([r,g,b],anteil)=>[r,g,b].map(x=>Math.round(x+(255-x)*anteil));
const alsHex=([r,g,b])=>"#"+[r,g,b].map(x=>Math.max(0,Math.min(255,x)).toString(16).padStart(2,"0")).join("");
function gruppenFarben(farbe){
  const rgb=farbeZuRgb(farbe)||farbeZuRgb("#173765");
  // Dunkle Farben werden staerker aufgehellt als ohnehin helle - so wird keine Farbe blass.
  const hell=mitWeiss(rgb,farbHelligkeit(rgb)<110?.74:.62);
  const vorder=farbHelligkeit(hell)>150?alsHex(rgb.map(x=>Math.round(x*.45))):"#ffffff";
...
function allProductsForCategory(){
  ...
  // Reihenfolge, in der die Artikel im Datenbestand stehen. Jetzt: Artikel mit einer
  // ausdrücklich gesetzten Nummer (>0) zuerst, in genau dieser Reihenfolge; alle anderen
  // danach unverändert in ihrer bisherigen Reihenfolge (stabile Sortierung - kein Artikel
  // "springt", der keine Nummer hat).
  list=list.map((p,i)=>({p,i})).sort((a,b)=>{const oa=a.p.sortOrder>0?a.p.sortOrder:1000+a.i,ob=b.p.sortOrder>0?b.p.sortOrder:1000+b.i;return oa-ob}).map(x=>x.p);
  const q=(el("productSearchInput")?.value||"").trim().toLowerCase();
  // Das Suchfeld durchsucht auch die Artikelnummer. Zwei Gruende: am Zapfhahn wird die Nummer
  // vom Etikett abgelesen und getippt, und ein Bluetooth-Scanner tippt seinen Code genau dorthin,
  // wenn der Cursor im Suchfeld steht (der Scanner verhaelt sich wie eine Tastatur). Ohne diese
  // Zeile blieb die Liste dann leer, obwohl der Artikel da ist.
  if(q){
    const nurZiffern=q.replace(/\D/g,"");
    list=productsForSale().filter(p=>{
      if(`${p.name} ${p.shortName||""} ${p.receiptText||""}`.toLowerCase().includes(q))return true;
      const bc=String(p.barcode||"").toLowerCase();
      if(bc&&bc.includes(q))return true;
      return !!nurZiffern&&!!bc&&bc.replace(/\D/g,"")===nurZiffern;
    });
  }
  return list;
}
function productsPerPage(){
  ...
  //
  // 11.09.2026, ECHTER FUND (Betreiber: "Kacheln muessen gut erreichbar und bedienbar sein" -
  // beim Nachpruefen der Groessenanpassung entdeckt, nicht gemeldet): die fruehere Loesung
  // maass live per getBoundingClientRect(), wie viele Kacheln hineinpassen wuerden - das lief
  // der eigentlichen Darstellung (passeArtikelgroesseAn(), setzt die Kachelgroesse ERST NACH
  // diesem Aufruf) zeitlich voraus UND war anfaellig fuer einen klassischen Ruckkopplungs-
  // Fehler: overflow-y:auto auf dem Raster blendet je nach Inhalt einen Scrollbalken ein oder
  // aus, der die gemessene BREITE um ein paar Pixel aendert - genau an der Kippschwelle
  // zwischen "5 Spalten passen" und "nur 4 Spalten passen" reichte das, um bei DERSELBEN
  // Warengruppe (Pfand, 5 Artikel) mal 5, mal nur 4 Kacheln zu zeigen. Der fuenfte Artikel
  // verschwand dabei nicht auf eine zweite Seite, er fiel einfach aus der Berechnung heraus -
  // bei einer echten Kasse waere das ein Artikel, der unauffindbar wird.
  // Deshalb jetzt grundsaetzlich anders geloest: KEINE Live-Messung, KEINE Vorab-Schaetzung
  // mehr. Die Seite zeigt IMMER alle Artikel der Gruppe auf einmal (passeArtikelgroesseAn()
  // waehlt dazu die passende Spalten-/Zeilenzahl, bis zu drei Zeilen); passt eine Warengruppe
  // ausnahmsweise auch verkleinert nicht auf eine Bildschirmseite, uebernimmt das ohnehin
  // vorhandene overflow-y:auto das Scrollen - zuverlaessiger als eine Berechnung, die kippen
  // kann. Damit kann kein Artikel mehr unbemerkt verschwinden.
...
function renderProductPager(){
  ...
  // Blaetterleiste nur zeigen, wenn es wirklich etwas zu blaettern gibt. Sie belegte sonst
  // dauerhaft 58px - Platz, der genau dann fehlt, wenn eine zweite Kachelreihe hineinpasste.
  // Dadurch wurden Artikel auf Seite 2 geschoben, obwohl darunter noch Flaeche frei war.
  // Nur im neuen Layout, damit sich am bisherigen nichts aendert.
  const leiste=document.querySelector(".pager");
  if(leiste&&document.body.classList.contains("kc-layout-neu")){
    const verstecken=pages<=1;
    if(leiste.hidden!==verstecken){
      leiste.hidden=verstecken;
      // Die Flaeche hat sich geaendert - also neu aufteilen. Der Schutz verhindert, dass sich
      // Ein- und Ausblenden gegenseitig immer wieder ausloesen.
      if(!renderProductPager._laeuft){renderProductPager._laeuft=true;setTimeout(()=>{renderProductPager._laeuft=false;renderProducts();},0);}
    }
  }else if(leiste){leiste.hidden=false;}
}
...
// 11.09.2026 (Betreiber: "Der Preis muss in der Kasse inkl. Pfand ausgegeben werden - Artikel
// 3,50 und Pfand 2,00 macht 5,50 als Preis"): NUR fuer die Anzeige auf der Kachel selbst -
// zeigt IMMER Artikelpreis+Pfand zusammen, unabhaengig von der Pfand-Regel-Einstellung
// (automatisch/manuell/inklusive). Der Bon/die Kasse rechnet weiterhin nach der bestehenden
// Regel ab (displayPrice/checkout unveraendert) - hier geht es nur um die Zahl auf dem Knopf.
function kachelPreis(p){
  const base=Number(p?.price||0);
  if(!Array.isArray(p?.depositComponents))return base;
  return base+p.depositComponents.reduce((sum,item)=>sum+Number(item.price||0),0);
}
  ...
  grid.innerHTML=productSet().map(p=>{
    ...
    return `<div class="product-tile-wrap ${imageV3?"image-v3":""} ${p.isPackage?"package-tile":""} ${p.isOffer?(p.isHappyHour?"happyhour-tile":"offer-tile"):""} ${infoVisible?"":"no-info"} ${state.lastSelectedProduct===p.id?"last-selected":""}">
      <button class="product-tile mode-${mode}" data-id="${p.id}" style="--tile-color:${p.color||"#315d8d"};--gruppen-rand:${(GROUPS.find(x=>x.name===p.category)||{}).color||p.color||"#315d8d"}" aria-label="${p.name} direkt verkaufen">
        ...
        <span class="product-label"><strong>${p.name}</strong><span class="${p.isOffer?"offer-price-line":""}">${p.isOffer?`<small class="offer-old-price">${money(p.originalPrice)}</small>`:""}${money(kachelPreis(p))}</span></span>
      </button>
...
// 11.09.2026 (Betreiber: "Artikelbuttons sollen den verfuegbaren Platz ausnutzen - bei 3
// Artikeln in einer Warengruppe groesser als bei 6, aber mit einer Obergrenze, damit sie nicht
// riesig werden"): setzt --kc-spalten/--kc-zeilen passend zur TATSAECHLICHEN Artikelzahl der
// aktuellen Seite. Absichtlich zuerst so wenig Zeilen wie moeglich (eine volle Reihe wirkt
// ausgewogener als schmale, gestapelte Tuerme) - erst wenn eine Reihe nicht reicht, kommt eine
// zweite/dritte dazu. Die eigentliche Obergrenze gegen "riesig" liegt in der max-width/-height
// auf der Kachel selbst (siehe styles.css) - unabhaengig von der Spaltenzahl wirksam, auch bei
// nur einem einzigen Artikel.
function passeArtikelgroesseAn(){
  ...
  // Obergrenze der Spaltenzahl kommt aus der gewaehlten Knopfgroesse (alte Ansicht) bzw. einem
  // festen, grosszuegigen Wert (neue Ansicht, die ohnehin per max-height begrenzt).
...
function addConfiguredProduct(p,option){
  if(p?.isPackage){
    ...
    const item={key,id:p.id,packageId:p.id,isPackage:true,name:p.name,price:Number(p.price||0),category:KOMBI_GRUPPE,image:p.image,qty:1,
...
/* Darf diese Bonzeile halbiert werden? Freigabe und Preis kommen aus dem Artikelstamm des
   PC-Managers. Nur wenn beides da ist, erscheint der ½-Knopf in der Zeile - ein Knopf, der
   nur meckert, hat am Stand nichts verloren.

   PFAND WIRD NIE HALBIERT. Halbiert wird ausschliesslich der Artikelpreis (item.price);
   item.deposits bleibt unangetastet. Das Glas ist ganz, egal wie voll es ist - der Gast
   bekommt beim Zurueckbringen die vollen 2,00 EUR. Das gilt in allen drei Pfandregeln:
...
/* Mengenanzeige einer Zeile. Bei halber Portion steht dort 0,5 statt 1 - bei zwei halben
   entsprechend 1 (zwei Halbe sind eine ganze Portion). Es ist eine PORTIONSANGABE, kein
   Faktor auf den Preis: die Zeilensumme ist Anzahl x ½-Preis, nicht Menge x Einzelpreis. */
function mengeAnzeige(item){
  const menge=Number(item.qty||0)*(Number(item.portionFactor||1)===0.5?0.5:1);
  return menge.toLocaleString("de-DE",{minimumFractionDigits:Number.isInteger(menge)?0:1,maximumFractionDigits:1});
}

/* Halbe Portion. Freigabe und Preis kommen aus dem PC-Manager (Artikel -> Verkauf:
   "½ Portion erlaubt" und "½-Preis"). Bewusst ein fester Preis statt der Haelfte des
   Verkaufspreises: 50 % von 5,50 EUR waeren 2,75 EUR, und am Stand wird Wechselgeld
   abgezaehlt. Pfand, Rabatt- und Rueckgabezeilen sind ausgenommen - eine halbe
   Glasrueckgabe gibt es nicht. */
function toggleSelectedHalfPortion(){
  ...
  if(!isHalf&&Number(item.qty||1)>1){
    item.qty-=1;
    const half={...item,key:`${item.id}:${item.option?.id||"base"}:${item.offerId||"normal"}:half:${crypto.randomUUID()}`,
...
function setCartNotice(text,type="info"){lastSettlementNotice=text;setSystemHint(text,type==="success"?"ok":"info")}
...
function showExactCashSettlementNotice({due,received,tip}){
  const card=el("changeDisplay")?.closest(".change-card"),paymentState=el("changePaymentState");
  if(card){card.classList.remove("payment-insufficient","payment-payout");card.classList.add("payment-sufficient","tip-settlement")}
  const title=card?.querySelector(".change-card-head>span");if(title)title.textContent="KEIN RÜCKGELD";
  el("changeDisplay").textContent=money(0);
  el("givenDisplay").textContent=money(received);
  el("dueDisplay").textContent=money(due);
...
// der Personal-Knopf buchte ausnahmslos alles. Diese Funktion liefert die Anzeigenamen aller
// Positionen im aktuellen Bon, die nicht auf Personal gebucht werden duerfen.
// Pakete werden ueber ihre Bestandteile geprueft (ein Paket ist technisch ein eigener Eintrag,
// die einzelnen Bestandteile tragen die Freigabe). Reklamationszeilen sind keine Warenentnahme
// und bleiben aussen vor.
function staffBlockedCartItems(){
  const namen=[];
  const pruefe=(id,anzeigename)=>{
    const product=PRODUCTS.find(item=>item.id===id);
    if(product&&product.allowStaff===false&&!namen.includes(anzeigename))namen.push(anzeigename);
  };
...
// Sichtbarer Zustand des Personal-Knopfes: sobald eine gesperrte Position im Bon liegt, wird der
// Knopf abgeblendet und nicht mehr klickbar - der Grund steht in der Kurzinfo (title).
function updateStaffButtonState(){
  const button=el("staffBtn");if(!button)return;
  const leer=!state.cart.length,isPayout=!leer&&toCents(total())<0;
  // 10.09.2026: bei Auszahlung uebernimmt der Knopf "ALS SPENDE" - die Personal-Sperrliste
  // (allowStaff:false je Artikel) ist in diesem Zustand nicht das Thema, gilt nur noch fuer den
  // normalen Personalverbrauch-Weg.
  const blockiert=isPayout?[]:staffBlockedCartItems();
  button.disabled=blockiert.length>0||leer;
  button.classList.toggle("staff-blocked",blockiert.length>0);
  button.classList.toggle("staff-spende",isPayout);
  button.textContent=isPayout?"♥ ALS SPENDE":"👥 PERSONAL";
  button.title=isPayout?"Pfandbetrag als Spende an den Köcheclub verbuchen (kein Geld auszahlen)":blockiert.length?`Nicht auf Personal buchbar: ${blockiert.join(", ")}`:leer?"Bitte zuerst Artikel wählen":"";
}
...
function updateChange(){
  ...
  if(undo){undo.disabled=!state.cashSelections.length||isPayout;undo.title=isPayout?"Bei Auszahlung ist keine Geldeingabe erforderlich":state.cashSelections.length?`Letzte Geldwahl zurücknehmen (${state.cashSelections.at(-1).label})`:"Noch keine Schein- oder Münzauswahl"}
  updateBarPaymentButton({due,change,hasDue,sufficient,isPayout});
  // User-Wunsch nach echtem Bedienproblem am Stand: "STIMMT SO" wird erst freigegeben, sobald
  // tatsächlich ein Betrag erfasst wurde - vorher ausgegraut und gesperrt, statt nur eine leicht
  // zu übersehende Warnung zu zeigen, wenn ins Leere getippt wird.
  const exactBtn=el("exactCashBtn");
  if(exactBtn){exactBtn.disabled=!hasDue||toCents(state.given)<=0;exactBtn.title=exactBtn.disabled?"Bitte zuerst einen Zahlbetrag erfassen":"Erfassten Betrag als passend verbuchen (kein Rückgeld)"}
  // Der normale Bar-Weg MIT Rückgeld bekommt einen eigenen Knopf direkt neben "STIMMT SO".
  // Vorher gab es an dieser Stelle nur "STIMMT SO", und der Bar-Knopf lag als schmale Säule
  // weit rechts - wer Rückgeld herausgeben wollte, hatte hier keinen sichtbaren Weg.
  // Er erscheint NUR im Rückgeldfall; ohne Geldeingabe bleibt die Zeile unverändert.
  const changeBtn=el("cashChangeBtn"),finishRow=el("cashFinishRow");
  if(changeBtn&&finishRow){
...
function updateBarPaymentButton(snapshot=null){
  ...
  const due=snapshot?.due??total(),isPayout=snapshot?.isPayout??toCents(due)<0,change=snapshot?.change??(isPayout?Math.abs(due):Math.max(0,state.given-due));
  ...
  button.classList.toggle("pay-payout",isPayout);
  button.classList.toggle("pay-direct",!cashEntered&&!isPayout);
  ...
  // 09.09.2026 (Betreiber): "Kunde sagt: kannste behalten als Trinkgeld." Nur bei einer echten
  // Auszahlung (Pfandrückgabe o.ä.) sichtbar - derselbe Zustand, der den BAR-Knopf rot macht.
  ...
  if(isPayout){
    ...
    button.title=`Pfandrückgabe abschließen und ${money(Math.abs(due))} auszahlen`;
  }else if(!cashEntered){
    title.textContent="BAR KASSIEREN";
    detail.textContent="PASSEND ABSCHLIESSEN";
...
function addCashSelection(value,label=money(value)){
  ...
  // Vergroesserte Zahlabwicklung anstossen (siehe kc-zahl-zoom.js) - mit Verzoegerung, damit
  // "eine Muenze und dann Trinkgeld" kein Aufspringen ausloest.
  window.KCZahlZoom?.nachGeldeingabe?.({positionen:state.cart.length,gegeben:state.given});
}
...
function renderCashSoundButton(){
  const button=el("cashSoundBtn");if(!button)return;
  const enabled=window.KCSoundCore?.isEnabled?.()!==false;
  button.classList.toggle("active",enabled);button.classList.toggle("muted",!enabled);
  button.setAttribute("aria-pressed",String(enabled));
  button.setAttribute("aria-label",enabled?"Kassenton eingeschaltet":"Kassenton ausgeschaltet");
  button.title=enabled?"Kassenton ausschalten":"Kassenton einschalten";
  button.innerHTML=`<span aria-hidden="true">${enabled?"🔊":"🔇"}</span>`;
}
function toggleCashSound(){
  if(!window.KCSoundCore)return setSystemHint("SoundCore ist nicht geladen","warn");
  window.KCSoundCore.unlock?.();const enabled=window.KCSoundCore.toggle();renderCashSoundButton();
  if(enabled){window.KCSoundCore.playCashRegister?.();setSystemHint("Kassenton eingeschaltet")}else setSystemHint("Kassenton ausgeschaltet");
}
function playCompletedSaleSound(){
  try{window.KCSoundCore?.playCashRegister?.()}catch(err){console.warn("Kassenton übersprungen",err)}
}
...
// Fragt oder druckt, je nach Einstellung. Bewusst NACH der Abschlussmeldung, damit der
// Bediener zuerst das Rückgeld sieht - das ist im Verkauf das Wichtigere.
function kcBonDruckModus(){
  const r=state.master.receipt||{};
  if(r.printMode)return r.printMode;                 // neue Einstellung
  return r.autoPrint===false?"nie":"fragen";         // alte Einstellung sinngemäß übernehmen
}
function kcBonDruckAnbieten(bonNummer){
  const modus=kcBonDruckModus();
  if(modus==="nie")return;
  if(modus==="immer"){setTimeout(()=>printBonByNumber(bonNummer),350);return;}
  // "fragen": kleine Rückfrage, die den Verkauf nicht aufhält - sie lässt sich mit einem
  // Fingertipp beantworten und verschwindet nach 12 Sekunden von selbst mit "nein".
  const alt=document.getElementById("kcBonFrage");if(alt)alt.remove();
  const box=document.createElement("div");
  box.id="kcBonFrage";box.className="kc-bon-frage";
  box.innerHTML=`<strong>Bon ${bonNummer} drucken?</strong>
    <div><button type="button" data-ja>Ja, drucken</button><button type="button" data-nein>Nein</button></div>`;
...
function renderMoney(){const notes=[5,10,20,50,100,200]; // 08.09.2026 (Betreiber): 200 EUR dazu; assets/schein_200.jpg ist vorerst ein gezeichneter Platzhalter
...
function voidBon(){if(!state.cart.length)return;askConfirm("Offenen Bon verwerfen",`Den noch nicht gebuchten Bon ${bonText()} mit allen Positionen verwerfen?`,()=>{const entry={voidId:crypto.randomUUID(),bon:bonText(),time:new Date().toISOString(),registerId:state.master.registerId,operator:state.master.operatorName,training:!!state.master.trainingMode,reason:"unbooked-cart-discarded",items:cloneData(state.cart)};
  // Sicherheitsebene 4: hash-verkettetes Protokoll, falls verfügbar - läuft im Hintergrund,
  // blockiert die eigentliche (sofortige) Bon-Löschung für den Bediener nicht.
  if(window.KCHashChain){const log=safeArray("kc_voids_v040");window.KCHashChain.appendChained(log,entry).then(()=>localStorage.setItem("kc_voids_v040",JSON.stringify(log)))}else{const log=safeArray("kc_voids_v040");log.push(entry);localStorage.setItem("kc_voids_v040",JSON.stringify(log))}
  state.cart=[];state.cartStartedAt=null;state.lastAdded=null;state.selectedCartKey=null;state.given=0;state.cashSelections=[];state.keypadBuffer="";quantityUndoStack.length=0;updateQuantityUndoButton();renderCart()})}
...
function initializeSettingsPanel(panel){
  try{
    if(panel==="layout"){
      updateLayoutPreview();
      workspaceDraft=workspaceConfig();
      ...
      // Umsatzberichte können bei vielen Buchungen aufwendig sein. Erst im
      // tatsächlich geöffneten Register und außerhalb des Klick-Handlers aufbauen.
      setTimeout(()=>{try{renderReportPreview()}catch(err){console.error("Report preview failed",err);setSystemHint("Berichtsvorschau konnte nicht geladen werden","warn")}},0);
    }else if(panel==="system"){
      renderFiscalInternalStatus();
      const hs=window.KCHealthCore?.getSettings?.()||state.master.healthMonitor||{};
      if(el("healthEnabledSetting"))el("healthEnabledSetting").checked=hs.enabled!==false;
      if(el("healthLevelSetting"))el("healthLevelSetting").value=hs.level||"normal";
      if(el("healthRushProtectionSetting"))el("healthRushProtectionSetting").checked=hs.autoRushProtection!==false;
      renderHealthAdminStatus();
    }
  }catch(err){
    console.error(`Settings panel ${panel} failed`,err);
    window.KCHealthCore?.record?.("settings-panel-error",{panel,message:String(err?.message||err)});
    setSystemHint(`Bereich ${panel} konnte nicht vollständig geladen werden`,"warn");
  }
}
...
function openSettings(){
  ...
  if(el("bonDruckModus"))el("bonDruckModus").value=kcBonDruckModus();
    ...
    window.KCHealthCore?.record?.("settings-open-error",{message:String(err?.message||err)});
    setSystemHint("Stammdaten konnten nicht geöffnet werden","warn");
    try{if(!el("settingsDialog").open)el("settingsDialog").showModal()}catch{}
  }
}
...
async function serviceLogin(e){
  ...
  if(!access?.active||(!access.pinKdf?.hash&&!access.pinHash)){
    registerAdminFailure("pin","Keine aktive Superadmin-PIN eingerichtet. Bitte das aktuelle Freigabepaket importieren.");
    return;
  }
...
document.querySelectorAll("[data-settings-tab]").forEach(btn=>btn.onclick=()=>{
  const panel=btn.dataset.settingsTab;
  ...
  // Das gewählte Register erst nach dem sichtbaren Umschalten initialisieren.
  setTimeout(()=>initializeSettingsPanel(panel),0);
});
...
function groupCommand(cmd){
  ...
  if(cmd==="export")return exportJson("KC_Bildrechner_Warengruppen.json",GROUPS);
...
function loadArticle(index){
  ...
  el("articleIngredients").value=p.info?.ingredients||"";el("articleAllergens").value=(window.KCAllergene?window.KCAllergene.alsText(p.info?.allergens):(p.info?.allergens||""));
...
function articleCommand(cmd){
  ...
  if(cmd==="export")return exportJson("KC_Bildrechner_Artikel.json",PRODUCTS);
...
el("saveTestSettings").onclick=e=>{
  ...
  Object.assign(state.master,{
    ...
    neuesLayout:el("neuesLayoutToggle")?el("neuesLayoutToggle").checked:state.master.neuesLayout,
...
function getArticleExportRows(){
  ...
  const rows=items.map(p=>[
    ...
    p.info?.ingredients||"",(window.KCAllergene?window.KCAllergene.alsText(p.info?.allergens):(p.info?.allergens||"")),p.info?.contents||"",p.info?.important||"",p.info?.notes||""
  ]);
...
function buildSalesReport(){
  ...
  if(type==="articles"){
    ...
    tx.forEach(t=>(t.items||[]).forEach(i=>{
      const key=i.id||i.name,prev=map.get(key)||{name:i.name,qty:0,full:0,half:0,equivalent:0,total:0};
      /* Ganze und halbe Portionen getrennt zaehlen. "Portionsaequivalent" ist die Menge in
         ganzen Portionen (zwei Halbe = eine Ganze) - das ist die Zahl fuer den Einkauf. */
      const qty=Number(i.qty||0),factor=Number(i.portionFactor||1)===0.5?0.5:1;
      prev.qty+=qty;if(factor===0.5)prev.half+=qty;else prev.full+=qty;
      prev.equivalent+=qty*factor;prev.total+=Number(i.price||0)*qty;map.set(key,prev);
    }));
    return {title:"Artikelumsätze",headers:["Artikel","Ganze","½","Portionsäquivalent","Umsatz"],rows:[...map.values()].map(x=>[x.name,x.full,x.half,x.equivalent.toLocaleString("de-DE",{maximumFractionDigits:1}),x.total.toFixed(2)])};
  }
  const map=new Map();
  tx.forEach(t=>(t.items||[]).forEach(i=>{
    const p=PRODUCTS.find(p=>p.id===i.id||p.name===i.name);
    // Buchungen von vor der Umbenennung tragen noch "Packages". Ohne diese Zeile stuenden in
    // der Auswertung ZWEI Warengruppen fuer dieselbe Sache - genau die Art Zahl, die hinterher
    // niemand mehr zusammenbringt.
    let group=p?.category||i.category||"Ohne Warengruppe";
    if(group==="Packages")group=KOMBI_GRUPPE;
...
function normalizeIncomingConfig(raw){
  ...
  return {
    format:"KC_MARKTKASSE_CONFIG",formatVersion:raw.version||1,appVersion:raw.sourceVersion||"externen Zielsystem",
    ...
    master:{
      ...
      neuesLayout:sourceSettings.neuesLayout??state.master.neuesLayout,
      ...
      showCard:sourceSettings.showCard??state.master.showCard,
      showAccount:sourceSettings.showAccount??state.master.showAccount,
      showDiscount:sourceSettings.showDiscount??state.master.showDiscount,
      showHappyHour:sourceSettings.showHappyHour??state.master.showHappyHour,
      showRushMode:sourceSettings.showRushMode??state.master.showRushMode,
      allowTraining:sourceSettings.allowTraining??state.master.allowTraining,
      ...
      categoryOrder:Array.isArray(sourceSettings.categoryOrder)?sourceSettings.categoryOrder:state.master.categoryOrder,
...
function applyConfig(pkg){
  ...
  state.master={...DEFAULTS,...incomingMaster,clubName:safeText(incomingMaster.clubName||DEFAULTS.clubName,100),clubLogo:safeImage(incomingMaster.clubLogo||"assets/kochmuetze-weiss.png"),eventName:safeText(incomingMaster.eventName||DEFAULTS.eventName,100),registerId:safeId(incomingMaster.registerId,DEFAULTS.registerId),registerName:safeText(incomingMaster.registerName||DEFAULTS.registerName,80),operatorName:safeText(incomingMaster.operatorName||DEFAULTS.operatorName,80),receipt:{...DEFAULTS.receipt,...incomingReceipt,head1:safeText(incomingReceipt.head1||DEFAULTS.receipt.head1,120),head2:safeText(incomingReceipt.head2||DEFAULTS.receipt.head2,120),foot1:safeText(incomingReceipt.foot1||DEFAULTS.receipt.foot1,200)}};
...
function sanitizePackage(raw,index=0){
  ...
  return {id:safeId(raw?.id,`PKG-${Date.now()}-${index}`),name:safeText(raw?.name||"Neues Package",80),componentIds:ids,
    price:ids.length?packageSum(ids):Math.max(0,Number(raw?.price||0)),category:KOMBI_GRUPPE,active:raw?.active!==false,autoManaged:!!raw?.autoManaged,
    source:raw?.source==="ai"?"ai":"manual",score:Number(raw?.score||0),imageProductId:safeId(raw?.imageProductId||ids[0]||"",""),
    // VORGABE DES VEREINS (01.09.2026): eine Kombination ist KEIN Rabatt, sondern ein Knopf.
    // Der Preis ist immer die Summe der Bestandteile - nie weniger. Vorher standen in der
    // Warengruppe Kacheln mit unterschiedlichen Preisen fuer dasselbe (11,50 und 11,40), und
    // die feste Kombination war 50 Cent guenstiger als beide Artikel einzeln. Deshalb wird der
    // Preis hier GERECHNET statt gespeichert: er stimmt damit auch dann noch, wenn im Manager
    // spaeter ein Artikelpreis geaendert wird.
    // Artikelnummer: feste Packages stehen in der gemeinsamen Tabelle, taeglich erzeugte
    // bekommen eine Nummer aus dem reservierten Tagesbereich 059xx. Ohne Nummer waere ein
...
function updatePackagePriceHint(){
  ...
  // Der Preis wird nicht mehr eingegeben, sondern aus den Bestandteilen gerechnet.
  if(el("packagePrice")&&ids.length===2){el("packagePrice").value=sum.toFixed(2);el("packagePrice").readOnly=true}
  if(el("packagePriceHint"))el("packagePriceHint").textContent=ids.length<2?"Bitte zwei Artikel auswählen."
    :`Einzelpreise ${money(sum)} · Kombipreis ${money(sum)} — eine Kombination ist ein Knopf, kein Rabatt.`;
}
...
function renderPackageTable(){
  ...
  body.innerHTML=PACKAGES.map(pkg=>{const comps=(pkg.componentIds||[]).map(id=>PRODUCTS.find(p=>p.id===id)?.name||id);const sum=packageSum(pkg.componentIds||[]);
    return `<tr data-package-id="${pkg.id}"><td><strong>${escapeHtml(pkg.name)}</strong></td><td>${escapeHtml(comps.join(" + "))}</td><td>${money(sum)}</td><td>${money(pkg.price)}</td><td class="${pkg.source==="ai"?"package-source-ai":"package-source-manual"}">${pkg.source==="ai"?"KI / Verkaufsdaten":"Zentrale Vorgabe"}</td><td>${pkg.active!==false?"Aktiv":"Inaktiv"}</td></tr>`}).join("");
...
function savePackageFromForm(){
  ...
  if(!name)return showMessage("Pflichtfelder","!","Die Kombination braucht einen Namen.");
...
function rebuildDailyPackages(){
  ...
  // BEFUND: die Tagesvorschlaege schlugen auch Kombinationen vor, die schon FEST angelegt
  // sind. In der Warengruppe standen dann vier Kacheln statt zwei - "Gruenkohl + Gluehwein rot"
  // doppelt und "Gruenkohl + Eierlikoerpunsch" einmal zu 11,50 und einmal zu 11,40 Euro.
  // Derselbe Name mit zwei Preisen sieht vor Publikum nach einem Fehler aus, und am Stand
  // greift der Bediener zur falschen Kachel. Der Vergleich laeuft ueber die Bestandteile,
  // nicht ueber den Namen - die Reihenfolge der beiden Artikel darf keine Rolle spielen.
  const schluessel=ids=>[...ids].sort().join("+");
  const schonVorhanden=new Set(PACKAGES.filter(x=>Array.isArray(x.componentIds)).map(x=>schluessel(x.componentIds)));
  const vorschlaege=PACKAGE_SUGGESTIONS.filter(s=>!schonVorhanden.has(schluessel(s.ids)));
  vorschlaege.slice(0,max).forEach((s,index)=>{const ps=s.ids.map(id=>PRODUCTS.find(p=>p.id===id)).filter(Boolean);if(ps.length!==2)return;
    PACKAGES.push(sanitizePackage({id:`AI-${today}-${index+1}`,name:ps.map(p=>p.name).join(" + "),componentIds:s.ids,price:s.price,active:true,autoManaged:true,source:"ai",score:s.score,imageProductId:s.ids[0],note:`Automatisch für ${new Date().toLocaleDateString("de-DE",{weekday:"long"})} aus Verkaufsdaten zusammengestellt.`},index));
  });
  savePackages();localStorage.setItem(PACKAGE_AI_STATE_KEY,JSON.stringify({lastDailyBuild:today,count:max}));
  renderPackageTable();renderCategories();renderProducts();setSystemHint(`${Math.min(max,vorschlaege.length)} Tages-Kombinationen neu erstellt`+(PACKAGE_SUGGESTIONS.length>vorschlaege.length?` \u00b7 ${PACKAGE_SUGGESTIONS.length-vorschlaege.length} bereits fest angelegt und deshalb \u00fcbersprungen`:""));
}
...
function saveOfferFromForm(){
  ...
  recordAdminChange("offer",before?"update":"create",offer.id,before,offer);setSystemHint("Preisaktion gespeichert");
}
...
function startOfferNow(id,source="central"){
...
function stopOfferNow(id,source="central"){
...
el("offerStartNowBtn")?.addEventListener("click",()=>{const id=el("offerId").value;if(!id)return showMessage("Keine Aktion gewählt","!","Bitte zuerst eine Aktion aus der Liste wählen.");startOfferNow(id,"central")});
el("offerStopNowBtn")?.addEventListener("click",()=>{const id=el("offerId").value;if(!id)return showMessage("Keine Aktion gewählt","!","Bitte zuerst eine Aktion aus der Liste wählen.");stopOfferNow(id,"central")});
el("happyHourQuickBtn")?.addEventListener("click",()=>{const id=el("happyHourQuickBtn").dataset.offerId,offer=OFFERS.find(o=>o.id===id);if(!offer)return;
  ...
  if(!offer.manualStart)return showMessage("Start gesperrt","🔒","Diese Aktion darf nur automatisch oder über eine berechtigte Freigabe gestartet werden.");
...
function posExchangeChecksumPayload(pkg){const c=JSON.parse(JSON.stringify(pkg));if(c.integrity)c.integrity={...c.integrity,value:""};delete c.checksum;delete c.signature;return JSON.stringify(c)}
...
function buildPosAdminChangeSet(){
  ...
  changes.forEach(change=>{if(change.entity==="group")changedGroupIds.add(change.entityId);if(change.entity==="article")changedArticleIds.add(change.entityId)});
  const payload=KCBExchange.cleanConfigPayload({groups:GROUPS.filter(group=>changedGroupIds.has(group.id)),articles:PRODUCTS.filter(article=>changedArticleIds.has(article.id)),settings:settingsChanged?state.master:undefined});
  if(displayProfileChanged)payload.settings={...(payload.settings||{}),displayProfile:{buttonSize:displayProfile.buttonSize,buttonMode:displayProfile.buttonMode,showPrice:displayProfile.showPrice}};
  return sealExchangePackage({schema:"KCB-CONFIG-1",packageId:crypto.randomUUID(),createdAt:new Date().toISOString(),validUntil:new Date(Date.now()+30*86400000).toISOString(),targetId:"EXTERNAL-CONFIG",payload});
}
function validatePosExchange(pkg){
  if(pkg?.format==="KC_EXCHANGE_PACKAGE"){
    const errors=[];if(pkg?.schemaVersion!=="1.0")errors.push("Altes Schema nicht unterstützt");if(posExchangeChecksum(posExchangeChecksumPayload(pkg))!==pkg?.checksum)errors.push("Prüfsumme falsch");const used=JSON.parse(localStorage.getItem(POS_EXCHANGE_USED_KEY)||"[]");if(used.includes(pkg?.packageId))errors.push("Paket bereits importiert");return errors;
  }
  const errors=KCBExchange.validateEnvelope(pkg,["KCB-CONFIG-1","KCB-EVENT-1"]);
  if(pkg?.integrity?.algorithm!=="KCB-CHECK-1")errors.push("Integritätsverfahren nicht unterstützt");
  if(posExchangeChecksum(posExchangeChecksumPayload(pkg))!==pkg?.integrity?.value)errors.push("Prüfsumme falsch");
  const used=JSON.parse(localStorage.getItem(POS_EXCHANGE_USED_KEY)||"[]");if(used.includes(pkg?.packageId))errors.push("Paket bereits importiert");
  return errors;
}
el("exportKCExchangeSales").onclick=async()=>{
  const password=prompt("Übertragungscode für die geschützte Austauschdatei:");
  if(!password||password.length<8)return showMessage("Übertragung abgebrochen","!","Der Übertragungscode muss mindestens 8 Zeichen haben.");
  const pkg=buildPosExchangeSales();
  const encrypted=await encryptObject(pkg,password);
  downloadText(`${state.master.registerId}_Vorgaenge_${pkg.packageId.slice(0,8)}.kce`,JSON.stringify(encrypted),"application/octet-stream");
  el("kcExchangePosStatus").textContent=`Geschützt exportiert: ${pkg.packageId}`;
};
el("exportAdminChanges").onclick=async()=>{
  if(!adminChanges().length)return showMessage("Keine Änderungen","✓","Es liegen keine Vor-Ort-Änderungen für das externe Zielsystem vor.");
  const password=prompt("Übertragungscode für die geschützte Austauschdatei:");
  if(!password||password.length<8)return showMessage("Übertragung abgebrochen","!","Der Übertragungscode muss mindestens 8 Zeichen haben.");
  appendAdminAudit("changeset-export","success",{message:`${adminChanges().length} Änderung(en)`});
  const pkg=buildPosAdminChangeSet();
  const encrypted=await encryptObject(pkg,password);
  downloadText(`${state.master.registerId}_Konfiguration_${new Date().toISOString().slice(0,10)}_${pkg.packageId.slice(0,8)}.kcc`,JSON.stringify(encrypted),"application/octet-stream");
  el("adminChangeCount").textContent=`${adminChanges().length} Änderung(en) geschützt exportiert · Paket ${pkg.packageId.slice(0,8)}`;
};
el("selectKCExchangeImport").onclick=()=>el("importKCExchangeFile").click();
el("importKCExchangeFile").onchange=async e=>{
  const f=e.target.files[0];if(!f)return;
  try{
    let pkg=JSON.parse(await f.text());
    if(pkg?.format==="KC_ENCRYPTED_V1"){
      const password=prompt("Übertragungscode der geschützten Austauschdatei:");
      if(!password)throw new Error("Übertragungscode fehlt");
      try{pkg=await decryptObject(pkg,password)}catch{throw new Error("Übertragungscode falsch oder Datei beschädigt")}
    }
    const errors=validatePosExchange(pkg);if(errors.length)throw new Error(errors.join(" · "));
    let c;
    if(pkg.schema==="KCB-CONFIG-1")c=KCBExchange.cleanConfigPayload(pkg.payload||{});
    else if(pkg?.format==="KC_EXCHANGE_PACKAGE"&&["config","bundle"].includes(pkg.packageType))c=KCBExchange.cleanConfigPayload(pkg.packageType==="bundle"?(pkg.data.config||{}):pkg.data);
    else throw new Error("Nur Konfigurationspakete erlaubt");
    ...
    if(c.settings)state.master={...state.master,...c.settings};if(c.receipt)state.master.receipt={...state.master.receipt,...c.receipt};
    saveGroups();saveProducts();savePackages();saveOffers();saveMaster();
    const used=JSON.parse(localStorage.getItem(POS_EXCHANGE_USED_KEY)||"[]");used.push(pkg.packageId);localStorage.setItem(POS_EXCHANGE_USED_KEY,JSON.stringify(used.slice(-500)));
    renderHeader();renderCategories();renderProducts();renderCart();el("kcExchangePosStatus").textContent=`Importiert: ${pkg.packageId}`;
  }catch(err){el("kcExchangePosStatus").textContent=`Abgelehnt: ${err.message}`}
  e.target.value="";
};

el("exportFullBackup").onclick=async()=>{
  const password=prompt("Passwort für die geschützte Sicherungsdatei:");
  if(!password||password.length<10)return showMessage("Sicherung abgebrochen","!","Das Sicherungspasswort muss mindestens 10 Zeichen haben.");
  const pkg=await buildConfigPackage(true);
  const backup={format:"KCB_BACKUP_1",formatVersion:5,createdAt:new Date().toISOString(),config:pkg,transactions:readTransactions(),trainingTransactions:readTrainingTransactions(),voids:safeArray("kc_voids_v040"),withdrawals:safeArray(WITHDRAWAL_KEY),discountAudit:safeArray("kc_discount_audit_v020"),closings:safeArray(CLOSING_KEY)};
  backup.checksum=await sha256Hex(JSON.stringify(backup));
  const encrypted=await encryptObject(backup,password);
  downloadText(`KC_Bildrechner_Sicherung_${new Date().toISOString().slice(0,10)}.kcbak`,JSON.stringify(encrypted),"application/octet-stream");
};
...
// Geldkassette (scope "split"): der Kassenwart bringt EINE Kassette, ihr Inhalt wurde im Money
// Butler einmal erfasst und auf die beiden Geldladen aufgeteilt. Der Code traegt beide Anteile;
// jede Kasse bucht ausschliesslich ihren eigenen. Dadurch bleibt alles Nachgelagerte
// (Doppelbuchungsschutz, Abschlussuebersicht, Abendzaehlung) unveraendert kassenbezogen.
// Im Code steht aus Platzgruenden NUR der Anteil der ersten Kasse (sonst sprengt die volle
// Stueckelung beider Kassen den QR-Code). Der Anteil der zweiten Kasse ist immer der Rest aus
// der ohnehin mitgelieferten Kassetten-Stueckelung.
function splitAnteilFuer(p,registerId){
  if(!p||p.scope!=="split")return null;
  const teilung=p.split;
  if(!teilung||typeof teilung!=="object")return null;
  if(!Array.isArray(p.registerIds)||!p.registerIds.includes(registerId))return null;
  const erste=registerId===teilung.ersteKasse;
  const anteil={looseBreakdown:{},breakdown:{},coinRolls:{},looseTotal:0,rollTotal:0};
  const loseGesamt=p.looseBreakdown||{},rollenGesamt=p.coinRolls||{};
  for(const schluessel of Object.keys(loseGesamt)){
    const wert=Number(schluessel),gesamt=Math.max(0,Number(loseGesamt[schluessel])||0);
    const beiErster=Math.min(Math.max(0,Number(teilung.lose?.[schluessel])||0),gesamt);
    const anzahl=erste?beiErster:gesamt-beiErster;
    anteil.looseBreakdown[wert]=anzahl;
    anteil.breakdown[wert]=(anteil.breakdown[wert]||0)+anzahl;
    anteil.looseTotal+=anzahl*wert;
  }
  for(const schluessel of Object.keys(rollenGesamt)){
    const wert=Number(schluessel),{rolls:gesamt,coinsPerRoll:muenzenJeRolle}=rollenLesen(wert,rollenGesamt[schluessel]);
    const beiErster=Math.min(Math.max(0,Number(teilung.rollen?.[schluessel])||0),gesamt);
    const anzahl=erste?beiErster:gesamt-beiErster;
    const wertJeRolle=wert*muenzenJeRolle;
    anteil.coinRolls[wert]={rolls:anzahl,coinsPerRoll:muenzenJeRolle,coinCount:anzahl*muenzenJeRolle,valuePerRoll:+wertJeRolle.toFixed(2),total:+(anzahl*wertJeRolle).toFixed(2)};
    anteil.breakdown[wert]=(anteil.breakdown[wert]||0)+anzahl*muenzenJeRolle;
    anteil.rollTotal+=anzahl*wertJeRolle;
  }
  anteil.looseTotal=+anteil.looseTotal.toFixed(2);
  anteil.rollTotal=+anteil.rollTotal.toFixed(2);
  anteil.total=+(anteil.looseTotal+anteil.rollTotal).toFixed(2);
  return anteil;
}
function splitAnteilAuspacken(p,registerId){
  const anteil=splitAnteilFuer(p,registerId);
  if(!anteil)return null;
  const eigen={
    ...p,
    registerId,
    transferId:`${p.transferId}#${registerId}`,
    kassetteTransferId:p.transferId,
    kassetteTotal:p.total,
    kassetteBreakdown:p.breakdown,
    breakdown:anteil.breakdown,
    looseBreakdown:anteil.looseBreakdown,
    coinRolls:anteil.coinRolls,
    looseTotal:anteil.looseTotal,
    rollTotal:anteil.rollTotal,
    total:anteil.total
  };
  delete eigen.split;
  delete eigen.checksum;
  return eigen;
}
function applyCashPayload(text,source="unknown"){
  let p=decodeCashPayload(text);
  ...
  // 12.09.2026 (Betreiber: "checke, ob der QR-Code noch kleiner werden kann - er enthielt zu
  // viele Informationen und liess sich nicht mehr richtig lesen"): ab version 6 steckt die
  // Stückelung als kompaktes Zahlen-Array (lb/cr, feste Reihenfolge - siehe
  // shared/kc-cash-denoms.js) statt als Objekt mit Stückelungswert als Schlüssel. ERST NACH der
  // Prüfsummen-Prüfung wieder in die gewohnte Objekt-Form zurückbauen (nicht schon beim
  // Dekodieren) - sonst würden diese zusätzlichen Felder mit in die Prüfsumme eingerechnet und
  // jeder neue Code faelschlich als "beschädigt" abgelehnt. ECHTER FUND beim ersten Testlauf:
  // genau das ist zunaechst passiert. Aeltere Codes ohne "lb"-Feld bleiben unveraendert lesbar.
  if(Array.isArray(p.lb)&&window.KCCashDenoms){
    p.looseBreakdown=window.KCCashDenoms.entpackeLoose(p.lb);
    p.coinRolls=window.KCCashDenoms.entpackeRollen(p.cr,MUENZEN_JE_ROLLE);
  }
  // 09.09.2026: Money Butler schickt "breakdown" nicht mehr mit (kuerzerer Code, siehe dort) -
  // hier bei Bedarf aus looseBreakdown+coinRolls zurueckrechnen, GENAUSO wie es splitAnteilFuer
  // weiter unten fuer die Geldkassette ohnehin schon tut. Ist "breakdown" (z.B. aus einem
  // aelteren, noch nicht aktualisierten Money Butler) trotzdem vorhanden, bleibt es unberuehrt.
  if(!p.breakdown){
    const rekonstruiert={};
    Object.entries(p.looseBreakdown||{}).forEach(([wert,anzahl])=>{rekonstruiert[wert]=(rekonstruiert[wert]||0)+Number(anzahl||0)});
    Object.entries(p.coinRolls||{}).forEach(([wert,rolle])=>{const {rolls,coinsPerRoll}=rollenLesen(Number(wert),rolle);rekonstruiert[wert]=(rekonstruiert[wert]||0)+rolls*coinsPerRoll});
    p.breakdown=rekonstruiert;
  }
  if(p.scope==="split"){
    const eigenerAnteil=splitAnteilFuer(p,state.master.registerId);
    if(!Array.isArray(p.registerIds)||!p.registerIds.includes(state.master.registerId)||!eigenerAnteil){
      recordCashImportAttempt({time:new Date().toISOString(),result:"rejected-wrong-register",source,transferId:p.transferId||null,expectedRegister:state.master.registerId,suppliedRegister:p.registerIds||null});
      throw new Error(`Diese Geldkassette enthält keinen Anteil für ${state.master.registerId}`);
    }
    const anteilsSumme=+p.registerIds.reduce((summe,kasse)=>summe+Number(splitAnteilFuer(p,kasse)?.total||0),0).toFixed(2);
    if(Math.abs(anteilsSumme-Number(p.total))>.009){
      recordCashImportAttempt({time:new Date().toISOString(),result:"rejected-split-mismatch",source,transferId:p.transferId||null,registerId:state.master.registerId,total:p.total});
      throw new Error(`Die Anteile ergeben zusammen ${money(anteilsSumme)}, die Kassette enthält aber ${money(Number(p.total))} – Code nicht übernehmen`);
    }
    p=splitAnteilAuspacken(p,state.master.registerId);
  }
...
// 09.09.2026: zentrale Anzeige fuer die Bargeldeinzahlung - vom Scan (irgendwo auf der Seite,
// Fenster geht dabei erst auf) genauso genutzt wie vom manuellen Weg im Mehr-Fenster.
function cashDepositZuruecksetzen(){
  el("cashDepositErgebnis").hidden=true;el("cashDepositErgebnis").className="cash-deposit-ergebnis";
  el("cashDepositManuell").hidden=false;
  el("cashDepositPayload").value="";el("cashDepositResult").textContent="";
  el("cashShortCodeInput").value="";el("cashShortCodeResult").textContent="";
}
function zeigeCashErgebnis(text,art,rohcode){
  const dlg=el("cashDepositDialog");
  if(!dlg.open){cashDepositZuruecksetzen();dlg.showModal()}
  const erg=el("cashDepositErgebnis");
  erg.hidden=false;erg.className="cash-deposit-ergebnis "+(art==="ok"?"ok":"fehler");
  erg.innerHTML=(art==="ok"?"✓ ":"⚠ ")+escapeHtml(text);
  // Angenommen: nur das Ergebnis, keine weiteren Knoepfe - Eingabefelder bleiben zu.
  // Abgelehnt: die manuelle Eingabe bleibt/wird sichtbar, der gescannte Text steht schon drin,
  // damit man ihn pruefen oder von Hand korrigieren kann, statt ihn neu abzutippen.
  el("cashDepositManuell").hidden=art==="ok";
  if(art==="fehler"&&rohcode)el("cashDepositPayload").value=rohcode;
}
el("cashDepositDialog").addEventListener("close",cashDepositZuruecksetzen);
el("applyCashDeposit").onclick=()=>{try{const p=applyCashPayload(el("cashDepositPayload").value.trim(),"manual-paste");el("cashDepositResult").textContent=`${p.type==="opening"?"Anfangsbestand":"Nachfüllung"} über ${money(p.total)} wurde für ${p.registerId} am ${displayBusinessDate(p.effectiveDate)} gespeichert.`}catch(err){el("cashDepositResult").textContent="Fehler: "+err.message}};   /* manueller Weg bleibt wie bisher: Ergebnis unter dem Feld, Fenster bleibt offen fuer weitere Versuche */
// Kurzer, von Hand eintippbarer Rückfall-Code (Format Vorgangsart-Kasse-Betrag-Prüfziffer, z.B. "2-1-060000-4").
// Baut daraus einen vollständigen, gültigen Bargeld-Übergabevorgang und leitet ihn durch dieselbe
// geprüfte Übernahmefunktion (applyCashPayload) wie der normale QR-Weg - keine zweite, separate
// Speicherlogik, dieselbe Duplikat-Erkennung und Prüfsummen-Sicherheit.
function applyCashShortCode(eingabe){
  const bereinigt=String(eingabe||"").trim().replace(/\s+/g,"");
  const protokolliereFehler=(grund)=>{recordCashImportAttempt({time:new Date().toISOString(),result:grund,source:"manual-shortcode",transferId:null,registerId:state.master.registerId||null,eingabe:bereinigt});};
  const teile=bereinigt.split("-");
  if(teile.length!==4){protokolliereFehler("rejected-shortcode-format");throw new Error("Format muss Vorgangsart-Kasse-Betrag-Prüfziffer sein, z. B. 2-1-060000-4")}
  const [typZiffer,kasseZiffer,betragCent,pruefzifferEingegeben]=teile;
  if(!/^[12]$/.test(typZiffer)){protokolliereFehler("rejected-shortcode-format");throw new Error("Erste Ziffer muss 1 (Anfangsbestand) oder 2 (Nachfüllung) sein")}
  // Die zweite Ziffer ist die Kassennummer 1-8 (KASSE-01 bis KASSE-08), 9 steht fuer
  // "gemeinsam". Vorher waren nur 1, 2 und 9 vorgesehen UND alles ausser 1 wurde unten als
  // KASSE-02 gebucht - mit einer dritten Kasse waere Geld auf der falschen Kasse gelandet.
  if(!/^[1-9]$/.test(kasseZiffer)){protokolliereFehler("rejected-shortcode-format");throw new Error("Zweite Ziffer muss die Kassennummer (1 bis 8) oder 9 für mehrere gemeinsam sein")}
  if(!/^\d{6}$/.test(betragCent)){protokolliereFehler("rejected-shortcode-format");throw new Error("Betrag muss genau 6 Ziffern haben")}
  if(!/^\d$/.test(pruefzifferEingegeben)){protokolliereFehler("rejected-shortcode-format");throw new Error("Prüfziffer muss eine einzelne Ziffer sein")}
  ...
  const pruefzifferErwartet=quersumme%10;
  if(String(pruefzifferErwartet)!==pruefzifferEingegeben){protokolliereFehler("rejected-shortcode-checksum");throw new Error("Prüfziffer stimmt nicht - bitte den Code noch einmal genau ablesen und eingeben")}
  const total=Number(betragCent)/100;
  if(total<=0){protokolliereFehler("rejected-shortcode-format");throw new Error("Betrag muss größer als 0 sein")}
  const type=typZiffer==="1"?"opening":"topup";
  const heute=localBusinessDate();
  const shortCodeId="KURZCODE-"+ziffern+"-"+heute;
  const payload={
    format:"KC_CASH_TRANSFER",version:4,transferId:shortCodeId,
    time:new Date().toISOString(),effectiveDate:heute,
    breakdown:{[total]:1},looseTotal:total,rollTotal:0,total,
    note:"Per Kurzcode ohne Aufschlüsselung eingegeben"
  };
  ...
  payload.checksum=payloadChecksum(raw);
  const text="KCASH1:"+btoa(unescape(encodeURIComponent(JSON.stringify(payload))));
  return applyCashPayload(text,"manual-shortcode");
}
el("applyCashShortCode").onclick=()=>{try{const p=applyCashShortCode(el("cashShortCodeInput").value);el("cashShortCodeResult").textContent=`${p.type==="opening"?"Anfangsbestand":"Nachfüllung"} über ${money(p.total)} wurde für ${p.registerId||"beide Kassen"} am ${displayBusinessDate(p.effectiveDate)} gespeichert.`}catch(err){el("cashShortCodeResult").textContent="Fehler: "+err.message}};
...
// Die frueheren Hilfsfunktionen drawFinder/drawQrFallback sind entfallen: sie malten bei
// fehlender QR-Bibliothek ein ZUFAELLIGES Muster, das wie ein QR-Code aussah, aber keiner
// war. Am Stand haette man das erst gemerkt, wenn das Einlesen fehlschlaegt. Das gemeinsame
// Modul shared/kc-qr.js zeigt stattdessen ein klar beschriftetes Hinweisbild.
function drawRealQr(canvas,text){
  // Zeichnet ueber das gemeinsame Modul shared/kc-qr.js - siehe dort die Begruendung.
  // Faellt die QR-Bibliothek aus, malt das Modul ein deutlich erkennbares Hinweisbild
  // statt eines huebschen, aber unlesbaren Codes.
  if(!canvas)return;
  const ergebnis=window.KCQrCode?window.KCQrCode.zeichne(canvas,text,260):{ok:false,grund:"QR-Modul fehlt"};
  if(!ergebnis.ok)console.warn("QR-Code konnte nicht erzeugt werden:",ergebnis.grund);
  return ergebnis;
}
function closingSnapshot(){
  ...
  // Personalverbrauch: Ware geht raus, aber KEIN Geld in die Kasse. Wird deshalb bewusst
  // aus allen Bargeld-Summen herausgehalten (siehe cashSales unten) und stattdessen als
...
// deshalb ist der Text ein Hinweis mit Weg, keine Fehlermeldung.
function pflegeAnfangsbestandHinweis(){
  const feld=el("closingOpeningHint");
  if(!feld)return;
  const heute=localBusinessDate();
  const eroeffnungen=safeArray("kc_cash_movements").filter(m=>m.type==="opening"&&m.registerId===state.master.registerId&&cashMovementDate(m)===heute);
  if(!eroeffnungen.length){
    feld.textContent=`Für heute ist an ${state.master.registerName||state.master.registerId} kein Anfangsbestand hinterlegt. Der erwartete Bestand unten ist dadurch zu niedrig. Der Übergabecode des Kassenwarts kann heute noch eingelesen werden.`;
    feld.className="closing-hinweis closing-hinweis-offen";
    feld.hidden=false;
    return;
  }
  const kassette=eroeffnungen.find(m=>m.scope==="split");
  if(kassette){
    feld.textContent=`Anfangsbestand aus der Geldkassette übernommen: ${money(kassette.total)} von ${money(kassette.kassetteTotal||kassette.total)} gesamt. Die andere Kasse muss denselben Code ebenfalls einlesen.`;
    feld.className="closing-hinweis closing-hinweis-info";
    feld.hidden=false;
    return;
  }
  feld.hidden=true;
...
function createClosing(){
  ...
  // Fertigen Abschluss unsichtbar sichern.
  //
  // GENAU HIER und nirgends sonst: erst in diesem Augenblick ist der Abschluss abgeschlossen -
  // der Stand ist eingefroren, die Kennung vergeben, der Code erzeugt. Ein frueher gemeldeter
  // Zwischenstand waere schlimmer als gar keiner, weil er wie eine fertige Tageszahl aussieht
  // und nach dem naechsten Verkauf still falsch wird. Deshalb traegt die Meldung ausdruecklich
  // status:"fertig"; alles ohne diese Kennzeichnung wird auf der Gegenseite verworfen.
  //
  // Nach aussen ist davon nichts zu sehen: kein Fenster, kein Knopf, und die Statuszeile sagt
  // nur "Sicherung" - kein Hinweis darauf, wohin das geht.
  meldeAbschluss(payload);
  setSystemHint(`Tagesabschluss ${payload.closingId.slice(0,8)} gespeichert`);return {payload,code};
}
// Abschluss sichern - still, ueber denselben zuverlaessigen Weg wie jeder Verkauf.
// Klappt es gerade nicht (kein Netz, Companion aus), bleibt der Abschluss auf dem Geraet und
// wird beim naechsten Versuch erneut gesichert. Verloren geht nichts: QR-Code, Datei und
// Ausdruck gibt es unveraendert weiter.
const ABSCHLUSS_GEMELDET_KEY="kc_closings_gemeldet_v1";
async function meldeAbschluss(payload){
  if(!payload?.closingId)return false;
  const ok=await window.KCMeldeweg?.ueberCompanion?.("closing",{...payload,status:"fertig"});
  if(ok){
    const liste=safeArray(ABSCHLUSS_GEMELDET_KEY);
    if(!liste.includes(payload.closingId)){liste.push(payload.closingId);
      localStorage.setItem(ABSCHLUSS_GEMELDET_KEY,JSON.stringify(liste))}
    setSystemHint("Sicherung übertragen");
  }
  return !!ok;
}
// Offene Abschluesse in Ruhe nachsichern - alle zwei Minuten, ohne jede Anzeige.
async function sichereOffeneAbschluesse(){
  const gemeldet=new Set(safeArray(ABSCHLUSS_GEMELDET_KEY));
  const offen=safeArray(CLOSING_KEY).filter(c=>c?.closingId&&!gemeldet.has(c.closingId));
  for(const abschluss of offen.slice(-5))await meldeAbschluss(abschluss);
}
setInterval(sichereOffeneAbschluesse,120000);
setTimeout(sichereOffeneAbschluesse,12000);
window.KCAbschlussSicherung={meldeAbschluss,sichereOffeneAbschluesse,
  // Fuer den Fernbefehl "Abschluss anfordern": gibt es fuer heute schon einen FERTIGEN?
  // Wenn nicht, wird auch keiner gebaut - das bleibt eine bewusste Handlung am Stand.
  letzterAbschlussHeute:()=>{
...
async function recordComplaintRefund(amount){
  await _txHydrated;
  const embeddedTraining=new URLSearchParams(location.search).get("embeddedTraining")==="1";
  if(state.master.trainingMode&&!embeddedTraining)throw new Error("Reale Reklamationsauszahlungen sind im Testmodus gesperrt.");
  ...
  const training=!!state.master.trainingMode,rows=training?readTrainingTransactions():readTransactions(),current=training?`T-REK-${String(rows.length+1).padStart(5,"0")}`:bonText(),endTime=new Date().toISOString(),previousHash=rows[rows.length-1]?.recordHash||null,reference=safeText(el("complaintBonReference").value,40),original=reference?rows.find(row=>String(row.bon||row.bonNumber)===reference||row.transactionId===reference):null,due=-Math.abs(Number(amount));
...
// 09.09.2026 (Betreiber): schneller, EIGENSTAENDIGER Reklamations-Ablauf - Artikel antippen,
// Grund antippen, Ergebnis antippen (Ersatz/Auszahlung/Nichts), fertig. Ruehrt den laufenden
// Warenkorb auf Seite 1 NIE an (anders als das alte addComplaintToCurrentCart), damit sich eine
// Reklamation nie mit dem Bon des naechsten Kunden vermischt. Jedes der drei Ergebnisse bucht
// sofort und einzeln fuer sich; das Protokoll landet im selben Entnahme-Speicher wie bisher
// (WITHDRAWAL_KEY), damit vorhandene Auswertungen es weiter finden - mit einem neuen Feld
// "outcome", das auch Ersatz und Nichts unterscheidbar macht (vorher gab es nur Auszahlung).
...
// BEFUND 01.09.2026, gemessen ueber die WLAN-Adresse: hier stand crypto.subtle.digest.
// Das gibt es nur im "sicheren Kontext" - https oder localhost. Das TABLET ruft die Kasse
// aber ueber http://192.168.178.79:8090 auf, und dort ist crypto.subtle undefined.
//
// Diese Funktion sitzt in completeSale: der Bon bekommt hier seine Pruefsumme, DANACH wird
// gespeichert und die Bonnummer hochgezaehlt. Warf sie, brach der ganze Abschluss ab -
// auf dem Tablet liess sich damit KEIN EINZIGER VERKAUF buchen. Gemessen: Warenkorb voll,
// Bezahlen ausgeloest, Bonnummer blieb auf 123 stehen, null Buchungen gespeichert.
// Am PC lief alles, weil 127.0.0.1 als sicher gilt - deshalb ist es nie aufgefallen.
//
// Jetzt ueber shared/kc-krypto.js: crypto.subtle wenn vorhanden, sonst dieselbe Rechnung in
// JavaScript mit identischem Ergebnis. Betrifft ueber diese eine Funktion auch Bonpruefsummen,
// Stornos, Backup-Pruefsummen und die Ledger-Kontrolle.
async function sha256Hex(text){
  if(window.KCKrypto)return window.KCKrypto.sha256Hex(text);
...
async function runTuvSelfTest({show=false}={}){
  ...
  add("version","Versionsstand",VERSION==="V0.31.3.6 Repair 12"?"pass":"fail",`Geladene Version: ${VERSION}`);
  add("dev-admin-access","Entwicklerzugang deaktiviert",DEV_ADMIN_ACCESS?"warn":"pass",DEV_ADMIN_ACCESS?"Temporärer Candidate-Zugang ist aktiv. Keine Leading-/Release-Freigabe.":"Kein Entwicklerzugang aktiv.");
...
el("downloadTuvReport").onclick=async()=>{
  if(state.role!=="superadmin")return showMessage("Geschützter Export","!","Der Diagnoseexport ist nur im geschützten Bereich verfügbar.");
  await exportEncryptedHealthDiagnostic();
};

function renderHealthAdminStatus(){
  const node=el("healthAdminStatus");if(!node)return;
  const st=window.KCHealthCore?.status?.();
  if(!st){node.textContent="HealthCore nicht geladen.";return}
  node.textContent=`${st.settings.enabled?"Aktiv":"Aus"} · ${st.settings.level} · ${st.eventCount} technische Ereignisse · zuletzt ${st.lastCycle?`${st.lastCycle.elapsedMs} ms`:"noch kein Leerlaufzyklus"}`;
}
function generatedExportCode(){
  const bytes=crypto.getRandomValues(new Uint8Array(12));
  return [...bytes].map(b=>"ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789"[b%57]).join("");
}
async function exportEncryptedHealthDiagnostic(){
  if(state.role!=="superadmin")return showMessage("Nicht erlaubt","!","Diagnoseexport benötigt den geschützten Bereich.");
  const code=el("healthExportCode")?.value||"";
  if(code.length<8)return showMessage("Exportcode fehlt","!","Bitte mindestens 8 Zeichen eingeben oder einen sicheren Code erzeugen.");
  const report=latestTuvReport||await runTuvSelfTest({show:false});
  const payload=window.KCHealthCore.buildDiagnostic({app:"KC_MARKTKASSE_POS",version:VERSION,registerId:state.master.registerId,display:{width:innerWidth,height:innerHeight}},report);
  const encrypted=await window.KCHealthCore.encryptDiagnostic(payload,code);
  downloadText(`${state.master.registerId}_Diagnose_${new Date().toISOString().replace(/[:.]/g,"-")}.kcdiag`,JSON.stringify(encrypted),"application/octet-stream");
  window.KCHealthCore.append("diagnostic-export","info",{format:encrypted.format});appendAdminAudit("diagnostic-export","success",{message:"Verschlüsselte Programmdiagnose"});renderHealthAdminStatus();
  showMessage("Diagnose exportiert","✓","Die Datei ist AES-256-GCM-verschlüsselt. Exportcode getrennt aufbewahren.");
}
el("healthDeepTestBtn")?.addEventListener("click",async()=>{el("tuvDialog").showModal();await runTuvSelfTest({show:true});renderHealthAdminStatus()});
el("healthGenerateCodeBtn")?.addEventListener("click",()=>{const code=generatedExportCode();el("healthExportCode").type="text";el("healthExportCode").value=code;showMessage("Exportcode erzeugt","✓","Code getrennt von der Diagnosedatei sicher aufbewahren.")});
el("healthExportBtn")?.addEventListener("click",()=>exportEncryptedHealthDiagnostic().catch(err=>showMessage("Export fehlgeschlagen","!",err.message)));
el("healthClearLogBtn")?.addEventListener("click",()=>askConfirm("Diagnosehistorie löschen","Nur technische HealthCore-Ereignisse werden gelöscht.",()=>{window.KCHealthCore?.clear?.();renderHealthAdminStatus();appendAdminAudit("diagnostics-clear","success")}));
["healthEnabledSetting","healthLevelSetting","healthRushProtectionSetting"].forEach(id=>el(id)?.addEventListener("change",()=>{window.KCHealthCore?.saveSettings?.({enabled:el("healthEnabledSetting").checked,level:el("healthLevelSetting").value,autoRushProtection:el("healthRushProtectionSetting").checked});renderHealthAdminStatus()}));
...
async function trySuperAdminQr(code){
  ...
  if(!access?.active||!access.hash){
    registerAdminFailure("qr","Kein aktiver Superadmin-QR eingerichtet. Bitte das aktuelle Freigabepaket importieren.",credentialId);return true;
  }
...
// V0.31.3.5: Dialogwechsel im Adminbereich immer erst nach vollständigem
// Schließen des Ausgangsdialogs. Ein unmittelbares showModal() im selben
// Ereigniszyklus konnte insbesondere auf Chromium-Tablets den Backdrop
// festhalten und die Oberfläche scheinbar einfrieren.
function runAfterDialogClosed(dialog,action){
  ...
  const finish=()=>{
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      try{action()}
        ...
        window.KCHealthCore?.record?.("admin-navigation-error",{message:String(err?.message||err)});
        setSystemHint("Adminbereich konnte nicht geöffnet werden","warn");
        try{if(adminSession&&!el("adminHomeDialog").open)el("adminHomeDialog").showModal()}catch{}
      }finally{setTimeout(()=>{adminNavigationInProgress=false},80)}
    }));
  };
  if(dialog?.open){
...
function openAdminTarget(target){
  runAfterDialogClosed(el("adminHomeDialog"),()=>{
    if(target==="reversal"){
      renderRecentBons();
      el("bonSearchResult").innerHTML="";
      el("bonPrintDialog").showModal();
      return;
    }
    openSettings();
    const requested=target==="devices"||target==="settings"?"general":target==="security"?"system":target;
    const tab=document.querySelector(`[data-settings-tab="${requested}"]`);
    if(tab&&requested!=="general")tab.click();
  });
}
document.querySelectorAll("[data-admin-target]").forEach(btn=>btn.onclick=()=>openAdminTarget(btn.dataset.adminTarget));
...
// V0.29.3: stabiler geheimer Zugang auf Maus, Touch und Stift
...
if(PUBLIC_BETA){
  ...
  secret.addEventListener("click",()=>{
    ...
    secretTapReset=setTimeout(()=>secretTapCount=0,4000);
  });
}
...
const CLOSING_STEPS=[
  ...
  {id:"count",label:"Bargeld mit dem Zählsystem erfassen"},
  {id:"compare",label:"Soll-/Ist-Vergleich im Abgleichsystem durchführen"},
...
el("finishWorkflow").onclick=()=>{
  ...
  if(state.workflowMode==="opening")enterKioskFullscreen();
};
el("openingAssistantBtn").onclick=()=>{enterKioskFullscreen();openWorkflow("opening")};
...
function showScreenSaver(reason="manual"){
  if(KC_EMBEDDED_TRAINING)return;
...
function resetScreenSaverTimer(){
  ...
  if(KC_EMBEDDED_TRAINING){
    clearInterval(screenSaverCountdownTimer);
    screenSaverActive=false;
    el("screenSaver")?.setAttribute("hidden","");
    document.body.classList.remove("screen-saver-active");
    return;
  }
...
if(el("screenLockBtn")){
  el("screenLockBtn").hidden=KC_EMBEDDED_TRAINING;
...
// 10.09.2026 (Betreiber: "Fenster nur im Notfall bzw. wenn was unklar ist, was sich aus dem
// gerade gemachten Ablauf nicht erkennen laesst"): der Trinkgeld-Knopf erkennt jetzt zwei
// eindeutige Faelle selbst und bucht SOFORT, ohne Fenster:
// 1) Pfandglas im Warenkorb, Kunde sagt "behalt als Trinkgeld" -> Warenkorb-Betrag ist negativ,
//    genau wie beim eigenen "BEHALTEN ALS TRINKGELD"-Knopf bei Auszahlung.
// 2) Kunde gibt mehr als noetig (Scheine/Muenzen bereits angetippt oder eingetippt), sagt
//    "Rest ist Trinkgeld" -> die Differenz (Ruckgeld) wird direkt als Trinkgeld verbucht.
...
function confirmOperator(profile,source="button"){
  if(!profile)return false;state.master.operatorName=profile.name;state.operatorConfirmedForSale=true;saveMaster();renderHeader();
  // BEFUND: wurde der Ausweis bei GEOEFFNETER Bedienerliste gescannt - also genau so, wie man
  // es tut, wenn man vorher oben links auf die Bedienertaste gedrueckt hat -, wechselte der
  // Bediener im Hintergrund, das Fenster blieb offen und zeigte weiter den ALTEN Namen mit
  // Haken. Wer dann noch einmal tippte, meldete den Vorgaenger wieder an.
  const liste=document.getElementById("operatorDialog");
  if(liste?.open){try{liste.close()}catch{}}
  document.getElementById("operatorBtn")?.classList.remove("needs-confirmation");
  setSystemHint(`Bediener ${profile.name} per ${source==="qr"?"QR":"Taste"} bestätigt`);return true;
}
// BEFUND: der Bediener wurde mit saveMaster dauerhaft gespeichert. Nach einem Neustart der
// Kasse - auch nach einem versehentlichen Neuladen oder am naechsten Markttag - stand oben
// links noch der Bediener vom Vorabend, und jeder Verkauf lief unter SEINEM Namen, bis
// jemand es bemerkte. Vorgabe des Vereins: nach dem Start immer "Team", bis sich jemand
// ausdruecklich anmeldet. Der Ausweis selbst gilt danach unbegrenzt weiter, bis sich ein
// anderer anmeldet - nur ueber den Neustart wird er nicht mitgenommen.
function setzeBedienerAufTeam(){
  const profile=normalizeOperatorProfiles();
  const team=profile.find(x=>x.name==="Team")||profile[0];
  if(!team)return;
  if(state.master.operatorName!==team.name){state.master.operatorName=team.name;saveMaster()}
  state.operatorConfirmedForSale=state.master.requireOperatorConfirmation!==true;
}
// Erkennung der beiden Ausweisarten - bewusst ohne Ruecksicht auf Gross-/Kleinschreibung
// und ohne Abhaengigkeit von einem einzelnen Sonderzeichen (siehe Begruendung am Scanner-
// Empfang weiter unten).
// BEFUND 02.09.2026 (Betreiber): gescannt wurde "KCOPE1::team" - mit ZWEI Doppelpunkten, und
// die Kasse sagte "Ausweis nicht bekannt". Erlaubt war genau EIN Trennzeichen. Woher das zweite
// kommt (Scanner-Vorsatz, doppeltes Etikett, Layout), ist fuer die Kasse unerheblich: an einem
// Trennzeichen mehr oder weniger darf eine Anmeldung am Stand nicht scheitern. Jetzt sind
// beliebig viele Trennzeichen und Leerraeume erlaubt - der Teil DAHINTER entscheidet.
const BEDIENER_VORSATZ=/^KCOPE1[\s:\-_.,;]*/i;
function istBedienerausweis(code){return BEDIENER_VORSATZ.test(String(code||""))}
function istMitgliedsausweis(code){const s=String(code||"");return /^KNG\b/i.test(s)||/(^|[^A-Z0-9])KC-\d{3,5}([^0-9]|$)/i.test(s)}
function mitgliedsnummerAus(code){const t=String(code||"").match(/KC-\d{3,5}/i);return t?t[0].toUpperCase():""}
function operatorFromCode(code){
  const rein=String(code||"").trim().replace(BEDIENER_VORSATZ,"").toLowerCase();
  if(!rein)return null;
  return normalizeOperatorProfiles().find(profile=>String(profile.id||"").toLowerCase()===rein
    ||String(profile.code||"").toLowerCase()===String(code||"").trim().toLowerCase())||null;
}
function operatorFromKngQr(raw){
  // Die Mitgliedsnummer wird aus dem ganzen Code gezogen. Frueher war es das dritte Feld
  // hinter einem "|" - genau das Zeichen, das ein falsch eingestelltes Tastaturlayout zerstoert.
  const memberNo=mitgliedsnummerAus(raw)||String(raw||"").split("|")[2]?.trim()||"";
  if(!memberNo)return null;
  return normalizeOperatorProfiles().find(profile=>String(profile.memberNo||"").toUpperCase()===memberNo.toUpperCase())||null;
}
...
function setKeypadMode(mode){
  ...
  if(mode==="price"&&state.role!=="superadmin"){
    setSystemHint("Preisänderung ist nur mit Serviceberechtigung möglich","warn");mode="cash";
  }
...
function renderKeypadDisplay(){
  ...
  // 10.09.2026 (Betreiber: "10€-Schein angetippt, im Ziffernblock stand trotzdem 0,00€ - sonst
  // hat der Ziffernblock keine Bewandtnis"): vorher zeigte dieses Feld AUSSCHLIESSLICH, was
  // gerade ueber die Zifferntasten getippt wird (state.keypadBuffer) - nach einem Schein-/
  // Muenz-Antippen wird der Puffer bewusst geleert (man tippt ja nichts mehr), wodurch die
  // Anzeige faelschlich auf 0,00€ zurueckfiel, OBWOHL der Betrag laengst korrekt gebucht war
  // (Ruckgeld daneben stimmte die ganze Zeit). Jetzt: ist gerade nichts eingetippt, zeigt das
  // Feld stattdessen den tatsaechlich schon gegebenen Betrag (state.given) - nie mehr eine
  // Null, waehrend im Hintergrund ein echter Betrag steht.
  if(state.keypadMode==="cash"||state.keypadMode==="price"){
    display.textContent=state.keypadBuffer?money(Number.isFinite(value)?value:0):money(state.keypadMode==="cash"?state.given:0);
  }
  ...
  // Waehrend im Bar-Modus getippt wird (vor dem OK), zeigt "Gegeben:"/"Rueckgeld" jetzt live
  // eine Vorschau - rein optisch, bucht NICHTS: der tatsaechliche state.given aendert sich
  // weiterhin erst durch OK. Ohne diese Vorschau sah es beim blossen Tippen so aus, als
...
function zeigeGegebenVorschau(vorschauWert){
  ...
  if(isPayout||!gd||!cd)return;
  const change=Math.max(0,vorschauWert-due);
  gd.textContent=money(vorschauWert);
  cd.textContent=money(change);
  if(ps)ps.textContent=vorschauWert>=due&&toCents(due)>0?`BETRAG AUSREICHEND (Vorschau) · ${money(change)} zurück`:`NOCH ${money(Math.max(0,due-vorschauWert))} FEHLEN (Vorschau)`;
}
function applyKeypadValue(){
  ...
  if(mode==="cash"){
    ...
    // Zahlenblock und Scanner benutzen bewusst dieselbe Suchfunktion, damit eine Nummer nicht
    // auf dem einen Weg gefunden wird und auf dem anderen nicht.
    const product=window.KCArtikelnummern?.findeArtikel(productsForSale(),query)
      ||productsForSale().find(p=>String(p.barcode||"").replace(/\D/g,"")===query||(String(p.id||"").replace(/\D/g,"")!==""&&String(p.id||"").replace(/\D/g,"")===query));
    ...
    const item=selectedCartItem();
    if(state.role!=="superadmin")return setSystemHint("Preisänderung ist nur mit Serviceberechtigung möglich","warn");
    if(!item)return setSystemHint("Bitte zuerst eine Bonposition markieren","warn");
...
// BEFUND: die sechs Umschaltknoepfe des Zahlenblocks (Bargeld, Menge, Rabatt, Artikel, Bon,
// Preis) hatten NIE einen Klick-Handler. Die Funktion setKeypadMode gibt es, sie wurde aber
// nur beim Start einmal aufgerufen - ein Antippen bewirkte nichts. Aufgefallen ist das nie,
// weil die Knoepfe im bisherigen Layout auf Hoehe 0 gedrueckt und damit unsichtbar waren.
// Damit waren Menge und Rabatt ueber den Zahlenblock faktisch nicht benutzbar, obwohl beides
// vollstaendig eingebaut ist.
document.querySelectorAll("[data-keypad-mode]").forEach(button=>button.onclick=()=>setKeypadMode(button.dataset.keypadMode));
el("undoCashBtn").onclick=undoCashSelection;
el("voidBonBtn").onclick=voidBon;

// Neu, rein additiv (User-Wunsch, eingeschränkt): Warenkorb+Rückgeld vergrößert anzeigen, nur
// wenn man das Warenkorb-Symbol antippt ODER die STIMMT-SO-Funktion nutzt. Dazu ein deutlicher
// Schließen-Knopf, um das Fenster schnell wieder zu verlassen. Rührt keine bestehende Logik an.
(function(){
  const overlay=document.createElement("div");
  overlay.id="kcCartExpandOverlay";
  overlay.setAttribute("aria-hidden","true");
  document.body.appendChild(overlay);
  function schliessen(){document.body.classList.remove("kc-cart-expanded");produktSpalte.style.top=""}
  // Kompakte Artikel-Spalte ganz links: nur die aktive Warengruppe, gestapelt, Bild+Preis.
  const produktSpalte=document.createElement("div");
  produktSpalte.id="kcCartExpandProducts";
  document.body.appendChild(produktSpalte);
  // Favoritenleiste unten: die wichtigsten Artikel als kleine Knoepfe fuer schnelles Kassieren.
  const favoritenLeiste=document.createElement("div");
  favoritenLeiste.id="kcCartExpandFavorites";
  document.body.appendChild(favoritenLeiste);

  // BEFUND (echter Fehler, war der Grund fuer die leere Artikelspalte): oeffnen() hat die
  // Spalte gefuellt, BEVOR die Klasse kc-cart-expanded gesetzt war - und genau darauf prueft
  // diese Funktion in ihrer ersten Zeile. Sie ist beim Oeffnen also immer sofort wieder
  // ausgestiegen, die Spalte blieb leer. Deshalb setzt oeffnen() jetzt erst die Klasse.
  function renderKompakteArtikel(){
    if(!document.body.classList.contains("kc-cart-expanded"))return;
    // Dieselbe Auswahl wie im normalen Kassenbild, inklusive Favoriten/Angebote/Happy Hour -
    // vorher filterte diese Spalte nur nach p.category und haette bei diesen drei Reitern
    // grundsaetzlich nichts angezeigt.
    const liste=allProductsForCategory();
    produktSpalte.innerHTML=`<div class="kc-cep-head">${escapeHtml(state.activeCategory)}</div>`+
      `<div class="kc-cep-grid">`+liste.map(p=>
      `<button type="button" class="kc-cep-item" data-id="${p.id}" title="${escapeHtml(p.name)}">
         <img src="${safeImage(p.image)}" alt="">
         <span>${escapeHtml(p.shortName||p.name)}</span>
         <b>${money(p.price)}</b>
       </button>`
    ).join("")+`</div>`;
    produktSpalte.querySelectorAll(".kc-cep-item").forEach(btn=>btn.addEventListener("click",()=>selectProduct(btn.dataset.id)));
    renderFavoritenleiste();
    setzeSpaltenAnfang();
  }
  // Favoriten unabhaengig vom gewaehlten Reiter - sie sollen immer erreichbar sein.
  function favoritenArtikel(){
    return productsForSale().filter(p=>["grot","gweiss","feuer","sauerkrautmett","gruenkohlmett"].includes(p.id)||p.favorite);
  }
  function renderFavoritenleiste(){
    const liste=favoritenArtikel();
    favoritenLeiste.innerHTML=`<span class="kc-cef-titel">★</span>`+liste.map(p=>
      `<button type="button" class="kc-cef-item" data-id="${p.id}" title="${escapeHtml(p.name)}">
         <span>${escapeHtml(p.shortName||p.name)}</span><b>${money(p.price)}</b>
       </button>`).join("");
    favoritenLeiste.querySelectorAll(".kc-cef-item").forEach(btn=>btn.addEventListener("click",()=>selectProduct(btn.dataset.id)));
  }
  // Die Artikelspalte beginnt genau unter der Warengruppen-Zeile.
  //
  // BEFUND: mit einer festen Zahl ging das schief, sobald eine Warengruppe dazukam. Bei
  // Happy Hour sind es zeitweise sieben statt sechs Reiter - die brauchen eine dritte Reihe,
  // und der siebte war dann nur noch durch Scrollen erreichbar. Deshalb wird die tatsaechliche
  // Hoehe gemessen, statt sie zu raten: so passt es auch, wenn spaeter Gruppen dazukommen.
  function setzeSpaltenAnfang(){
    if(!document.body.classList.contains("kc-cart-expanded"))return;
    const reiter=el("categories");
    if(!reiter)return;
    produktSpalte.style.top=`${Math.round(reiter.getBoundingClientRect().bottom)+8}px`;
  }
  window.addEventListener("resize",setzeSpaltenAnfang);
  function oeffnen(){
    document.body.classList.add("kc-cart-expanded");
    renderKompakteArtikel();
    requestAnimationFrame(setzeSpaltenAnfang);
  }
  window.__kcRenderExpandedProducts=renderKompakteArtikel;
  document.querySelector(".cart-heading-icon")?.addEventListener("click",()=>{
    document.body.classList.contains("kc-cart-expanded")?schliessen():oeffnen();
  });
  // FRUEHER stand hier: el("exactCashBtn")?.addEventListener("click",oeffnen,{capture:true});
  // Damit baute sich beim Abschliessen die ganze Ansicht um - gemeint war nur ein Vergroessern
  // des Zahlbereichs. Das macht jetzt kc-zahl-zoom.js, ohne die Anordnung zu wechseln.
  overlay.addEventListener("click",schliessen);
  // Deutlicher Schließen-Knopf oben im vergrößerten Warenkorb-Fenster.
  const closeBtn=document.createElement("button");
  closeBtn.type="button";
  closeBtn.id="kcCartExpandClose";
  closeBtn.setAttribute("aria-label","Vergrößerte Ansicht schließen");
  closeBtn.title="Schließen";
  closeBtn.textContent="✕ SCHLIESSEN";
  closeBtn.addEventListener("click",schliessen);
  document.querySelector(".cart-area")?.appendChild(closeBtn);
})();
...
async function checkoutSale(source="button"){
  ...
  let result;
  if(isPayout){
    result=await completeSale(source==="qr"?"cash-qr-payout":"cash-button-payout",{silent:false,directSettlement:false});
    if(new URLSearchParams(location.search).get("embeddedTraining")==="1")parent.postMessage({type:"KC_TRAINING_SALE_COMPLETED",source,due},"*");
    return result;
  }
  const cashEntered=toCents(state.given)>0;
  if(!cashEntered){
    setGiven(total());state.keypadBuffer="";renderKeypadDisplay();
    result=await completeSale(source==="qr"?"cash-qr-direct":"cash-button-direct",{silent:true,directSettlement:true});
    if(new URLSearchParams(location.search).get("embeddedTraining")==="1")parent.postMessage({type:"KC_TRAINING_SALE_COMPLETED",source,due},"*");
    return result;
  }
  ...
  result=await completeSale(source==="qr"?"cash-qr-change":"cash-button-change",{silent:false,directSettlement:false});
  if(new URLSearchParams(location.search).get("embeddedTraining")==="1")parent.postMessage({type:"KC_TRAINING_SALE_COMPLETED",source,due},"*");
  return result;
}
el("cashSoundBtn").onclick=toggleCashSound;
...
// 10.09.2026 (Betreiber: "kein Platz fuer einen weiteren Knopf - ein anderer, der in dem Moment
// nicht gebraucht wird, soll die Funktion uebernehmen. Ein Kunde bringt ein Pfandglas zurueck
// und sagt als Spende fuer den Koecheclub"): "PERSONAL" ergibt bei einer Auszahlung (Kunde
// bekommt Geld zurueck) ohnehin keinen Sinn - Personalverbrauch heisst ja "Ware geht raus, kein
// Geld fliesst", das Gegenteil einer Rueckzahlung. Genau in diesem Moment (negativer
...
document.querySelectorAll('.more-grid button[data-action]').forEach(button=>button.onclick=()=>{
  ...
  // Wertmarke: der Knopf ist bewusst schon vorhanden, damit der Platz im Bedienablauf steht -
  // gebucht wird aber noch NICHT. Solange nicht entschieden ist, ob eine Teilzahlung moeglich
  // sein soll und ob die Wertmarken selbst ueber die Kasse verkauft werden, ist unklar, wie der
  // Betrag zu verbuchen ist. Ein Knopf, der schon bucht und dabei vielleicht falsch bucht,
  // faellt erst beim Kassensturz auf - deshalb hier ausdruecklich nur ein Hinweis.
  // Das Kontogeruest dafuer liegt bereits (siehe Personalverbrauch: Ware raus, kein Geld in die
  // Kasse, eigene Zeile im Abschluss); es muss spaeter nur noch angeschlossen werden.
  if(action==="wertmarke")return leaveMore(()=>showMessage("Wertmarke noch nicht freigeschaltet",money(total()),
    "Die Bezahlung per Wertmarke ist vorbereitet, aber noch nicht in Betrieb. Bitte den Bon vorerst wie gewohnt abrechnen."));
  // Reklamation aus dem Mehr-Fenster nimmt denselben Weg wie die Sondertaste.
  if(action==="reklamation")return leaveMore(()=>el("complaintBtn").click());
...
document.addEventListener("keydown",async e=>{if(["INPUT","TEXTAREA","SELECT"].includes(document.activeElement.tagName))return;clearTimeout(state.scanTimer);if(e.key==="Enter"){const code=state.scanBuffer.trim();state.scanBuffer="";
    // BEFUND: JEDE Meldung, die aus einem Scan entstand, war unsichtbar. Der Scanner schickt
    // hinter dem Code ein Enter; dieselbe Enter-Taste hat den Hinweis, den sie gerade
    // ausgeloest hat, im selben Moment wieder weggeklickt (die Fenster tragen
    // <form method="dialog"> mit einem Standardknopf, den Enter ausloest). Ein unbekannter
    // Ausweis, eine unbekannte Artikelnummer, ein abgelehnter Geld-QR - alles blieb stumm,
    // und am Stand sah es aus, als sei der Scan gar nicht angekommen.
    if(code)e.preventDefault();
    // TASTATURLAYOUT (Beleg 01.09.2026: "Gelesen wurde: ß??"). Ein Scanner ist eine Tastatur;
    // steht er amerikanisch und das Tablet deutsch, wird aus "-" ein "ß" und aus "_" ein "?".
    // Ziffern sind davon nie betroffen - deshalb gingen Artikelnummern und Ausweise nicht.
    // Die Vertauschung ist eine feste Tabelle, also rechenbar: passt der gelesene Code nirgends,
    // wird die zurueckgerechnete Schreibweise probiert. Entschieden wird weiterhin allein an den
    // Listen DIESER Kasse - geraten wird nichts.
    const schreibweisen=window.KCTastaturlayout?window.KCTastaturlayout.schreibweisen(code):[code];
    let layoutRepariert=false;
    // Nimmt die erste Schreibweise, mit der `pruefe` etwas findet. Unveraendert schlaegt immer.
    const ersterTreffer=pruefe=>{
      for(let i=0;i<schreibweisen.length;i+=1){
        let wert=null;
        try{wert=pruefe(schreibweisen[i])}catch(err){wert=null}
        if(wert){if(i>0)layoutRepariert=true;return{wert,code:schreibweisen[i]}}
      }
      return null;
    };
    // Die Kasse arbeitet normal weiter und sagt EINMAL dazu, was am Scanner zu tun ist.
    const layoutHinweis=()=>{if(layoutRepariert)setSystemHint("Scanner steht auf amerikanischer Tastatur - Seite 12 im Heft, Code \u201eGerman Keyboard\u201c scannen. Die Kasse rechnet es bis dahin selbst zurueck.")};
    if(ersterTreffer(c=>/^CMD-CHECKOUT$/i.test(c))){layoutHinweis();return checkoutSale("qr")}
    // 09.09.2026 (Betreiber): "Abends nicht durch Klick und PIN lange Muehe haben." Eine eigene,
    // ausgedruckte QR-Karte oeffnet DIREKT den Tagesabschluss - ohne Warenkorb-Halten, ohne PIN.
    // Bewusst ohne Schutz: das Fenster zeigt nur Zahlen und erzeugt Codes, es bewegt kein Geld
    // und veraendert nichts. Dieselbe Karte funktioniert an jeder Kasse (kein registerId noetig -
    // der Abschluss gilt ohnehin nur fuer das Geraet, das ihn oeffnet).
    if(ersterTreffer(c=>/^KCTAGESABSCHLUSS$/i.test(c))){layoutHinweis();openClosingDialog();return}
    // Ein Handscanner ist eine TASTATUR. Steht sein Tastaturlayout auf einer anderen Sprache
    // als das Tablet, kommen Ziffern zwar richtig an, Sonderzeichen aber nicht: aus "|" wird
    // je nach Layout ein anderes Zeichen, aus ":" ebenso. Manche Scanner koennen ausserdem
    // alles in GROSSBUCHSTABEN senden, und ein eingeschaltetes Caps Lock am Tablet tut
    // dasselbe. Deshalb wird ein Ausweis nicht mehr buchstabengetreu verglichen: erkannt wird
    // er an seinem Anfang ohne Ruecksicht auf Gross- und Kleinschreibung, und die
    ...
    if(code.startsWith("KCASH1:")){
      ...
      // Bestätigungsknöpfe, der Kassenwart soll nur sehen: angenommen. Geht der Code nicht,
      // muss dasselbe Fenster die manuelle Eingabe ermöglichen." Vorher lief das nur als
      // kleine Statuszeile im Hintergrund; jetzt öffnet der Scan selbst das Fenster.
      try{
        const p=applyCashPayload(code,"hid-scanner");
        zeigeCashErgebnis(`${p.type==="opening"?"Anfangsbestand":"Nachfüllung"} über ${money(p.total)} für ${displayBusinessDate(p.effectiveDate)} übernommen.`,"ok");
      }catch(err){
        zeigeCashErgebnis(err.message,"fehler",code);
      }
      return;
    }    // BEFUND: hier stand nur eine fest verdrahtete Liste aus fuenf Demo-Codes. Ein echtes
    // Etikett mit der Artikelnummer wurde vom Scanner zwar gelesen, aber von der Kasse
    // ignoriert - ohne jede Meldung. Jetzt wird zuerst der Artikelstamm ueber die
    // Artikelnummer durchsucht (shared/kc-artikelnummern-core.js), die alten Demo-Codes
    // bleiben als Rueckfall erhalten, und ein unbekannter Code sagt das auch.
    const map={ART0001:"mettwurst",ART1001:"grot",ART1002:"gweiss",ART1003:"feuer",DEP0001:"glasplus"};
    const alt=ersterTreffer(c=>map[c]||null);
    if(alt){selectProduct(alt.wert);layoutHinweis();return}
    const treffer=ersterTreffer(c=>window.KCArtikelnummern?.findeArtikel(productsForSale(),c)||null);
    if(treffer){const g=treffer.wert;selectProduct(g.id);
      setSystemHint(`${g.name} gescannt (${g.barcode||g.id})`);layoutHinweis();return}
    // Ein unbekannter Code sagt jetzt IMMER etwas - und nennt dabei, was tatsaechlich
    // angekommen ist. Das ist die schnellste Selbstauskunft bei einem falsch eingestellten
    // Scanner: steht dort Buchstabensalat statt der Nummer, stimmt sein Tastaturlayout nicht.
    if(code.length>=3){
      // Sieht der gelesene Text nach vertauschtem Layout aus, wird das GESAGT statt angedeutet -
      // samt der Seite im Heft. Sonst bleibt es beim allgemeinen Hinweis.
      const nachLayout=window.KCTastaturlayout?.riechtNachLayout(code);
      showMessage("Code nicht erkannt","?",`Zu diesem Code gibt es hier nichts.\n\nGelesen wurde: ${code}\n\n`
        +(nachLayout
          ?"Die gelesenen Zeichen deuten auf ein falsches Tastaturlayout am Scanner: er sendet amerikanisch, das Ger\u00e4t ist deutsch. Zu tun: im Scanner-Heft auf Seite 12 den Code \u201eGerman Keyboard\u201c scannen."
          :"Steht dort etwas anderes als auf dem Etikett, ist am Scanner das falsche Tastaturlayout eingestellt."));
    }
    return}if(e.key.length===1)state.scanBuffer+=(e.key);
  // Der Sammelspeicher wartet auf das naechste Zeichen. Grund: der HW0010 kennt vier
  // Uebertragungsgeschwindigkeiten (High/Medium/Low/Super Low). Wer wegen fehlender Zeichen
  // auf eine langsamere Stufe geht - genau das empfiehlt sein Handbuch -, haette sonst mitten
  // im Code einen Abbruch gehabt: die Kasse haette den Rest als neuen Code gelesen.
  // 10.09.2026 (Betreiber, echter Test mit dem Money-Butler-Geldübergabe-Code: "wird
  // scheinbar gelesen, aber mit Verzoegerung - dann JSON Parse error: Expected '}'"): genau
  // das Muster eines mitten im Scan abgeschnittenen Codes - der Puffer wurde geleert, bevor
  // der Scanner fertig war, JSON.parse bekam dadurch nur einen Bruchteil des Codes. Vorher
  // 900 ms, jetzt grosszuegiger auf 1800 ms angehoben - bleibt fuer eine echte Handeingabe
  // (niemals ununterbrochen hunderte Zeichen ohne Fokus auf ein Eingabefeld) weiterhin um
  // Groessenordnungen zu kurz, um versehentlich als Scan zu gelten.
  state.scanTimer=setTimeout(()=>state.scanBuffer="",1800)});
...
const KC_TRAINING_ACTION_LOG=[];
if(new URLSearchParams(location.search).get("embeddedTraining")==="1"){state.master.trainingMode=true;state.master.rushMode=false;saveMaster();applyModes()}
if(new URLSearchParams(location.search).get("embeddedTraining")==="1")document.addEventListener("click",event=>{
  const node=event.target?.closest?.("button,[data-cat],[data-cart-qty],[data-a],[data-action],[data-withdraw-reason],[data-complaint-reason]");if(!node)return;
  KC_TRAINING_ACTION_LOG.push({time:Date.now(),id:node.id||"",action:node.dataset.action||node.dataset.a||"",cartQty:node.dataset.cartQty||"",category:node.dataset.cat||"",withdrawReason:node.dataset.withdrawReason||"",complaintReason:node.dataset.complaintReason||"",text:String(node.textContent||"").trim().replace(/\s+/g," ").slice(0,80),classes:node.className||""});
  if(KC_TRAINING_ACTION_LOG.length>160)KC_TRAINING_ACTION_LOG.splice(0,KC_TRAINING_ACTION_LOG.length-160);
},true);
window.KCTrainingAPI={
  ...
  getTrainingState(){
    const transactions=readTrainingTransactions(),withdrawals=safeArray(WITHDRAWAL_KEY),voids=safeArray("kc_voids_v040"),tips=tipRecords();
    return {items:cloneData(state.cart),total:total(),given:Number(state.given||0),activeCategory:state.activeCategory,trainingMode:!!state.master.trainingMode,rushMode:!!state.master.rushMode,selectedCartKey:state.selectedCartKey,openDialogs:[...document.querySelectorAll("dialog[open]")].map(x=>x.id),actions:cloneData(KC_TRAINING_ACTION_LOG),transactionCount:transactions.length,lastTransaction:cloneData(transactions.at(-1)||null),withdrawalCount:withdrawals.length,lastWithdrawal:cloneData(withdrawals.at(-1)||null),voidCount:voids.length,lastVoid:cloneData(voids.at(-1)||null),tipCount:tips.length,lastTip:cloneData(tips.at(-1)||null)};
  },
  resetTrainingTask(){state.cart=[];state.cartStartedAt=null;state.lastAdded=null;state.selectedCartKey=null;state.given=0;state.cashSelections=[];state.keypadBuffer="";KC_TRAINING_ACTION_LOG.length=0;document.querySelectorAll("dialog[open]").forEach(d=>{try{d.close()}catch{}});renderMoney();renderCart();return true},
  closeAllDialogs(){
    document.querySelectorAll('dialog[open]').forEach(d=>{try{d.close()}catch{}});
    return true;
  },
...
/* CurrencyCore V0.29.3 – ECB reference rates, local cache and history. */
...
async function enterKioskFullscreen(){
  ...
  // 09.09.2026 (Betreiber am iPad: "wenn ich Vollbild verlasse, scannt der Scanner - sobald ich
  // wieder reingehe, geht kein Scan mehr"): Der echte Browser-Vollbildmodus blockiert auf diesem
  // iPad die Weitergabe der Bluetooth-Scanner-Tastatur an die Seite - live durchgetestet: mit
  // Vollbild aus landeten die gescannten Artikel im Warenkorb, mit Vollbild an nicht mehr. Ein
  // Scanner, der nicht scannt, wiegt schwerer als eine sichtbare Adresszeile - deshalb wird der
  // echte requestFullscreen() jetzt NIE mehr aufgerufen, unabhaengig vom Geraet. Der "KASSE
  // STARTEN"-Knopf und sein Starttor bleiben unveraendert (bewusster erster Tipp bleibt
  // erhalten), nur ohne die Vollbildfunktion dahinter, die den Scanner blockierte. Die alte
  // Annahme "iPad-Safari kann ohnehin kein echtes Vollbild" stimmte auf diesem Geraet nicht -
  // es konnte es sehr wohl, und genau das war das Problem.
  try{const kannVollbild=false;const target=document.documentElement;if(!document.fullscreenElement&&kannVollbild)await target.requestFullscreen({navigationUI:"hide"});kioskFullscreenActive=!!document.fullscreenElement||document.body.classList.contains("fullscreen-active")||!kannVollbild;if(!kannVollbild)document.body.classList.add("fullscreen-active");syncViewportHeight();syncFullscreenGate();setSystemHint(kioskFullscreenActive||runsAsInstalledApp()?"Kasse gestartet":"Vollbild konnte vom Browser nicht gestartet werden",kioskFullscreenActive||runsAsInstalledApp()?"ok":"warn")}catch(error){kioskFullscreenActive=false;syncFullscreenGate();setSystemHint("Vollbild ist auf diesem Gerät nur über die installierte App/PWA verfügbar","warn")}
}
async function leaveKioskFullscreen(){try{if(document.fullscreenElement&&document.exitFullscreen)await document.exitFullscreen();syncViewportHeight();setSystemHint("Vollbildmodus beendet")}catch(error){setSystemHint("Vollbild konnte nicht beendet werden","warn")}}
function openExitMenu(){const dialog=el("exitDialog");if(!dialog)return;el("exitOpenBonWarning").hidden=!state.cart.length;dialog.showModal()}
function finishApplicationSession(){const finish=()=>{try{sessionStorage.clear()}catch{};if(document.fullscreenElement&&document.exitFullscreen)document.exitFullscreen().catch(()=>{});document.body.classList.add("application-ended");el("exitDialog")?.close();el("endedScreen")?.removeAttribute("hidden")};if(state.cart.length)return askConfirm("Kasse mit offenem Bon beenden","Der offene Bon bleibt unverändert erhalten. Kasse wirklich verlassen?",finish);finish()}
function confirmHeaderExit(){
  const warning=state.cart.length?" Ein offener Bon bleibt gespeichert und wird nicht abgerechnet.":"";
  askConfirm("Programm verlassen",`Wollen Sie das Programm wirklich verlassen?${warning}`,()=>{
    try{sessionStorage.clear()}catch{}
    if(document.fullscreenElement&&document.exitFullscreen)document.exitFullscreen().catch(()=>{});
    document.body.classList.add("application-ended");
...
if(KC_EMBEDDED_TRAINING){
  document.body.classList.add("embedded-training");
  ["fullscreenGate","enterFullscreenBtn","leaveFullscreenBtn"].forEach(id=>{const node=el(id);if(node){node.hidden=true;node.setAttribute("aria-hidden","true")}});
}
syncViewportHeight();syncFullscreenGate();
const renderCartWithDensity=renderCart;renderCart=function(){const list=el("cartList");if(list){list.classList.toggle("cart-density-medium",state.cart.length>=5);list.classList.toggle("cart-density-compact",state.cart.length>=8)}return renderCartWithDensity()};
window.KCMessageCore?.init?.();normalizeOperatorProfiles();ensureBedienerstamm();setzeBedienerAufTeam();normalizeDisplayProfile();initializeSmartPackages();initializeOfferCore();renderHeader();renderCashSoundButton();setKeypadMode("cash");renderCategories();renderProducts();renderMoney();renderCart();renderAdminChangeStatus();drawRealQr(el("checkoutQrMini"),"CMD-CHECKOUT");setTimeout(()=>drawRealQr(el("checkoutQrMini"),"CMD-CHECKOUT"),120);window.addEventListener("load",()=>drawRealQr(el("checkoutQrMini"),"CMD-CHECKOUT"));tick();setInterval(tick,1000);setInterval(()=>refreshOfferRuntime(false),30000);
setTimeout(()=>runTuvSelfTest().catch(err=>console.error("Selbsttest fehlgeschlagen",err)),900);
window.KCHealthCore?.start?.({probes:{
  isSaleActive:()=>state.cart.length>0,
...
(function initKassenCoach(){
  ...
  const steps=[
    {title:"Willkommen zur Bildrechner-Schulung",text:"Sie üben jetzt einen vollständigen Verkauf. Alle Buchungen bleiben im Trainingsspeicher und zählen nicht als echter Umsatz.",target:"#trainingBanner"},
...
// AccountChargeCore + SharedCash extension V0.29.3
...
// 08.09.2026 (Betreiber): "Richte die Konten Stadtmarketing Werne und Bauhof Werne ein."
// Vorher stand hier nur Stadtmarketing, gueltig erst ab 01.12. und mit der Gruppe "Essen" -
// die Warengruppe heisst aber "Speisen", damit war an der Kasse kein Artikel kontofaehig.
// Jetzt: beide Konten, gueltig ab sofort bis Ende Januar, Speisen und Getraenke erlaubt,
// Hoechstbetrag 500 EUR. Ein vorhandener Kontenstand wird nicht ueberschrieben, nur fehlende
// Konten werden ergaenzt (Admin-Bereich -> Kontenverwaltung bleibt der Ort zum Aendern).
const KC_KONTEN_STANDARD=[
 {id:"ACC-STADTMARKETING",name:"Stadtmarketing Werne",status:"active",validFrom:"2026-09-01",validUntil:"2027-01-31",limit:500,allowedGroups:["Speisen","Getränke"],allowProducts:[],blockProducts:[],version:2},
 {id:"ACC-BAUHOF",name:"Bauhof Werne",status:"active",validFrom:"2026-09-01",validUntil:"2027-01-31",limit:500,allowedGroups:["Speisen","Getränke"],allowProducts:[],blockProducts:[],version:1},
 // 10.09.2026 (Betreiber: "Testfirma anlegen, damit das auf Konto klappt" - fuer die Schulung
 // auf ausgeliehenen Tablets ohne Manager-Anbindung): ein eigenes, klar als Uebung
 // gekennzeichnetes Konto, an dem Trainees die KONTO-Buchung ausprobieren koennen, ohne
 // versehentlich das echte Limit von Bauhof/Stadtmarketing zu verbrauchen. Alle Warengruppen
 // freigegeben (Trainees sollen mit JEDEM Artikel ueben koennen) und ein grosszuegiges Limit,
 // das fuer eine ganze Schulung reicht, ohne dass "Limit erreicht" den Ablauf stoert.
 {id:"ACC-TESTFIRMA",name:"Testfirma (nur Übung)",status:"active",validFrom:"2020-01-01",validUntil:"2099-12-31",limit:99999,allowedGroups:["Speisen","Getränke","Pfand","Favoriten","Kombi","Sonstiges"],allowProducts:[],blockProducts:[],version:1}
];
function kcSeedAccounts(){
  const rows=kcRead(KC_ACCOUNT_KEY,null);
  if(!rows){kcWrite(KC_ACCOUNT_KEY,KC_KONTEN_STANDARD.map(a=>({...a,approvedAt:new Date().toISOString()})));return}
  let geaendert=false;
  KC_KONTEN_STANDARD.forEach(st=>{
    const i=rows.findIndex(x=>x.id===st.id);
    if(i<0){rows.push({...st,approvedAt:new Date().toISOString()});geaendert=true;return}
    // Der alte Stadtmarketing-Eintrag (Version 1) war unbrauchbar - einmalig auf Version 2 heben.
    if(st.id==="ACC-STADTMARKETING"&&Number(rows[i].version||1)<2){rows[i]={...rows[i],...st,approvedAt:rows[i].approvedAt||new Date().toISOString()};geaendert=true}
  });
  if(geaendert)kcWrite(KC_ACCOUNT_KEY,rows);
}
...
// 10.09.2026: Trainingsbuchungen (siehe kcPostAccount) zaehlen bewusst NICHT gegen das echte,
// lokal ungemeldete Guthaben - sonst wuerde jede Uebungsbuchung das reale Limit verbrauchen.
...
function kcRuleCheck(a,item){
  ...
  // 10.09.2026 (Betreiber: "Glas kommt zurück, soll als Gutschrift aufs selbe Konto - das muss
  // die Kasse annehmen"): eine RUeCKGABE (negativer Preis, z.B. Pfandrueckgabe) ist keine neue
  // Belastung, sondern gibt Geld zurueck, das genau fuer diese Warengruppe zuvor schon
  // gebucht wurde. Die Warengruppen-Freigabe (allowedGroups) soll nur NEUE Belastungen
  // einschraenken, nicht das Zurueckbuchen selbst. Deshalb: negative Positionen ueberspringen
  // diese Pruefung grundsaetzlich - gesperrte Artikel (blockProducts, oben) bleiben trotzdem
  // gesperrt, das ist ein bewusstes Verbot, kein Warengruppen-Filter.
  if(item.price<0)return {ok:true};
...
async function kcPostAccount(){
  ...
  // Trainingskennzeichen). Ab jetzt wird die Buchung markiert; die Summenbildung schliesst sie
  // unten explizit aus - echte Konten bleiben unangetastet, ganz gleich wie oft im Testmodus
  // geuebt wird.
  ...
  el("accountChargeDialog").close();
  // 10.09.2026: dieselbe Meldungszeile statt Fenster - eine Gutschrift (negativer Betrag, z.B.
...
function kcRenderAccountControl(){
  ...
  el("amAccounts").querySelectorAll("[data-edit-account]").forEach(b=>b.onclick=()=>kcLoadAccountControl(b.dataset.editAccount));
...
function kcLoadAccountControl(id){
...
function kcSaveAccountControl(){
  ...
  if(old)rows[rows.findIndex(x=>x.id===id)]=a;else rows.push(a);kcWrite(KC_ACCOUNT_KEY,rows);recordAdminChange("account",old?"update":"create",id,old||null,a);kcRenderAccountControl();setSystemHint("Konto freigegeben und für Synchronisation vorgemerkt");
}
function kcCreateInvoice(){
  ...
  inv.push({invoiceId,invoiceNo,accountId,accountName:a.name,from,until,total:sum,status:"issued",createdAt:new Date().toISOString(),eventIds:selected.map(e=>e.eventId)});kcWrite(KC_ACCOUNT_INVOICES_KEY,inv);kcRenderAccountControl();el("invoiceResult").textContent=`${invoiceNo} über ${money(sum)} erzeugt. ${selected.length} Buchungen gemeinsam in Rechnung gestellt.`;
}
...
// Ein Mehrbetrag wird als Trinkgeld verbucht; es wird kein Rückgeld ausgegeben.
el("exactCashBtn")?.addEventListener("click",async()=>{
  const due=total(),received=state.given;
  if(!state.cart.length)return setSystemHint("Bitte zuerst Artikel wählen","warn");
  if(toCents(due)<=0)return setSystemHint("Kein positiver Zahlbetrag vorhanden","warn");
  if(toCents(received)<=0)return setSystemHint("Bitte zuerst einen Zahlbetrag erfassen","warn");
  if(toCents(received)<toCents(due))return setSystemHint(`Zahlbetrag reicht nicht aus · noch ${money(due-received)} fehlen`,"warn");
  const tip=fromCents(Math.max(0,toCents(received)-toCents(due)));
  state.keypadBuffer="";renderKeypadDisplay();
...
el("accountControlOpen")?.addEventListener("click",()=>runAfterDialogClosed(el("adminHomeDialog"),()=>{kcRenderAccountControl();el("accountControlDialog").showModal()}));
el("sharedCashControlOpen")?.addEventListener("click",()=>runAfterDialogClosed(el("adminHomeDialog"),()=>{el("sharedCashDate").value=localBusinessDate();el("sharedCashControlDialog").showModal()}));
el("amSave")?.addEventListener("click",kcSaveAccountControl);
...
// SoundCore darf erst nach einer Benutzerinteraktion einen AudioContext aktivieren.
document.addEventListener("pointerdown",()=>window.KCSoundCore?.unlock?.(),{once:true,passive:true});
document.addEventListener("keydown",()=>window.KCSoundCore?.unlock?.(),{once:true});
