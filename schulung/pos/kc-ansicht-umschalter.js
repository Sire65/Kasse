...
(function (global) {
  ...
  function anwenden(neuerIndex, merken) {
    ...
    // app.js zeichnet die Artikelkacheln beim allerersten Laden BEVOR dieses Modul hier ueberhaupt
    // laeuft (siehe applyVisibility() -> renderProducts() in derselben Zeile) - zu dem Zeitpunkt
    // steht kc-layout-neu noch auf "false" (Vorgabewert). Schaltet diese Funktion es gleich
    // danach auf "true" (der Start-Zustand "Normal"), aendert sich zwar die KLASSE, aber die
    // bereits gezeichneten Kacheln blieben stehen - mit der ALTEN, fest verdrahteten Anzahl pro
    // Seite. Ohne diesen Nachzieher zeigte die Kasse deshalb beim Start durchgehend zu wenige
    // Artikel und eine Blaetterleiste, die eigentlich gar nicht noetig waere.
    if (typeof global.renderProducts === 'function') { try { global.renderProducts(); } catch (e) {} }
  }
