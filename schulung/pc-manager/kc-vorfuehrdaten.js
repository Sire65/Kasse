...
(function (global) {
  ...
  // NUR in dieser eigenständigen Schulungs-/Vorführkopie (schulung/pc-manager): anders als
  // im echten, produktiven PC-Manager oben beschrieben, gibt es hier nie einen "echten
  // Markttag", der versehentlich mit Vorführdaten vermischt werden könnte - diese Kopie
  // dient ausschließlich Vorführungen. Deshalb hier einmalig automatisch laden, damit die
  // Kopie direkt nach dem Öffnen ohne einen Klick voller Zahlen ist.
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { if (!localStorage.getItem(MARKE)) laden(); });
  } else if (!localStorage.getItem(MARKE)) {
    laden();
  }
})(window);
