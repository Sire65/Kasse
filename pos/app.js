...
const DEFAULT_PRODUCTS=[
 {id:"schussrum",sortOrder:9000,name:"Schuss Rum",price:1,category:"Getränke",image:"assets/rum_version_3.png"},
 {id:"schussamaretto",sortOrder:9001,name:"Schuss Amaretto",price:1,category:"Getränke",image:"assets/amaretto_version_3.png"},
...
function setSystemHint(text,type="ok"){
  if(window.KCMessageCore?.add){window.KCMessageCore.add(text,type);return;}
...
const KATEGORIE_BILDER={
  ...
  "Pfand":'<path d="M8.2 3.4 5 8.8M13 8.8 9.8 3.4"/><rect x="3" y="11.8" width="18" height="8.8" rx="1.7"/><path d="M3 11.8h18M9 11.8v8.8M15 11.8v8.8"/><path d="M16.6 3.4c1.7 1.3 2.8 3.3 2.8 5.5"/>',
  ...
  "Kombi":'<rect x="3" y="9" width="18" height="12" rx="2"/><path d="M3 13h18M12 9v12"/><path d="M12 9c-2.5 0-4-1-4-2.5S9.5 4 12 9zM12 9c2.5 0 4-1 4-2.5S14.5 4 12 9z"/>',
  "Packages":'<rect x="3" y="9" width="18" height="12" rx="2"/><path d="M3 13h18M12 9v12"/><path d="M12 9c-2.5 0-4-1-4-2.5S9.5 4 12 9zM12 9c2.5 0 4-1 4-2.5S14.5 4 12 9z"/>',
  ...
  "Happy Hour":'<path d="M3.5 4.5h17L13.4 13v6.5M13.4 19.5H9.2M13.4 19.5h4.2"/><path d="M6.2 8h12"/>',
};
...
function allProductsForCategory(){
  ...
  // Feste Bedienreihenfolge: sortOrder bestimmt die Position, nicht die Kachelgroesse.
...
function updateStaffButtonState(){
  ...
  const blockiert=staffBlockedCartItems();
  button.disabled=blockiert.length>0;
  button.classList.toggle("staff-blocked",blockiert.length>0);
  button.title=blockiert.length?`Nicht auf Personal buchbar: ${blockiert.join(", ")}`:"";
}
...
function splitAnteilFuer(p,registerId){
  ...
  for(const schluessel of Object.keys(rollenGesamt)){
    const wert=Number(schluessel),rolle=rollenGesamt[schluessel]||{};
    const gesamt=Math.max(0,Number(rolle.rolls)||0),muenzenJeRolle=Math.max(0,Number(rolle.coinsPerRoll)||0);
...
document.querySelectorAll("[data-withdraw-reason]").forEach(button=>button.onclick=()=>{withdrawalReason=button.dataset.withdrawReason;document.querySelectorAll("[data-withdraw-reason]").forEach(x=>x.classList.toggle("active",x===button));el("complaintPanel").hidden=withdrawalReason!=="Reklamation";beschrifteWithdrawKnopf();if(withdrawalReason==="Reklamation"){renderComplaintArticles();el("withdrawAmount").value=complaintArticleTotal()?complaintArticleTotal().toFixed(2):""}});
...
document.addEventListener("keydown",async e=>{if(["INPUT","TEXTAREA","SELECT"].includes(document.activeElement.tagName))return;clearTimeout(state.scanTimer);if(e.key==="Enter"){const code=state.scanBuffer.trim();state.scanBuffer="";
  ...
  // Der Sammelspeicher wartet 300 ms auf das naechste Zeichen (vorher 130 ms). Grund: der
  // HW0010 kennt vier Uebertragungsgeschwindigkeiten (High/Medium/Low/Super Low). Wer wegen
  // fehlender Zeichen auf eine langsamere Stufe geht - genau das empfiehlt sein Handbuch -,
  // haette sonst mitten im Code einen Abbruch gehabt: die Kasse haette den Rest als neuen
  // Code gelesen. 300 ms decken auch die langsamste Stufe ab und sind fuer eine Handeingabe
  // immer noch zu kurz, um versehentlich als Scan zu gelten.
  state.scanTimer=setTimeout(()=>state.scanBuffer="",300)});
...
async function enterKioskFullscreen(){
  ...
  try{const target=document.documentElement;if(!document.fullscreenElement&&target.requestFullscreen)await target.requestFullscreen({navigationUI:"hide"});kioskFullscreenActive=!!document.fullscreenElement||document.body.classList.contains("fullscreen-active");syncViewportHeight();syncFullscreenGate();setSystemHint(kioskFullscreenActive||runsAsInstalledApp()?"Vollbildmodus aktiv":"Vollbild konnte vom Browser nicht gestartet werden",kioskFullscreenActive||runsAsInstalledApp()?"ok":"warn")}catch(error){kioskFullscreenActive=false;syncFullscreenGate();setSystemHint("Vollbild ist auf diesem Gerät nur über die installierte App/PWA verfügbar","warn")}
}
